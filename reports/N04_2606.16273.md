...
Theorem 4.1 的结构是标准的「稠密性 ⇒ 半对偶目标逼近 ⇒ Gibbs 律 TV 收敛」，加上一步图特有的处理：嵌入空间的样本一般不在 \(\varphi_\Psi(\Gamma)\) 上，作者用热核平滑 \(t\) 再最近投影，让极限交换（\(m_k,t_k\) 联合取极限）。tropical 嵌入的价值在于它是**内在**的（只依赖边长与圈结构），欧氏实现则依赖画法；两者在同一神经框架内比较。原文承认（§6 限制）投影会在图的分叉点附近引入偏差，热核平滑是为此付出的代价。

## 4. 实验与证据

Table 1（曼哈顿 Uber，\(10^6\) 训练样本，评估用 16,384 点 Sinkhorn 子样本；含「评估噪声底」行——两份独立测试子样本互比的分数）：
...
最好的神经模型把图 \(W_1\) 压到噪声底的 1.2 倍；两个启发式基线差一个数量级。原文注明基线因稠密图 OT 不可行只在 16,384 样本的受限支撑上训练（§5.2）——这既是本篇「扩展性更好」论断的依据，也意味着对比不完全对等。合成图上的完整表见 Appendix C.3。
