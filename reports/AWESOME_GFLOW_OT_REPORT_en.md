...
**Abstract.** Generative Flow Networks (GFlowNets) formulate reward matching as flow conservation on a graph: terminal edge flow equals reward, and inflow to internal states equals outflow. Rewards only anchor the boundaries, leaving a family of valid solutions for internal flows. This report organizes 18 papers around a central theme: on finite graphs allowing cycles, the total internal state flow is proportional to the expected trajectory length (T36, Prop. 3.12, counting internal state visits); for a single source and fixed terminal distribution, the expected length reaches its lower bound \(\sum_xR(x)d_G(s_0,x)/Z\) if and only if each terminal object arrives along its respective shortest path (O07, Thm. 3.4, requiring non-shortest path probabilities to be zero); further, if the first-step edge flow is fixed to a source distribution \(L\), the total internal edge flow objective, after subtracting a constant, equals the Kantorovich optimal transport with graph shortest paths as costs, and under the optimal policy, the joint distribution of "source state \(u\) entered in the first step and terminal object \(x\) before stopping" is an optimal coupling (O08, Thm. 3.2). Each link in this chain of equivalences has a counterpart in classic minimum cost flow and the Beckmann problem (O01, O02). Our assessment: the novelty lies not in the "output policy" itself (classic edge flow normalized also yields local policies, and GSBoG also learns executable policies), but in the combination of "non-acyclic GFlowNet minimum flow objective + fixed marginals at both ends + neural policy parameterization + training and execution on implicit graphs"—the \(20!\) permutation experiment in O08 demonstrates executability, a scale where no exact OT reference value exists. The report also provides a competitive landscape (comparison of three main conference papers ULOT, GSBoG, DDSBM, and two workshop papers, with dimensions including whether the graph is the object to be matched or the venue for transport, whether entropy-regularized, whether time horizon is fixed, and whether an explicit graph is needed), a scan of trends for 2025–2026 (199 arXiv hits, of which 6 are deeply read, 87 are candidate inclusions), ratings for four candidate research topics, and an experimental design ready for immediate commencement. Our assessment: among the four candidates compared, "OT optimality certification with dual potentials as certificates" has the least competition and the necessary foundations are already in place; its core difficulty—that zero balance residual only guarantees flow legality but not minimum total flow, thus certificates must simultaneously handle primal feasibility repair, dual feasibility, and optimality gap, and no computable global certificate exists for implicit graphs—is discussed in Chapter 9 §6b.
...
| Want to start immediately | Decisive experiment in Chapter 9 + Repository tour in Chapter 10 |

**Evidence Discipline.** Every theorem, number, and experimental result is cited (repository ID + original theorem/table number); any inference made by this report, rather than a conclusion from the paper, begins with "Our assessment:". Publication status distinguishes between main conference / journal / workshop / preprint: O08 is ICML 2026 SPIGM Workshop (not main conference, as noted on arXiv page); O07's workshop status is based only on old survey tags used in this repository, without primary evidence, marked as **to be verified**; GSBoG is ICML 2026 main conference; ULOT is NeurIPS 2025 main conference; DDSBM is ICLR 2025 main conference.

**How this report was generated.** Paper-level content in Chapters 2–7 is drawn from corresponding sections (core contributions, assumptions, main thread position, insight) of 18 individual interpretation reports under `reports/`, assembled by `scripts/build_report.py`; Chapters 1, 8, 9, 10, and the appendix are comprehensively written. For notation, derivations, and experimental details for each paper, please refer to the original interpretation reports.

# Chapter 1: Problem: Why is internal flow not unique, and why is this an optimal transport problem?

## 1.1 Rewards only constrain terminal edge flow

GFlowNets define non-negative edge flows \(F(s\to s')\) on a finite directed graph \(G\) (T02's setting is a pointed DAG: single source \(s_0\), single sink \(s_f\); T19/T36 later relax this to allow cycles), requiring two conditions: internal state flow conservation \(\sum_{u}F(u\to s)=\sum_{v}F(s\to v)\), and terminal edge flow equals reward \(F(x\to s_f)=R(x)\). As long as both hold, the terminal distribution of the forward policy \(P_F(s'\mid s)=F(s\to s')/F(s)\) will be \(R(x)/Z\), where \(Z=F(s_0)=\sum_xR(x)\) (T02 core correctness theorem; see `reports/T02_2111.09266.md` §2).

Both constraints are linear (flow conservation: T02 Prop. 19 / Eq. (22); terminal edge flow = reward: T02 v5 Eq. (32)), and the feasible set is a polytope. Rewards only appear on terminal edges, so they fix the "boundary" of the polytope; the degree of freedom for internal edge flows depends on the graph structure. T02 states this in Prop. 18, item 3: on a finite pointed DAG, Markovian flow is uniquely determined by terminal flows and a backward policy \(P_B\) on non-terminal edges (compatible with the graph, normalized over parent sets). In other words, **choosing a \(P_B\) is choosing an internal flow**. T02 §2.6 lists several ways to choose \(P_B\)—uniform distribution over parents, preference for shorter paths, or learning a \(P_B\) that makes the forward policy easier to learn—but does not provide a clear optimization criterion for this degree of freedom. This report focuses precisely on what objective function to equip this degree of freedom with.

## 1.2 More degrees of freedom, and more danger, with cycles

On DAGs, trajectory length has an upper bound, and flow freedom only comes from \(P_B\). Once cycles are allowed, T19 proves that the set of non-negative R-flows is the sum of the "set of non-negative acyclic R-flows" and the "non-negative cycle flow cone \(H^1_+(G)\)" (Prop. 5; the decomposition is not unique, and the acyclic component varies with the decomposed flow). Flow on cycles does not change the terminal distribution but can be arbitrarily amplified. T19 Thm. 3 shows that for ratio-based losses like FM/DB/TB, adding 0-flow can reduce the loss (a structural property, not that any optimization process will infinitely explode flow); Thm. 4 provides **sufficient conditions** for stability for a class of difference-based FM/DB losses (not "only difference-based are stable"). There is only an inequality between total flow and expected trajectory length \(\mathbb E(\tau)\le F_{\mathrm{out}}(S^*)/R(S^*)\) (Thm. 2)—here \(F\) is a given edge flow, and T19 allows the sampler flow actually induced by the sampling process to be smaller than it.

T36 reconstructs this theory into a computable form on finite discrete graphs: constructing flow from a strictly positive \(P_B\) on all edges, where flow is defined as expected visit count times total sink flow \(F(s_f)\), there is a bijection between "strictly positive edge flows satisfying flow matching" and "strictly positive \(P_B\) on all edges + positive \(F(s_f)\)" (Prop. 3.7; O07 subsequently allows zero-probability edges to accommodate shortest path solutions, where policies on zero-flow states cannot be uniquely recovered from flow). Within T36's induced flow framework, the inequality becomes an equality:

\[\sum_{s\notin\{s_0,s_f\}}F(s)=Z\cdot\mathbb E[n_\tau]\qquad\text{(T36 Prop. 3.12 / Eq. (10); }n_\tau\text{ counts internal state visits, repeated visits counted repeatedly)}.\]

This equality is the turning point of the entire report: it connects the total flow, a linear functional, with the expected trajectory length. "Learning a GFlowNet with the shortest expected trajectory" can thus be written as the constrained optimization in T36 Eq. (11)—the objective is linear in flow, but the original variables also include forward and backward policies, and constraints involve products and logarithms; to become an LP on a polytope, policies must first be eliminated and edge flows used as variables (O08 Appendix A.1 does exactly this step), and the strictly positive domain relaxed to non-negative boundaries. Neural parameterization plus balance + flow regularization training with finite weights does not equate to precisely solving this LP.

## 1.3 Why This Is Optimal Transport

With edge flows as variables, a linear objective, linear constraints, and non-negative variables form a linear program with a graph flow structure: the constraints are vertex conservation, and the objective is the total flow. O01 (Peyré Lecture Notes) Prop. 6.23 and O02 (Essid & Solomon) Eq. (1)⇔(3) are two ways of stating the same thing: the Kantorovich problem on a graph with shortest path costs is equivalent to minimum cost flow (discrete Beckmann problem) with edge flows as variables and vertex conservation as constraints. What is missing is a **non-degenerate source-side marginal constraint**: under a single source \(\delta_{s_0}\), the coupling set \(\Gamma(\delta_{s_0},R)\) is a singleton (O01 Remark 3.2), leaving no choice for endpoint coupling—but the **paths** realizing this coupling still have degrees of freedom, and O07 addresses the latter.

O08 precisely fills this gap: it fixes the first-step edge flow to the source distribution \(L\). At this point, the total internal state flow = 1 + total internal edge flow (the first step contributes a constant 1 under normalized marginals, explicitly subtracted in O08 §3.1). The objective of the total internal edge flow after subtracting the constant is equal to \(\min_{\Pi\in\Gamma(L,R)}\sum_{u,x}d_G(u,x)\Pi(u,x)\) (Thm. 3.2), where the coupling is defined as \(\Pi(u,x)=\Pr(s_1=u,\ s_{n_\tau}=x)\). The equivalence holds in two directions: the cost of any feasible flow is no less than the shortest path cost of its endpoint coupling (path length ≥ endpoint distance); any coupling can be assigned along shortest paths to obtain a feasible flow with the same cost. An optimal flow induces some optimal coupling, but aggregating edge flows does not preserve the source labels of an arbitrarily specified coupling. Chapters 3–5 clarify each link in this chain in the order T19 → T36 → O07 → O08.

## 1.4 Our assessment: What is new and what is not

- **Not new**: OT with shortest path costs on graphs ≡ minimum cost flow (classical, Beckmann 1952; O02 2018 provides a regularized version). Note the difference in conventions: O01 Prop. 6.23 uses directed representations of undirected graphs, signed edge flows, and absolute value costs, while O02 uses directed non-negative edge flows and potentially asymmetric costs; this report follows O08's directed non-negative flows and one-sided potential difference constraints.
- **New (Our assessment)**: Not "outputting local policies" itself—classical minimum cost edge flows, when normalized on the support of positive flow, are also local policies, and GSBoG also learns executable policies—but rather the combination of "non-acyclic GFlowNet minimum flow objective + fixed two-sided marginals + neural policy parameterization + training and execution on implicit graphs." O08 §4.2's \(S_{20}\) permutation experiment is a demonstration of executability; this scale lacks precise OT references and cannot serve as evidence of "verified optimal plans having been obtained."
- **Cost**: The LP structure comes from edge flow reparameterization, independent of normalization; however, O08 uses normalized two-sided marginals (Assumption 3.1, \(\sum L=\sum R=1\)) to avoid dealing with unknown normalization constants. In the paper's setting, learning \(Z\) is no longer needed; the GFlowNet characteristic of "only needing unnormalized rewards" is not used in this line of work. If extended to unnormalized rewards, mass matching and normalization constants would need to be handled separately.

## 1.5 Comparison Table of Applicable Conditions and Counting Conventions

| Paper | Graph | Definition of Flow | Positivity / Absorption Condition | Marginal Constraints | Type of Conclusion |
|---|---|---|---|---|---|
| T02 | Finite pointed DAG | Edge flow satisfying FM; Markovian flow | Flow non-negative; total reward non-zero | Terminal edge flow = \(R\) | Sufficient condition: FM + reward matching ⇒ \(P_T=R/Z\); \(P_B\) degrees of freedom (Prop. 18) |
| T19 | General measurable space, allows cycles | Finite non-negative measure; sampler flow ≤ given flow | 0-flow characterizes cycles; absorption derived from finite measure | Terminal flow = \(R\) (measure equality) | Structural properties: ratio loss unstable (Thm. 3); sufficient condition for stable difference loss (Thm. 4); \(\mathbb E(\tau)\) upper bound (Thm. 2) |
| T36 | Finite discrete graph, allows cycles | Expected visit count × \(F(s_f)\), induced by \(P_B\) | \(P_B\) strictly positive on all edges; \(\mathbb E[n_\tau]<\infty\) | \(P_B(x\mid s_f)=R(x)/Z\) | Equality \(\sum_{\text{internal}}F=Z\,\mathbb E[n_\tau]\) (Prop. 3.12); minimum flow optimization Eq. (11) |
| O07 | Finite directed graph, allows cycles, single source | Same as T36 | Relaxed to \(\mathbb E[n_\tau]<\infty\) (Assumption 3.1), allowing zero probability edges | Reward matching | Necessary and sufficient: minimum expected length ⇔ only shortest paths are taken (Thm. 3.4) |
| O08 | Finite directed graph, allows cycles, multiple sources | Edge flow (after policy elimination) | All-pairs reachability; \(\sum L=\sum R=1\) | First-step edge flow = \(L\); terminal edge flow = \(R\) | Exact equality: reduced objective = Kantorovich OT (Thm. 3.2); duality (Thm. 3.3 single source; App. A.4 multiple sources) |

Counting conventions (consistent throughout the report): \(n_\tau\) counts the number of visits to **internal states** (excluding \(s_0,s_f\)), repeated visits are counted repeatedly; O08's \(\mathbb E|\tau|\) is the length of the transport segment, equal to the number of internal edges; the total internal state flow and total internal edge flow differ by a constant 1 under normalized marginals. \(R\) is unnormalized reward in T02/T19/T36/O07, and normalized to a probability distribution in O08. The current main line focuses on graph shortest path OT induced by **unregularized, unit edge costs**; generally weighted costs require corresponding weighted edge cost objectives, and coupling entropy, path space KL, and quadratic edge flow regularization are different objectives that cannot be indiscriminately merged into the same equivalence chain.

# Chapter 2: GFlowNet Basics and Training Objectives

Only content essential to the main line is retained: definitions of flow and correctness theorems, original sources for internal flow degrees of freedom, and the family of training objectives and their meaning in flow space. Full interpretations of each paper can be found in `reports/`.

## 2.1 Overview of Training Objective Family

| Objective | Constraint Granularity | Key Quantity | Relationship to Main Line |
|---|---|---|---|
| FM (T00) | Single state inflow = outflow | Edge flow \(F(s\to s')\) | Directly constrains flow conservation; ratio-type unstable with cycles (T19 Thm. 3) |
| DB (T02) | Single edge \(F(s)P_F(s'\mid s)=F(s')P_B(s\mid s')\) | State flow \(F(s)\) | Requires explicit state flow; T36 public code uses DB + state flow regularization to train minimum flow GFN (O08's own Eq. (20) is regularized TB with a source distribution term) |
| TB (T03) | Entire trajectory \(Z\prod P_F=R\prod P_B\) | \(\log Z\) | \(\log Z\) is the learned baseline; \(Z\) is known in O08's LP setting |
...
> **One Sentence Summary**: This paper reformulates the problem of "sampling composite objects proportionally to their rewards" as a flow conservation problem on a directed acyclic graph, and proves that any policy satisfying flow conservation necessarily yields \(\pi(x)=R(x)/Z\). It addresses the fundamental flaw of autoregressive/MaxEnt RL being biased by the number of paths \(n(x)\) when "an object has multiple generation paths." On the GFlowNet × OT map, it is the origin: the terminal distribution is uniquely fixed by the reward, but the internal flow leaves an entire family of solutions—the paper explicitly writes out this family of solutions in the appendix but provides no selection principle. The main line of OT research aims to equip this degree of freedom with an objective function.
...
| This Repo PDF | `papers/2106.04399.pdf` · Chinese translation `papers_zh/2106.04399.zh.pdf` (not yet generated) |
| Reading Priority | **P0** · All subsequent GFlowNet theories (including OT results) rewrite its flow conservation equation; without reading it, it's impossible to determine which constraints are essential and which were added later |
...
> **One Sentence Summary**: This paper reconstructs the theory of non-acyclic GFlowNets from scratch on finite discrete graphs: it takes \(P_B\) as the primary object, defines flow as "expected number of visits × terminal flow", proves its one-to-one correspondence with \((P_B,F(s_f))\) (Prop. 3.7), thereby definitively closing the T19 issue of "flows can exist but not be samplable". Its two main conclusions—uniqueness of solution when \(P_B\) is fixed, and irrelevance of stability (Corollary 3.11); "minimum expected length ⇔ minimum total flow" when learning \(P_B\) (Prop. 3.12, Eq. (11))—are direct precursors to O07/O08 writing internal flow selection as minimum cost flow / optimal transport.
...
\]
In acyclic cases, expected number of visits = visit probability, consistent with classical definitions; in cyclic cases, they differ, and only the former is conserved (Sec. 3.2).

**Differences from Standard DAG-GFlowNet and T19.**

- With DAG theory (Sec. 2.1): DB condition Eq. (2), FM condition Eq. (3), TB condition Eq. (1) remain unchanged in form, but the trajectory set is infinite, and the support of trajectory length can be unbounded (each fixed and all-edge positive \(P_B\) still has finite expected length, Lemma 3.4; but expected length can be arbitrarily large when policies approach non-absorbing boundaries, with no uniform upper bound across policy families); the DAG fact "fixed \(P_B\) implies unique \(F,P_F\)" is generalized by Prop. 3.8.
...
#### 2. Core Contributions (Numbered as in Sec. 1 of the Original Paper)

**C1. From-scratch Construction of Finite Discrete Acyclic GFlowNets (Sec. 3, Lemma 3.4, Def. 3.5, Prop. 3.6–3.10).**
Conclusion: Starting from a fully positive \(P_B\), the trajectory distribution is well-defined and has finite expected length (Lemma 3.4); the expected visit count flow satisfies DB and FM (Prop. 3.6); there is a one-to-one correspondence between positive conservative edge flows and \((P_B,F(s_f))\) (Prop. 3.7); \(P_F\) exists uniquely (Prop. 3.8), and the reverse also holds (Prop. 3.9); the terminal edge flow equaling the reward \(F(x\to s_f)=R(x)\) implies that the terminal distribution is \(R/Z\) (Prop. 3.10, sufficient condition); the reverse equivalence requires fixing the scale \(F(s_f)=Z\) – generally only \(F(x\to s_f)/F(s_f)=R(x)/Z\).
...
**C2. Stability Irrelevant When \(P_B\) is Fixed (Corollary 3.11).**
Conclusion: Once \(P_B>0\) is fixed, any loss function from acyclic literature can be directly used to learn \(P_F\); the solution is unique, \(\mathbb E[n_\tau]\) is finite, and T19's loss stability "does not apply."
...
Conclusion: \(\mathbb E_{\tau\sim P}[n_\tau]=\frac{1}{F(s_f)}\sum_{s\notin\{s_0,s_f\}}F(s)\), tightening T19 Theorem 2's "≤" to "="; therefore, "learning an acyclic GFlowNet with minimum expected length" is equivalent to "learning an acyclic GFlowNet with minimum total flow," formulated as a constrained optimization in Eq. (11); an approximate solution method is DB + state flow regularization \(\lambda F_\theta(s)\) (Eq. (12)).
The original paper lists "pointing out this equivalence" as one of its key contributions and states that utilizing it is a crucial direction for future research.

**C4. Scale Hypothesis and Experiments (Sec. 4, Appendix B.2).**
Conclusion: In practice, what determines stability is the scale at which errors are calculated: \(\Delta\log F\) (standard DB) can have arbitrarily large lengths without regularization; \(\Delta F\) (SDB) favors small flows and thus does not diverge, but fits the reward distribution worse. Unstable loss + state flow regularization, surprisingly, yields the best sampling quality.
This is a hypothesis plus experiment, not a theorem.

**C5. Acyclic Generalization of Entropy-Regularized RL Equivalence (Theorem 3.13, Appendix A.7, Lemma A.1).**
Conclusion: On a graph-induced deterministic MDP, taking \(r(s,s')=\log P_B(s\mid s')\), \(r(x,s_f)=\log R(x)\), \(\gamma=1\), \(\lambda=1\), the entropy-regularized optimal policy is \(P_F\), and \(V^\star(s)=\log F(s)\) (\(s\ne s_f\); Eq. (13) defines \(V(s_f)=0\), while \(F(s_f)=Z\)), \(Q^\star=\log F(s\to s')\).
...
Reverse all edges of the graph, and starting from \(s_f\), perform a random walk according to \(P_B\), letting \(s_0\) be an absorbing state. For any intermediate state \(s\), the return probability \(p_s:=\mathbb P[\exists t>0:Y_t=s\mid Y_0=s]<1\): Assumption 3.1 guarantees the existence of an acyclic path from \(s_0\) to \(s\), and \(P_B>0\) guarantees that the probability of returning to \(s_0\) along it is strictly positive; once at \(s_0\), it's impossible to return to \(s\). Thus \(\mathbb P[N'_s>k]=p_s^k\),
\[
...
\]
Finite expected length, in turn, implies \(\sum_\tau P(\tau)=1\): otherwise, infinite trajectories would occur with positive probability, contradicting finite expectation. Editorial note: Compare with T19: T19 derives absorption from "flow is a finite measure" (its Theorem 2), while this paper derives finite flow from "\(P_B>0\) + connectivity"; these two paths converge on finite positive graphs.

##### 3.2 Visit Probability Not Conserved, Expected Visit Count Conserved (Counterexample in Sec. 3.3)
...
- Edge visit probability: \(a\to b\), \(b\to c\), \(c\to s_f\) are all 1, \(c\to b\) is \(\tfrac12\). At \(b\): in \(1+\tfrac12\ne\) out \(1\); at \(c\): in \(1\ne\) out \(1+\tfrac12\). This is the " \(1\ne1+0.5\) " in the original text.
- Expected visit count: \(\mathbb E[k]=\sum_kk\,2^{-(k+1)}=1\), so \(F(b\to c)=2\), \(F(c\to b)=1\), \(F(a\to b)=F(c\to s_f)=1\) (taking \(F(s_f)=1\)). At \(b\): in \(1+1=\) out \(2\); at \(c\): in \(2=\) out \(1+1\). Conservation is restored.

The general proof of Prop. 3.6 writes this observation as an identity: for each trajectory, point-wise \(\mathbb I\{s_t=s\}=\sum_{s''\in\mathrm{in}(s)}\mathbb I\{s_{t-1}=s'',s_t=s\}=\sum_{s'\in\mathrm{out}(s)}\mathbb I\{s_t=s,s_{t+1}=s'\}\); taking the expectation yields FM; the DB condition \(F(s\to s')=F(s')P_B(s\mid s')\) uses the Markov property of the reverse chain plus Fubini (Lemma 3.4 guarantees commutativity). \(F(s_0)=F(s_f)\) because each trajectory visits \(s_0\) exactly once.

##### 3.3 Existence and Uniqueness: Positive Conservative Flow \(\Leftrightarrow (P_B,F(s_f))\) (Prop. 3.7, Appendix A.3)

Given a fully positive and FM-satisfying \(F\), define \(P_B(s\mid s')=F(s\to s')/\sum_{s''}F(s''\to s')\). Let \(\hat F\) be the expected visit count flow induced by it. By Prop. 3.6, \(\hat F(s\to s')=\hat F(s')P_B(s\mid s')=C(s')F(s\to s')\), where \(C:=\hat F/F\). Both are conservative, so
\[
...
Meaning: T19's "zombie flow" \(F-\bar F\) (T19 Def. 5, Prop. 2) is always zero here—every positive conservative flow is precisely sampled by its own \(P_B\). Appendix B.4 describes the same thing as a system of linear equations Eq. (17) \(\hat F(s)=\sum_{s'\in\mathrm{out}(s)}P_B(s\mid s')\hat F(s')\), \(\hat F(s_f)=F(s_f)\), whose solution is the row corresponding to \(s_f\) of the fundamental matrix \(N=(I-Q)^{-1}\) of the absorbing chain (Kemeny–Snell Theorem 3.2.1), multiplied by \(F(s_f)\).

Prop. 3.8 (\(P_F\) uniqueness): Repeating the calculation of Prop. 3.6 for the forward chain yields \(F(s\to s')=F(s)P_F(s'\mid s)\). Combining this with DB gives \(P_F(s'\mid s)=F(s')P_B(s\mid s')/F(s)\). Existence relies on \(F(s_0)=F(s_f)\) making the telescopic products of \(\prod P_B\) and \(\prod P_F\) equal. Prop. 3.9 is the converse: any positive triplet \((F,P_F,P_B)\) satisfying DB induces the same trajectory distribution, and \(F\) is the expected visit count flow of \(P_B\). Prop. 3.10: The terminal edge is visited at most once, \(F(x\to s_f)=F(s_f)\,\mathbb P[s_{n_\tau}=x]\), so reward matching \(\Leftrightarrow\) Eq. (9), and \(F(s_0)=F(s_f)=Z\), \(P_B(x\mid s_f)=R(x)/Z\).

##### 3.4 Why Internal Flows Are Not Unique When Terminal Distributions Are the Same

Combining the conclusions from 3.3:

- Reward matching only constrains \(P_B(\cdot\mid s_f)=R/Z\) (Prop. 3.10). For every other \(s'\ne s_0,s_f\), \(P_B(\cdot\mid s')\) is an arbitrary positive distribution on \(\mathrm{in}(s')\), totaling \(\sum_{s'}(|\mathrm{in}(s')|-1)\) free parameters.
- Different \(P_B\) values yield different edge flows (Prop. 3.7 is a bijection), but the terminal distribution is always \(R/Z\) (Prop. 3.10). Therefore, "non-unique internal flows" is not pathological; it is simply the parameter space of \(P_B\). T19's "adding a 0-flow" translates to "changing \(P_B\)".
- The original text Sec. 3.5 (immediately following Prop. 3.10) points out that T19 overlooks a common knowledge in acyclic literature: manually choosing \(P_B\) (e.g., \(1/|\mathrm{in}(s')|\) for non-terminal states, and \(R/Z\) for \(s_f\)) can yield a valid solution. The problem of unknown \(Z\) is circumvented by learning unnormalized flows or treating \(Z\) as a learnable parameter.

Editorial note: (Extending the calculation from the example in 3.2 by replacing \(P_B(c\mid b)\) with a free parameter \(q\), numerical values not from the original text): The probability of circling \(k\) times is \((1-q)q^k\), \(\mathbb E[k]=q/(1-q)\). Taking \(F(s_f)=Z\):
\[
F(a)=Z,\qquad F(b)=F(c)=\frac{Z}{1-q},\qquad
\mathbb E[n_\tau]=\frac{F(a)+F(b)+F(c)}{Z}=1+\frac{2}{1-q}.
\]
The unique terminal state is \(c\). Any \(q\in(0,1)\) yields the same terminal distribution, but the internal flow (measured by \(F/Z\)) increases from \((1,1,1)\) as \(q\to0\) to infinity as \(q\to1\). For \(q=\tfrac12\), it is \((1,2,2)\) from the original text, and \(\mathbb E[n_\tau]=5\). On this graph, "internal flows with the same terminal distribution" form a curve parameterized by \(q\), rather than a single point. The difference between cyclic and acyclic graphs is only that: when the reward scale is fixed, the set of positive flows on a DAG is bounded (the closure allowing zero flow is compact; generally, the feasible set is a multi-dimensional set, not a curve), while on a cyclic graph, it goes to infinity.

##### 3.5 How to select a minimum flow (Prop. 3.12, Eq. (10)–(12), Appendix B.1)

The proof of Prop. 3.12 is a single line: the trajectory length is the sum of visit counts for each state. Taking the expectation and dividing by \(F(s_f)\), we get
\[
...
\]
The feasible set is "all valid \(P_B\) corresponding to positively conserved flows" as described in 3.4; the objective is linear. In the example from 3.4, it is \(Z\,[1+2/(1-q)]\) (3.4 already set \(F(s_f)=Z\); dividing by \(Z\) gives the expected length \(1+2/(1-q)\)). The infimum is \(3Z\) at \(q\to0\), which corresponds to the shortest path \(s_0\to a\to b\to c\to s_f\). This is how minimum flow is selected: the minimum of a linear objective on a closed convex polytope lies on its boundary, and the boundary precisely corresponds to "some \(P_B\) taking 0" – meaning backward edges are no longer used, and detours are pruned. Two points need clarification:

- The minimum point is outside the positivity assumption of Definition 3.5 (\(P_B(c\mid b)=0\)). This paper does not directly solve Eq. (11), but uses the \(\lambda F_\theta(s)\) regularization in Eq. (12) to approximate it from within; \(\lambda\) balances expected length and reward distribution accuracy (Figure 5). The theory of closure is provided by T19 Prop. 5: non-negative R-flows = acyclic R-flows + non-negative combinations of directed cycles. Removing any cycle component strictly decreases \(\sum_sF(s)\), so the minimum point must be acyclic.
- Appendix B.1: During on-policy training, the expected gradient of the regularization term is \(\mathbb E_{\tau\sim P_F}\sum_t\nabla_\theta F_\theta(s_t)=\frac{1}{2F_\theta(s_f)}\nabla_\theta\sum_sF_\theta(s)^2\) – **provided that** \(F_\theta\) exactly equals the state flow induced by \(P_F^\theta\) (Appendix B.1 explicitly makes this assumption first). In general training, where the predicted flow head and policy occupancy flow are not yet consistent, the expected update direction should be written as \(\sum_sd_{P_F^\theta}(s)\nabla_\theta F_\theta(s)\) (\(d\) is the occupancy measure, and we do not differentiate with respect to the sampling distribution). Thus, one cannot unconditionally claim that the entire training optimizes a fixed quadratic objective. Under this premise, it is a **quadratic penalty on state occupancy**. If \(P_F(\cdot\mid s_0)\) is fixed to be uniform and regularization is only added to the first state of each trajectory, the weights return to uniform; the authors state that the difference is not significant in experiments. **Revision**: The initial draft of this report stated "this sum of squares is exactly the quadratic regularization in O02 (its Eq. (4))" – I have checked O02 Eq. (4), and its quadratic term is the sum of squares of **edge flows** \(\sum_eJ_e^2\), whereas here it is the sum of squares of state flows \(\sum_s(\sum_{e\in\mathrm{out}(s)}J_e)^2\), which includes cross-terms between outgoing edges of the same state. These are generally not equal, nor is it necessarily strictly convex with respect to edge flows; therefore, O02's conclusions regarding edge flow uniqueness and small regularization thresholds cannot be directly inherited.

The relationship between Corollary 3.11 and Eq. (11): fixing \(P_B\) pins a point in the feasible set; stability only affects the optimization path, not the endpoint. Learning \(P_B\) is about picking a point in the feasible set. In this case, without an objective function (the standard loss is zero across the entire feasible set), it will be arbitrarily pushed to infinity by optimization dynamics – this is the paper's re-diagnosis of "flow trapped in cycles": it's not that the loss is "unstable," but that the problem is underdetermined.
...
\]
Standard DB = \(\Delta_{\log F}^2\); SDB = \(\log(1+\varepsilon\Delta_F^2)(1+\eta F_\theta(s))\) (Eq. (5)). The two can be interchanged in scale, yielding four combinations, all measured in Sec. 4.
...
- The two curves on the \(\Delta F\) scale quickly plateau and their derivatives approach zero as \(x\) decreases: there is almost no gradient when the flow needs to be increased, but a large gradient when it needs to be decreased.
...
- The formal statement of the scale hypothesis (beginning of Sec. 4): when \(P_B\) is trainable, the primary factor determining whether "average trajectory length is controlled" in practice is the error scale; \(\Delta\log F\) without regularization can lead to arbitrarily large lengths, while \(\Delta F\) biases towards small flows and does not diverge.

##### 3.7 Entropy-Regularized RL Equivalence (Theorem 3.13, Lemma A.1)

For the MDP \(\mathcal M_G\): states = vertices, actions = \(\mathrm{out}(s)\), deterministic transitions, \(\gamma=1\), \(r(s,s')=\log P_B(s\mid s')\), and \(r(x,s_f)=\log R(x)\). The entropy-regularized value in Eq. (13) at \(\lambda=1\) simplifies to
\[
...
\]
so the optimal \(\pi^\star\) is the policy with zero KL divergence, which is the unique \(P_F\) from Prop. 3.8. The soft Bellman equation Eq. (14) is satisfied by \(Q^\star(s,s')=\log F(s\to s')\) (using \(\log F(s\to s')=\log F(s')+\log P_B(s\mid s')\) from Prop. 3.6), and \(V^\star=\log\sum_{s'}e^{Q^\star}=\log F(s)\). Lemma A.1 addresses well-definedness in the undiscounted case: it calls this the "regularized shortest path problem." Under the assumptions that \(r\le0\) (\(r=0\) if and only if \(|\mathrm{in}(s')|=1\), and each optimal policy has finite expected length—the original text of Lemma A.1 states "if and only if") and that the optimal policy has finite expected length, the optimization is strictly concave (linear reward + strongly concave relative conditional entropy) on the occupancy measure polytope
\[
\mathcal K=\Big\{d\ge0:\ \sum_{s'}d(s,s')=\sum_{s''}d(s'',s),\ \sum_{s'}d(s_0,s')=1,\ \sum_{s''}d(s'',s_f)=1\Big\}
\]
and the solution is unique. The bijection between \(d^\pi(s,s')=\pi(s'\mid s)d^\pi(s)\) and Prop. 3.7 on the reverse graph tightly couples occupancy and policy. \(\mathcal K\) shares the conservation structure with the flow polytope of 3.5 (pointwise conservation only for internal states), but only fixes the total source mass to 1, without fixing individual termination edge masses \(R(x)/Z\). Thus, it is a larger feasible region than that of Eq. (11). It becomes identical only after adding the constraint \(d(x,s_f)=R(x)/Z\) for each termination edge.

##### 3.8 Key Aspects of the Training Loop (Sec. 3.5, Appendix B.1, B.3, C.1)

The original text does not provide pseudocode. The training procedure is pieced together from Eq. (12), Eq. (16), and Appendix C:

1.  The network shares a backbone, with three linear heads outputting \(\log F_\theta(s)\), logits for \(P_F\), and logits for \(P_B\) (Appendix C.2). When \(P_B\) is trainable, there is also a scalar \(\log Z_\theta\).
...
3.  Calculate the DB residual for each transition: Eq. (4) for intermediate transitions; Eq. (16) \(\big[\log Z_\theta-\log|S\setminus\{s_0,s_f\}|-\log P_B(s_0\mid s,\theta)-\log F_\theta(s)\big]^2\) for the first transition; for terminal transitions, \(F_\theta(s_f)P_B(x\mid s_f)\) is replaced by \(R(x)\).
4.  Add \(\lambda F_\theta(s_t)\) for each visited state not \(s_0,s_f\) (\(\log Z_\theta\) is not added, Appendix B.3).
5.  One Adam step; \(\log Z_\theta\) uses a 10x learning rate (following Malkin et al. 2022).

Consequences of on-policy training (Appendix B.1): Step 4 is, in expectation, \(\frac{\lambda}{2F_\theta(s_f)}\nabla\sum_sF_\theta(s)^2\). The choice of a loss other than FM is intentional: FM does not explicitly parameterize \(P_B\) and cannot be used for experiments with fixed \(P_B\) (Appendix C.1).

##### 3.9 Theorem-Level Comparison with T19 (Editor's Summary)

| T19 (B24) | T36 (This paper) | Relationship |
|---|---|---|
| Definition 5 sampler flow \(\bar F\le F\) | Definition 3.5 expected visit count flow | T36 directly defines \(\bar F\); under positivity, \(\bar F=F\) (Prop. 3.7) |
| Theorem 2: \(E(\tau)\le F_{\mathrm{out}}(S^*)/R(S^*)\) | Prop. 3.12: \(\mathbb E[n_\tau]=\sum_sF(s)/F(s_f)\) | Inequality tightened to equality (original text states "refinement") |
| Theorem 2 absorption (finite measure \(\Rightarrow\)) | Lemma 3.4 absorption (\(P_B>0\) + Assumption 3.1 \(\Rightarrow\)) | Assumptions are reversed, conclusion is the same |
...
| Definition 3 stability; Theorem 3 instability | Corollary 3.11: irrelevant when \(P_B\) is fixed; scaling hypothesis | Rewrites "loss property" as "whether the problem is underdetermined + error scale" |
| Theorem 1: stable loss + flow regularization (coefficient tends to zero, global minimum sequence converges) \(\Rightarrow\) acyclic limit | Eq. (11)–(12): minimum total flow + \(\lambda F_\theta(s)\) | Same idea; T36 shows, at the method and experimental level, that standard loss with state flow regularization can also train effectively, **but does not prove** T19 Theorem 1 after removing the stability prerequisite |
| Example 2 stable DB | Eq. (5) SDB | Same loss; T36 experiments show it is biased under \(\Delta F\) scaling |
| (None) | Theorem 3.13 entropy-regularized RL equivalence | New to T36 |

#### 4. Experiments and Evidence

**Common Setup (Sec. 4.1, Appendix C).** Loss notation: DB / SDB denotes the loss form, \(\Delta\log F\) / \(\Delta F\) denotes the error scale, \(\lambda=C\) denotes the state flow regularization strength; (DB, \(\Delta\log F\)) = Eq. (4), (SDB, \(\Delta F\)) = Eq. (5), (DB, \(\Delta\log F\), \(\lambda\)) = Eq. (12). Two \(P_B\) settings: fixed (\(\mathrm{out}(s_0)=\{s_{\mathrm{init}}\}\), for other states \(P_B\) is uniform over parent nodes, \(P_B(s_0\mid s_{\mathrm{init}})=1-\varepsilon\), \(\varepsilon=10^{-8}\)) and trainable (\(\mathrm{out}(s_0)=S\setminus\{s_0,s_f\}\), \(P_F(\cdot\mid s_0)\) fixed uniform, \(\log Z_\theta\) learnable, Eq. (16)). All are on-policy, Adam learning rate \(10^{-3}\), \(\log Z_\theta\) learning rate \(10^{-2}\); SDB uses \(\varepsilon=1.0\), \(\eta=10^{-3}\) (larger \(\eta\) shortens trajectories but "significantly interferes with sampling fidelity"). The metric is not the average reward from T19, because it does not measure the fit to \(R/Z\).

| Task | Setup (Source) | Metrics | Results |
|---|---|---|---|
| Hypergrid \(7\times7\) | States \(\{0,\dots,H-1\}^D\), transitions ±1 and within bounds, each state can terminate; \(R=R_0+R_1\prod_i\mathbb I\{0.25<\lvert s_i/(H-1)-0.5\rvert\}+R_2\prod_i\mathbb I\{0.3<\lvert s_i/(H-1)-0.5\rvert<0.4\}\), \((R_0,R_1,R_2)=(10^{-3},0.5,2.0)\) (Appendix C.2); MLP 2×256, batch 16, \(2\cdot10^6\) trajectories total | TV distance \(=\tfrac12L_1\) between empirical distribution of last \(2\cdot10^5\) samples and \(R/Z\) (Sec. 4.2, Appendix C.2; \(L_1\) labeled on y-axis in figures), average trajectory length | With fixed \(P_B\), both (DB, \(\Delta\log F\)) and (SDB, \(\Delta F\)) converge to the true \(\mathbb E[n_\tau]\) for that \(P_B\) (exact solution in Appendix B.4), verifying Corollary 3.11; trainable \(P_B\) yields shorter trajectories for all losses; \(\Delta F\) scale converges slower and with slight bias; unregularized \(\Delta\log F\) does not diverge in this environment (Figure 1) |
| Hypergrid \(20^4\) | Same as above, only trainable \(P_B\) (fixed \(P_B\)'s \(\mathbb E[n_\tau]\) is orders of magnitude larger than small grid, unusable) | \(L_1\), average length, \(\log Z_\theta\) | \(\Delta F\) scale (DB and SDB) both learn biased policies, with significantly larger bias than \(7\times7\); \(\Delta\log F\) + \(\lambda=0.001\) correctly fits and learns \(\log Z\); final lengths of all methods are similar, but \(\Delta F\) is shorter in the middle of training (Figure 2). Unregularized \(\Delta\log F\) average length significantly increases within the observed range (original phrasing is "tends to infinity", but Figure 4 only shows a finite training interval, and related runs did not complete the budgeted amount due to increased sampling costs, so it cannot be read as mathematical asymptotic divergence) (Appendix D, Figure 4); larger \(\lambda\in\{0.1,0.01,0.001,0.0001\}\) leads to shorter lengths and more biased policies (Figure 5) |
| Permutation \(S_n\), \(n=4\) | Cayley graph: \(n-1\) adjacent transpositions + one right cyclic shift; \(R(s)=\exp(\tfrac12\sum_k\mathbb I\{s(k)=k\})\) (harder than T19's \(\mathbb I[s(1)=1]\), which has trivial policies); MLP 2×128, batch 512, \(10^5\) steps | \(L_1\) of \(C(k)\) (probability vector of \(k\) fixed points), average length | Conclusions for fixed vs. trainable \(P_B\) are the same as hypergrid, the only difference being that (SDB, \(\Delta F\)) converges faster with trainable \(P_B\) (Figure 6) |
...
Our assessment: For \(n=20\), the \(\Delta\log Z=42.98\) for both \(\Delta F\) losses exactly equals the true value \(\log Z\approx42.98\), meaning the normalization constant estimation is severely inaccurate (Table 1 reports the absolute error \(|\log Z_\theta-\log Z|\), which does not uniquely imply \(\log Z_\theta\approx0\)—the other sign branch is approximately 85.96; aggregating across seeds also cannot recover the sign of each run). Moreover, \(\mathbb E[n_\tau]\approx2\), indicating they completely failed to learn the normalization constant. The average trajectory length reported in the table is about 2 (which, according to Definition 3.2, means about two internal state visits and about three edges; the mean does not imply all trajectories are of equal length). "Stability" here means collapse. \(\Delta\log F\) + regularization reduces all three errors to the \(10^{-2}\) order of magnitude for both \(n\) values. As \(\lambda\) decreases from \(10^{-3}\) to \(10^{-5}\), \(C(k)\,L_1\) for \(n=20\) drops from 0.041 to 0.017/0.014, at the cost of \(\mathbb E[n_\tau]\) increasing from 3.2 to 7.5. This is an empirical trade-off between "sampling accuracy vs. trajectory length" in the penalty training of Eq. (12) (Eq. (11) treats reward matching as a hard constraint and does not inherently involve such a trade-off).

**To what extent is the evidence supported?**

- Direct verification: Corollary 3.11 (the fixed \(P_B\) curves in Figures 1 and 6 fall on the analytical \(\mathbb E[n_\tau]\)); Prop. 3.12 is implicit in all length curves (Prop. 3.12 is an analytical identity; the original paper **does not** separately report consistency checks between predicted total internal state flow and measured trajectory length at the same checkpoint, so this cannot be called empirical verification).
- Hypothesis level: The scale hypothesis is supported by Figures 2–4 and Table 1, but the original paper itself calls it a hypothesis, and the explanation for Figure 3 is heuristic.
- Untested: Theorem 3.13 has no RL algorithm experiments (not provided in the original paper); Appendix B.1 states that the difference between "sum of squares vs. sum" is "not significant" but provides no numbers.
- Three conclusions from Sec. 4.4: Fixed \(P_B\) can avoid stable loss and regularization, but manually selecting \(P_B\) with low \(\mathbb E[n_\tau]\) is difficult; when learning \(P_B\), \(\Delta F\) is scale-stable but often fails to fit; \(\Delta\log F\) fits well to scale but must be accompanied by state flow regularization.
...
- **Theorem 3.13**: \(P_B>0\) is fixed and satisfies reward matching; \(\gamma=1\), \(\lambda=1\); Lemma A.1 additionally requires \(r\le0\) (\(r=0\) if and only if \(|\mathrm{in}(s')|=1\), and each optimal policy has finite expected length—the original Lemma A.1 states "if and only if") and finite expected length for the optimal policy—the proof normalizes \(R\) to \(Z=1\) and uses \(\log R<0\) to satisfy the former.
...
- **Experimental Setup**: When \(P_B\) is trainable, \(P_F(\cdot\mid s_0)\) must be fixed (otherwise it is not learnable when \(\mathrm{out}(s_0)=S\setminus\{s_0,s_f\}\)), and \(P_B(s_0\mid s)\) must be trainable (Appendix B.3).
- **Positive Statement of Applicability**: Any GFlowNet on a finite graph with positive edges, with or without cycles, can learn \(P_F\) (with fixed \(P_B\)) using losses from acyclic literature, or learn \((P_F,P_B)\) using "standard loss + \(\lambda F_\theta(s)\)"; implicit flow parameterizations like SubTB and Deleu et al. 2022 can also add the same regularization (Appendix C.1).
...
- In Sec. 6, equating the polytope \(\mathcal K\) from Lemma A.1 with the feasible region of O02 Eq. (3) is the editor's judgment: both are "non-negative + pointwise conservation + fixed endpoint margins," differing only in multi-source/single-source and normalization.
- Table 1 values are transcribed cell by cell from the PDF; the original uses blue/red to mark optimal metrics and shortest lengths, which are not preserved here.
- **External Review Revisions (Codex, gpt-6-astra, 2026-09)**: Withdrew "the sum of squares of state flows is exactly the quadratic regularizer of O02 Eq. (4)" (O02 is sum of squares of edge flows, there are cross-term differences); added back the premise of Appendix B.1 (\(F_\theta\) is consistent with occupancy flow); Prop. 3.10 changed to a sufficient condition with scale explanation; added factor \(Z\) to target value in Sec. 3.4 example; difference between \(\mathcal K\) and the feasible region of Eq. (11); state range of \(V^\star=\log F\); "if and only if" in Lemma A.1; \(\Delta\log Z\) being absolute error cannot imply \(\log Z_\theta\approx0\); chapter numbering Sec. 3.4 → 3.5; narrowed phrasing for "tends to infinity" and "direct verification."

# Chapter 4: Minimum Flow ⇔ Shortest Path (O07)

O07 transforms T36's minimum total flow objective into a necessary and sufficient characterization: the expected trajectory length is minimized if and only if the policy places all probability mass on the shortest path (Thm. 3.4); and reduces unweighted graph pathfinding to training a non-acyclic GFlowNet with flow regularization. It is a special case of O08 where "\(R\equiv1\), single-source"; O08 Thm. 3.3 explicitly states that it "recovers the corresponding claim of Morozov et al. (2026)". Publication status: ICML 2026 SPIGM Workshop, not main conference. This chapter is a full interpretation report.

> **One sentence summary**: It proves that the expected trajectory length \(\mathbb E[n_\tau]\) of a non-acyclic GFlowNet is minimized if and only if the policy places all probability mass on the shortest path from \(s_0\) to the terminal state (Theorem 3.4, necessary and sufficient). Based on this characterization, any unweighted graph pathfinding problem is reduced to "training a non-acyclic GFlowNet with flow regularization": reverse edges, treat the target node as \(s_0\), set every node as a terminal state, take \(R\equiv1\), and the trained backward policy is a shortest path solver. It achieves comparable solution lengths to CayleyPy Cube on a 3x3x3 Rubik's Cube with a smaller beam budget (Table 1). It is a direct bridge to O08: O08 merely replaces the single source \(s_0\) here with a source distribution \(L\).
...
| Code | [github.com/GreatDrake/gfn-pathfinding](https://github.com/GreatDrake/gfn-pathfinding) (given at the end of Sec. 1 in the original text) |
| This repository PDF | `papers/2603.01786.pdf` (14 pages: 10 pages main text + Appendices A/B/C) |
| Reading Priority | **P0** — Essential prerequisite for O08. The necessary and sufficient proof for the chain "why minimum flow favors shortest path" is only here; O08 only provides its LP dual version. |
...
**Policy and Consistency** (original Eq. (1)(2)): \(\mathbb P(\tau)=\prod_tP_F(s_{t+1}\mid s_t)=\prod_tP_B(s_t\mid s_{t+1})\), with reward matching written as \(P_B(x\mid s_f)=R(x)/Z\).

**Flow and Expected Length** (original Eq. (3), i.e., T36 Definition 3.5 / Proposition 3.12):
...
Therefore, "minimum expected length" = "minimum total flow", which is the starting point of the entire paper. Sec. 2.1 of the original paper also provides a practical approach: adding \(\lambda\mathcal F_\theta(s)\) to the loss, for example, DB + state flow regularization:
...
2. **The assumption \(P_B>0\) has been replaced**. T36 requires \(P_B>0\) on all edges for \(\mathbb P\) to be well-defined, but policies that minimize \(\mathbb E[n_\tau]\) must assign 0 to non-shortest path edges on non-terminal edges in **positive flow states** (Theorem 3.4 directly constrains the probability of complete trajectories; backward transitions on zero-flow states do not affect the objective and can be arbitrary). These two requirements are in direct conflict. The original paper uses **Assumption 3.1: \(\mathbb E[n_\tau]<+\infty\)** as a replacement, where \(n_\tau\) is the number of steps for a backward random walk starting from \(s_f\) and following \(P_B\) to reach \(s_0\). This is weaker and more natural, while still preserving " \(P_B\) is the transition kernel of an absorbing Markov chain, and \(\mathbb P\) is a legitimate probability distribution on \(\mathcal T\)" (Appendix A provides a derivation, citing Kemeny & Snell 1969);
3. The objective is not sampling quality itself, but **using a backward policy for pathfinding**: \(\ell(s')\) denotes the shortest path length from \(s_0\) to \(s'\), which always exists by Assumption 2.1.

### 2. Core Contributions (by original numbering)

**Contribution 1 (Theorem 3.4, necessary and sufficient characterization)**: Under Assumption 2.1 + Assumption 3.1, a backward policy \(P_B\) satisfying reward matching \(P_B(x\mid s_f)=R(x)/Z\) (the statements in Lemma 3.3 and Theorem 3.4 of the original paper omit \(Z\), but Eq. (2), the construction in Lemma 3.3, and Appendix B all use \(R/Z\)) minimizes \(\mathbb E[n_\tau]\) **if and only if** for any trajectory \(\tau\) terminating at \(x\in\mathcal X\), \(n_\tau\ne\ell(x)\Rightarrow\mathbb P(\tau)=0\). That is: minimizing expected trajectory length ≡ assigning zero probability to all non-shortest path trajectories. Remark 3.5 of the original paper adds: by T36 Proposition 3.8 (for any \(P_B\), there exists a unique equivalent \(P_F\), and vice versa), this theorem also holds for forward policies, as the condition only concerns the trajectory distribution itself.

**Contribution 2 (Sec. 3.2, constructive reduction)**: The problem of finding "shortest paths from all points to a target point \(v_g\)" on any unweighted finite graph can be reduced to training a non-acyclic GFlowNet that minimizes \(\mathbb E[n_\tau]\). Unlike approaches that "learn a value function and then feed it to search" (Agostinelli et al. 2019; Chervov et al. 2025b), this method directly learns a policy whose optimal solution is the exact shortest path.

**Contribution 3 (Sec. 3.3, training algorithm)**: Regularized trajectory balance (original Eq. (7)(8), Algorithm 1), plus two engineering decisions: truncating training trajectory length to \(N_{\max}\) and calculating TB for every prefix.
...
**How the construction works**: Define \(\mathrm{par}:S\setminus\{s_0,s_f\}\to S\setminus\{s_f\}\). For each \(s\), arbitrarily choose a parent node \(\mathrm{par}(s)\) such that \((\mathrm{par}(s)\to s)\in E\) and \(\ell(s)=\ell(\mathrm{par}(s))+1\). Such a parent node must exist, as the predecessor of \(s\) on a shortest path from \(s_0\) to \(s\) satisfies this condition. Then set \(P_B(x\mid s_f)=R(x)/Z\) and \(P_B(s\mid s')=\mathbb I\{s=\mathrm{par}(s')\}\). Thus, backward sampling first jumps from \(s_f\) to some \(x\), and then at each step moves strictly one edge closer to \(s_0\), necessarily sampling a shortest path. Therefore, \(\mathbb E[n_\tau\mid s_{n_\tau}=x]=\ell(x)\).
...
- **(⇒) Optimal ⇒ Only shortest paths**: By contradiction. If there exists a \(\tau\) terminating at \(x'\) such that \(n_\tau>\ell(x')\) and \(\mathbb P(\tau)>0\), then \(\mathbb E[n_\tau\mid s_{n_\tau}=x']>\ell(x')\). For other terminal states, we still have \(\ge\ell(x)\), so \(\mathbb E[n_\tau]>\sum_x\frac{R(x)}{Z}\ell(x)\). However, Lemma 3.3 shows that this right-hand side can be attained, which is a contradiction.

**Key mechanism in one sentence**: The marginal distribution of terminal states is fixed by reward matching (\(R>0\) ensures that the weight of each \(x\) is strictly positive), so \(\mathbb E[n_\tau]\) is a weighted sum of conditional expected lengths under a fixed set of weights; the lower bound for each term is \(\ell(x)\), and these lower bounds can be attained simultaneously. Therefore, "minimum total flow" does not **tend towards** shortest paths—it is **precisely equivalent** to only traversing shortest paths. Original Remark 3.5 also points out that the optimal \(P_B\) is generally **not unique**: for any \(s\), it can arbitrarily distribute probability among "parent nodes that lie on a shortest path," as long as it assigns 0 to parent nodes not on a shortest path.

**Byproduct of Remark 3.6**: If the construction in Lemma 3.3 uses a degenerate \(P_B(s\mid s')=\mathbb I\{s=\mathrm{par}(s')\}\), then \(\mathrm{par}\), after excluding \(s_f\), defines a directed tree rooted at \(s_0\). The corresponding forward policy has a closed form \(P_F(s'\mid s)=V(s')/V(s)\), where \(V(s)\) is the sum of rewards of **all terminal states** in the subtree rooted at \(s\) (including \(s\) itself if it can terminate). The original Remark 3.6 uses "leaves" as a shorthand; generally, terminal states need not be leaves. In this paper's pathfinding construction, every state can be a terminal state, and only accumulating leaves would miss internal terminal states and lose the stopping probability (citing Bengio et al. 2021).

#### 3.2 Relationship with T36: Removing an Assumption, Adding a Characterization
...
3.  Transitions \(\mathcal E\) are the **reverse** of edges in \(G\), plus an edge from every state (except \(s_f\)) to \(s_f\).

**Why this is a valid environment**: \(s_0\) has no incoming edges (because \(v_g\)'s outgoing edges were removed and then reversed); \(s_f\) has no outgoing edges; every state has an edge to \(s_f\), so **all non-\(s_f\) states are terminal states** (this is repeatedly utilized by the training algorithm later); \(s_0\) is reachable from every state, because in the original graph every point can reach \(v_g\), which means \(v_g\) can reach every point in the reversed graph.

**Why this solves the original problem**: Take any positive reward \(R\) (the original text recommends task-independent \(R\equiv1\)). By Theorem 3.4, the \(P_B\) that minimizes \(\mathbb E[n_\tau]\) assigns non-zero probability only to shortest paths from \(s_0\) (i.e., \(v_g\)) to terminal states (i.e., all vertices); since \(P_B\) traverses the reversed graph, it is equivalent to traversing the original graph in the forward direction. Thus, sampling from \(P_B\) starting from an arbitrary state \(s\) yields the shortest path from \(v\) to \(v_g\) in the original graph. When \(R\equiv1\), the optimal \(\mathbb E[n_\tau]\) is exactly the average shortest path length over the entire graph, \(\frac1{|V|}\sum_{v\in V}\ell(v)\), which provides a directly readable ground-truth reference for training.

**Division of Labor for the Two Policies** (original text Sec. 3.2 "Forward policy"): \(P_B\) is the product—solving from any configuration to the target; \(P_F\) is the auxiliary component needed for training—walking backward from the target, sampling states according to \(R\) (uniform), and deciding when to stop (\(P_F(s_f\mid s)\)). In the context of a Rubik's Cube: \(P_B\) restores any configuration in the minimum number of steps, while \(P_F\) scrambles the solved state into a uniformly distributed configuration in the minimum number of steps.

#### 3.4 Training Objective (Sec. 3.3, Eq. (7)(8), Algorithm 1)

Two practical modifications, both justified in the original paper:

- **Truncating trajectory length to \(N_{\max}\)**: In early training, \(\mathbb E[n_\tau]\) is extremely large, and the cost of sampling full trajectories is prohibitive in large environments (Brunswic et al. 2024 also does this). During sampling, the stop action \(P_F(s_f\mid s,\theta)\) is masked, yielding partial trajectories of fixed length \(\tau'=(s_0\to\cdots\to s_{N_{\max}})\).
- **Discarding DB for TB**: In early experiments, DB converged extremely slowly. The original paper provides two explanations: TB has more efficient credit assignment (Malkin et al. 2022); more critically, **the training signal in this construction primarily comes from the target state \(s_0\) itself**. DB is defined on single transitions, most of which do not contain \(s_0\), whereas TB is defined on entire trajectories, and by construction, every trajectory contains \(s_0\). The authors leave a deeper analysis of this phenomenon for future work.

Since every state is a terminal state, every prefix \(\tau'_{0:i}\) plus a terminal transition forms a complete trajectory. Thus, we sum over all prefixes (original Eq. (7)). Also, because \(R\equiv1\), \(Z=|V|\) is known, and there is no need to learn \(\log Z_\theta\). The flow also does not need to be learned separately: in an environment where "every point has an edge to \(s_f\)", \(\mathcal F(s)=R(s)/P_F(s_f\mid s)\) – this step is derived by combining the terminal edge flow \(\mathcal F(s\to s_f)=R(s)\) (T36 Proposition 3.10) and a part of DB \(\mathcal F(s\to s_f)=\mathcal F(s)P_F(s_f\mid s)\) (T36 Proposition 3.8). The final objective (original Eq. (8)):
...
Algorithm 1 is: sample \(B\) trajectories of length at most \(N_{\max}\) → compute \(\frac1B\sum_i\nabla_\theta\mathcal L_{\mathrm{regTB}}\) → update. The original paper emphasizes that **simultaneously optimizing \(P_F\) and \(P_B\) is a critical component of the algorithm**, and points out that specialized backward policy optimization methods (Jang et al. 2024; Gritsaev et al. 2025) might further improve training.

#### 3.5 Beam Search at Test Time (Sec. 3.4)

The theoretically optimal policy precisely yields the shortest path, but on large graphs, what is learned is only an approximation. Therefore, beam search is used at test time (following Chervov et al. 2025b): with width \(W\), at each step, all continuations are expanded, and the top \(W\) are retained based on the log product of backward transition probabilities of the trajectory (i.e., \(\sum_t\log P_B(s_t\mid s_{t+1},\theta)\)). It stops upon reaching \(s_0\). An additional deduplication heuristic (dropping duplicate states at each step) is added, which the original paper states slightly improves performance. \(W=1\) degenerates to a greedy search on \(P_B\), \(\arg\max_sP_B(s\mid s',\theta)\) – **if the policy is optimal, greedy search still produces the shortest path**, because the probability of non-shortest path transitions must be 0 (Theorem 3.4). The original paper also mentions entropy-regularized MCTS (Morozov et al. 2024; Xiao et al. 2019) as an optional alternative.

### 4. Experiments and Evidence

Network: MLP with 6 `ReLU(Linear(LayerNorm(x)) + x)` residual blocks, one-hot state input, \(P_F,P_B\) share a backbone with different linear heads (forward logits count = number of outgoing edges in original graph + 1 stop action). The difference from previous work is replacing BatchNorm with LayerNorm to stabilize training and evaluation. Implemented in JAX, with the entire training loop JIT-compiled (Appendix C, citing Tiapkin et al. 2025's gfnx).

#### 4.1 Swap Puzzle (Sec. 4.1, Figure 2, Figure 3 top-left)

Task: Sort an arbitrary \(n\)-permutation using the minimum number of adjacent transpositions, i.e., pathfinding on the Cayley graph of \(S_n\) with adjacent transpositions as generators. **Ground truth is computable**: optimal solution length = number of inversions in the permutation (two reasons given in the original paper: only the identity permutation has 0 inversions; one adjacent transposition changes the number of inversions by ±1).

- Scale: \(n=15\) and \(n=20\), Cayley graphs with approximately \(1.3\cdot10^{12}\) and \(2.4\cdot10^{18}\) states respectively (original Sec. 4.1).
...
- Results (Figure 2): After sufficient training, **both greedy and beam search protocols yield exact shortest paths for every permutation in the test set**; the faithfully sampled policy is on average close to optimal.
- Generalization: The model for \(n=20\) only saw about \(10^9\) states during training, a tiny fraction of \(2.4\cdot10^{18}\) (original Sec. 4.1).
- Cost: Running 100k iterations for \(n=20\) takes 15 minutes on a single NVIDIA H200 (Figure 2 caption).
- Hyperparameters (Appendix C.1): 100,000 iterations, batch size 128, AdamW (weight decay \(10^{-5}\)), lr \(3\cdot10^{-4}\), hidden size 1024, gradient norm clipping threshold 100, \(N_{\max}=50\); \(\lambda=10^{-3}\) (for \(n=15\)), \(\lambda=10^{-4}\) (for \(n=20\)).

#### 4.2 Rubik's Cube (Sec. 4.2, Table 1)

Competitor: CayleyPy Cube (Chervov et al. 2025b, NeurIPS 2025), the current SOTA ML method for this task, which has been shown to outperform DeepCubeA (Agostinelli et al. 2019) and EfficientCube (Takano 2023) in solution length and runtime efficiency. Fairness treatment: For CayleyPy, two models were trained: one with "original hyperparameters" and one with "network size equivalent to this paper," and the better one was chosen. Both methods used beam search. Only 90° face rotations were allowed, so solution length is measured in QTM. Test set: 100 instances for 2×2×2 from Chervov et al. 2025b, and 1000 instances for 3×3×3 from Agostinelli et al. 2019. Solve rate refers to the proportion of instances where a valid path was found within 100 steps.
...
(Numbers are copied directly from the original Table 1; "x" is the original mark, indicating that no valid solution was found for the entire test set at that beam width, so solution length is not reported. 10.64 is the optimal average length for this 2×2×2 test set, verified by BFS in the original paper.)
...
- 3×3×3: In the range \(W\in[2^0,2^9]\), this paper performs better (\(W=2^9\): 23.49 vs 24.34). For \(W\in\{2^{12},2^{15},2^{18}\}\), both methods are comparable (22.42/22.44, 21.70/21.61, 21.24/21.15, with CayleyPy slightly better in the latter two tiers).
- Runtime (\(W=2^{18}\), single H200): This paper's 25M parameter model takes an average of **1.74 seconds** to solve one configuration, while CayleyPy's 4M parameter model takes **6.19 seconds**. The explanation provided in the original paper is that CayleyPy and DeepCubeA-like methods need to run a forward pass for each neighbor to estimate value/distance (3×3×3 has 12 neighbors, thus 12 times more forward passes), whereas this paper outputs backward policy logits for all neighbors in a single forward pass.
- Hyperparameters (Appendix C.2): For 2×2×2, 500,000 iterations / batch 128 / hidden 1024 / \(\lambda=10^{-2}\) / \(N_{\max}=12\) were used. For 3×3×3, 1,000,000 iterations / batch 2048 / hidden 2048 / \(\lambda=5\cdot10^{-7}\) / \(N_{\max}=24\) were used. The original paper notes that both \(N_{\max}\) values are smaller than the longest path in their respective environments, yet still train well, serving as further evidence of generalization ability. The same number of sampled configurations was used when comparing with CayleyPy.

#### 4.3 \(\lambda\) Ablation (Sec. 4.3, Figure 3)

Observation: **Within the effective settings shown in the figure, larger \(\lambda\) performs better**—the maximum values in Figure 3's legend are 0.001 (Swap \(n=15\)) and 0.01 (2×2×2), both yielding effective results and adopted in Appendix C. The authors also state in the figure caption that even larger values would lead to complete failure, but do not provide the failing values. This is a limited ablation, not a universally monotonic rule. The resulting rule of thumb for hyperparameter tuning: evaluate after very few iterations and choose the largest \(\lambda\) that "can still find a valid path to the target."

#### 4.4 Assessment of Evidence Strength

- Directly supported by experiments: Greedy/beam search protocols on Swap Puzzle yield **exact** shortest paths (with ground truth inversions); 2×2×2 achieves the BFS-verified optimal 10.64 with 16 times smaller beam budget; 3×3×3 outperforms SOTA in the small beam range and is comparable in the large beam range; wall-clock time for solving a single configuration is 1.74s vs 6.19s.
- Author's inference, not directly verified: "TB is better than DB because of credit assignment + every trajectory contains the target state"—the original text explicitly states this as a hypothesis and leaves it for future work; generalization ability is supported by two indirect pieces of evidence: "number of seen states / total number of states" and "effectiveness even when \(N_{\max}\) is smaller than the longest path," with no out-of-distribution testing.
- Not provided in the original paper: Specific numerical table for Swap Puzzle (only Figure 2 curves, precise numbers cannot be cited in the report); comparison with classic BFS/bidirectional BFS on medium-sized graphs; explanation for why \(\lambda=5\cdot10^{-7}\) for 3×3×3 is five orders of magnitude smaller than \(10^{-2}\) for 2×2×2; weighted graphs (authors list this as future work in Sec. 5).
...
3.  **Unweighted Graph, Hop Distance**: \(\ell(\cdot)\) counts the number of edges. Weighted/cost-sensitive settings are explicitly listed as future work in the original paper's Sec. 5.
...
6.  **Theorem is about exact optimal solutions**: Theorem 3.4 characterizes the policy that minimizes \(\mathbb E[n_\tau]\). Actual training uses a soft version with a \(\lambda\) penalty, and too large a \(\lambda\) can cause training to collapse (Sec. 4.3). Therefore, for large graphs, beam search is needed to mitigate approximation errors. This is not a theoretical flaw but a real cost when translating the theorem into an algorithm.

**Scope of Application**: Unweighted pathfinding where the state space is too large to store, but transition rules are clear and reversible (Cayley graphs, permutation puzzles, combinatorial configuration spaces). Particularly suitable for "train once, solve from any starting point" amortized modes, and scenarios where all neighbors need to be scored at each step (outputting logits for all neighbors in a single forward pass is its structural advantage over value-based methods). Not suitable for weighted graphs, scenarios requiring optimality certificates, or graphs small enough for BFS to run directly.

### 6. Position in the GFlowNet × OT Main Line

**Predecessors**:

-   **T19** (Brunswic et al. 2024, AAAI): Non-acyclic GFlowNets and flow regularization ideas; the practice of truncating training trajectory lengths also comes from it.
...
-   **O08** (arXiv 2606.06272): Directly built upon this paper. The bridging relationship is precise—this paper deals with **single-source** (one point \(s_0\)) to multiple targets, with cost \(\ell(x)=d(s_0,x)\); O08 adds a constraint \(\mathcal F(s_0\to u)=L(u)\) to replace the single-point source with a **source distribution**. The objective (note the distinction between mass and count: O07's optimal expected length is \(\sum_xR(x)\ell(x)/Z\), optimal total internal state flow is \(\sum_xR(x)\ell(x)\); O08's total internal state flow under unit mass is \(1+\) total internal edge flow, which becomes Eq. (11) after removing the constant) then changes from \(\sum_xR(x)\ell(x)\) to \(\sum_{u,x}d(u,x)\Pi(u,x)\), i.e., Kantorovich OT. The proofs in both directions also correspond one-to-one: Lemma 3.2 in this paper ("actual path length \(\ge\ell(x)\)") corresponds to the inequality in O08 Eq. (18); Lemma 3.3 in this paper ("construct along shortest path") corresponds to "lay mass along shortest paths" in O08 Part 1. Additionally, O08 Theorem 3.3 re-derives the conclusions of this paper using LP duality + complementary slackness (O08 states "recovers the corresponding claim of Morozov et al. (2026)"), and inherits the TB objective and per-prefix computation techniques from this paper (flow regularization and the trade-off between "cost vs. marginal matching accuracy" in O08 Sec. 4.2 are cited from T36, not this paper; O08 also does not directly adopt the \(\lambda\) tuning rules from this paper).
-   From another perspective, the division of labor between the two papers is: **This paper provides a probabilistic proof (necessary and sufficient) for "why minimum flow selects shortest paths", while O08 provides an LP duality proof for the same fact and gives certificates (\(\pi^\star_x=d(x)\), complementary slackness)**. To implement a primal-dual GFN-OT, both papers must be read: the semantics of the objective function are here, and the identity of the dual variables is in O08.

**Contribution to the Main Line**: It is the first link in the chain "internal flow selection = optimal transport". The chain has three segments: T36 defines flow as expected visit counts, making the "minimum total flow" objective expressible → This paper proves that minimum total flow ⟺ only shortest paths are taken → O08 adds source distribution constraints, elevating it to Kantorovich OT. Without this paper's link, O08's Theorem 3.2 could still be proven (its proof is self-contained), but the questions "why is this objective function worth minimizing" and "what does the path structure of the optimal solution look like" are only answered by this paper, and answered more strongly than O08's dual version—**necessary and sufficient**, rather than just providing support set inclusion.

### 7. Reusable Insights and Open Problems

1.  **"Reward matching anchors the termination edge" is the hidden fulcrum for the minimum flow criterion to work.** The lower bound \(\sum_xP_B(x\mid s_f)\ell(x)\) in Lemma 3.2 is constant and can be used for comparison only because \(P_B(x\mid s_f)=R(x)/Z\) is fixed. If the termination distribution is relaxed, the solution minimizing \(\mathbb E[n_\tau]\) would degenerate into "terminating only at the state closest to \(s_0\)," losing the guarantee of a fixed termination edge and coverage of all endpoints. The quality might concentrate on the closest endpoints, but the positive probability trajectories would still be shortest paths. Any GFN-OT variant that modifies reward matching (especially unbalanced versions) must re-evaluate this argument.
2.  **The optimal \(P_B\) is not unique and can be treated as a free regularization degree** (Remark 3.5). How probabilities are distributed among shortest path parent nodes is completely free. This means a second objective can be optimized within the constraint of "only taking shortest paths": maximum entropy (diverse shortest paths), minimum variance, or robustness to unseen states. Theorem draft: Construct a maximum entropy GFlowNet on the shortest path subgraph, making the shortest path distribution uniform conditioned on each endpoint (if endpoint \(x\) has \(K_x\) shortest paths, each has probability \(R(x)/(ZK_x)\); only when \(R(x)/Z\propto K_x\) is it a uniform distribution over all shortest paths across endpoints). This is directly applicable to applications requiring diverse solutions (multiple puzzle solutions, molecular synthesis pathways).
3.  **Training signal localization is the authors' hypothesis for TB outperforming DB** (Sec. 3.3.1, explicitly left for future research in the original text). This argument can be transferred to any GFlowNet task where "signals are concentrated in a few special states": DB's per-transition loss is diluted by many signal-free transitions, while TB's per-trajectory loss inherently contains signals in every trajectory. Testable hypothesis: Scan a range of partial trajectory lengths for SubTB as an interpolation parameter. A testable prediction is whether convergence speed monotonically changes with "the probability that the segment contains the target state" (segment length simultaneously changes credit assignment, gradient variance, and computational budget).
4.  **Outputting all neighbor logits in a single forward pass is a structural advantage of policy-based methods over value-based methods** (Sec. 4.2). On 3×3×3, this leads to a 12x difference in forward passes, measured at 1.74s vs 6.19s. This advantage will be amplified in environments with large branching factors (molecular editing action spaces can reach hundreds) and is worth reproducing as an independent efficiency argument in other tasks.
5.  **The cliff-like failure of \(\lambda\) requires a better mechanism.** The rule of thumb in Sec. 4.3 (taking the largest \(\lambda\) that still finds a path) is essentially manual trial and error. Moreover, the **chosen** \(\lambda\) for 2×2×2 and 3×3×3 differs by \(2\times10^4\) times (\(10^{-2}\) vs \(5\cdot10^{-7}\), about 4.3 orders of magnitude). The networks, batch sizes, and truncation lengths for the two tasks are simultaneously different, suggesting sensitivity to the task or training configuration, and whether it is strongly coupled with graph diameter/scale. Possible replacements: Replace the \(\lambda\) penalty with an explicit constraint on \(\mathbb E[n_\tau]\) (augmented Lagrangian or dual ascent to automatically tune \(\lambda\)). (Note: Length constraints correspond to scalar multipliers, while O08's node potentials correspond to flow conservation constraints; these are not the same dual variables. Also, Thm. 3.3's distance potential only applies to single-source problems without initial step constraints—no analytical reference exists here.)
6.  **Weighted graph extension (listed as future work in original Sec. 5) is technically low-risk:** Replace \(\ell(\cdot)\) with weighted shortest paths, and change the flow regularization to a weighted sum over **edges** \(\lambda\sum_{(s,s')}c(s,s')\mathcal F_\theta(s\to s')\) (state cost \(c(s)\mathcal F(s)\) cannot express different edge costs from the same state; in this case, the objective is to minimize expected path cost rather than hop count). The arguments in Lemma 3.2/3.3 only use "path cost \(\ge\) shortest path cost" and "existence of a deterministic parent mapping along the shortest path." Both hold under strictly positive weights. Strictly positive edge weights are a sufficient condition for proof; if non-negative edge weights are allowed, the existence of shortest paths, zero-cost cycles, construction of strictly decreasing distance parent nodes, and absorption time need to be handled separately, and ties can be broken by hop count among minimum cost paths.
7.  **The lack of optimality certificates is a major gap in the current form.** The Swap Puzzle has inversion count, and 2×2×2 has BFS, so they can be verified; 3×3×3 has no ground truth, so only horizontal comparison is possible. O08's complementary slackness \(\mathcal F^\star(s\to s')(\pi^\star_{s'}-1-\pi^\star_s)=0\) provides an edge-by-edge verifiable alternative: by additionally learning a \(\pi_\phi(s)\approx d(s)\), a gap proxy can be reported on graphs without ground truth. This is the most direct actionable benefit of reading the two papers together.

8.  **\(N_{\max}\) truncation lacks theoretical cost analysis, but empirically can be much smaller than the graph diameter.** 2×2×2 uses \(N_{\max}=12\), and 3×3×3 uses \(N_{\max}=24\). The original text states that both are smaller than the longest path in their respective environments, and training is still effective (Appendix C.2). Truncation changes the trajectory distribution during training: \(\mathcal L_{\mathrm{regTB}}\) is evaluated only on prefixes of length \(\le N_{\max}\), which means balancing constraints are applied only to the part of the state space "within \(N_{\max}\) steps of the target." This leaves a provable question: Let \(p\) be the proportion of states whose shortest path length is \(>N_{\max}\). Can the gap between the minimal solution of the training objective and the true minimum flow solution be controlled by \(p\)? If such a bound can be given, \(N_{\max}\) would transform from an engineering knob into a justified hyperparameter. The original text treats it purely as a cost control measure, without analysis.

9.  **The performance of greedy \(W=1\) should be used as an optimality diagnostic, not just a cheap evaluation protocol.** Theorem 3.4 states that the optimal policy assigns zero probability to non-shortest path transitions, so greedy search can also find shortest paths. Conversely, if greedy search fails, it indicates that non-shortest path transitions still carry maximum probability, meaning the policy has a localizable deviation from optimality. The original text implicitly uses this signal in Figure 2 by not plotting points where greedy search fails (the caption explicitly states these points are omitted), but it does not quantify it. An actionable approach: Report the curve of "difference between greedy and \(W\) large solution lengths" over training, and stratify by state-to-target distance to localize whether errors are concentrated in near-target or far-target regions. This is much more informative than a single average solution length.

10. **The reduction construction encodes the "target point" into \(s_0\), so one model serves only one target point.** In Rubik's Cube, the target is the solved state, which is not an issue. However, for multiple targets (shortest path between any pair of start and end points, or targets changing over time), the target needs to be conditioned into the network \(P_B(s\mid s',v_g)\). In this case, Theorem 3.4 holds only for each \(v_g\) individually, and there is no theoretical guarantee of consistency across targets. This is another axis of amortization orthogonal to O08's amortization problem: O08 learns a transport policy for a given pair \((L,R)\) (amortization across distribution pairs is a further research extension, not implemented in O08), while here, the amortization is over the "target vertex \(v_g\)." The latter has additional structure available in Cayley graphs—group translation invariance means \(d(u,x)=d(e,u^{-1}x)\), so conditioning can in principle be simplified to replacing the state with \(v_g^{-1}s\). This point is not mentioned at all in the original text, but it does not hold for general graphs beyond permutation puzzles, and is worth independent verification.
...
1.  **Publication Status**: The PDF does not contain a venue line (the template appears to be a UAI/AISTATS-style two-column format + Supplementary Material pagination). According to the `data/papers.yaml` repository entry, it is registered as `ICML 2026 SPIGM Workshop`, and both the report and bibtex label it as a Workshop paper, not a main conference paper.
2.  **Theorem Numbering**: The main conclusion is **Theorem 3.4**, the two lemmas are **Lemma 3.2 / Lemma 3.3**, and the weakened backward policy assumption is **Assumption 3.1** (the environment assumption is **Assumption 2.1**). Proofs are in Appendix B, and discussion of the backward policy assumption is in Appendix A.
3.  **No Numerical Table for Swap Puzzle**: The original paper only reports results using curves in Figure 2, without providing a table. Therefore, the report only restates qualitative conclusions (greedy and beam search yield exact shortest paths) and explicit numbers from the caption (500 test cases, \(\approx1.3\cdot10^{12}\) / \(2.4\cdot10^{18}\) states, \(10^9\) visited states, 15 minutes / H200), without reading values from the curves.
4.  **"x" in Table 1** is the original paper's mark, inferred from the caption to mean "no valid solution found for the entire test set at this beam width, so average solution length is not reported"; the original paper does not explain each instance, this is my interpretation.
5.  **§7, items 2, 5, 6, 8, 9, 10 are my inferences and experimental drafts**, not provided in the original paper; the direction of weighted extension in item 6 is endorsed by Sec. 5 of the original paper (listed as future work), but the restriction "zero-weight edges can create zero-cost cycles" is my addition. In item 10, the translation invariance of Cayley graphs \(d(u,x)=d(e,u^{-1}x)\) is common group theory knowledge, not a conclusion of the paper.
6.  **Correspondence with O08** (Lemma 3.2 ↔ O08 Eq. (18), Lemma 3.3 ↔ O08 Part 1) is my mapping; O08 only vaguely states "recovers the corresponding claim of Morozov et al. (2026)" without detailed alignment.
7.  **External Review Revisions (Codex, gpt-6-astra, 2026-09)**: Corrected the omission of \(Z\) in reward matching (following the original typo), reversed the "product of logarithms" for beam scoring, clarified the applicability of Theorem 3.4's edge-wise conclusion (only for positive flow states), the uniqueness constraint for padding \(\mathcal G'\) with zeros, the abbreviation of "leaves" in Remark 3.6, the interpretation of \(\lambda\) in Figure 3, the magnitude difference of \(\lambda\) between the two tasks (\(2\times10^4\), not five orders of magnitude), the comparison with O08's objective, handling edge weighting for weighted extensions, the scope of the maximum entropy draft, and misrepresenting the authors' hypothesis of TB ≫ DB as a verified mechanism.

# Chapter 5: Core Theorem: GFlowNet Min Total Flow ⇔ Kantorovich Optimal Transport (O08)

This chapter is the core of the entire report, containing the full interpretation of O08: the four-step proof skeleton (bilinear → LP → divergence constraint → two-sided squeeze), duality and complementary slackness certificates, eight prerequisite assumptions, all numerical results from hypergrid and permutation experiments, and the basis for the judgment "the equivalence is classic, the interface is new." Publication status: ICML 2026 SPIGM Workshop, not main conference.

> **One sentence**: By adding another constraint to the minimum total flow problem of non-acyclic GFlowNets—fixing the first-step edge flow to the source distribution \(L\)—the minimum total flow objective precisely becomes Kantorovich OT with graph shortest paths as the ground cost. The distribution of trajectory endpoints sampled by the optimal forward policy is the optimal coupling (Theorem 3.2). The equivalence between shortest-path OT on graphs and min-cost flow is a classic result. The value of this paper lies not in this equivalence itself, but in translating it into the GFlowNet language, where the output is no longer a coupling matrix, but a routing policy \(P_F(s'\mid s)\) that can progressively execute legal local actions on a huge implicit graph. This work defines the interface for the GFlowNet × OT main thread in this repository and is the only one that provides geometric principles for "which internal flow to choose."

| Field | Content |
|---|---|
| arXiv | [2606.06272](https://arxiv.org/abs/2606.06272) (v1 [cs.LG] 2026-06-04; PDF footer self-signed "Preprint. June 5, 2026") |
| Publication | **ICML 2026 SPIGM Workshop (Workshop paper, not main conference)** |
...
| Code | Not publicly available (neither the main text nor the appendix provides a code link; Appendix B only states that the implementation is based on public code from Morozov et al. 2025) |
| PDF in this repository | `papers/2606.06272.pdf` (17 pages: 8 pages main text + Appendix A theory + Appendix B experimental details) |
| Reading Priority | **P0** — The only work in the entire repository that equates the GFlowNet objective function with Kantorovich OT; all subsequent GFN-OT topics must start from its set of assumptions |
...
**Contribution 1 (Original Sec. 1, Item 1 + Theorem 3.2 + Appendix A.1)**: The minimum total flow learning problem in T36 can be equivalently reformulated as a linear program. When the initial edge flow distribution is fixed in this LP, the minimum total flow objective precisely becomes a Kantorovich OT problem with graph distance as cost, which is also equivalent to the discrete graph form of the Beckmann problem (Beckmann 1952; Essid & Solomon 2018). The optimal forward policy under this setting samples optimal paths between the initial and terminal distributions, thereby inducing an optimal coupling. As a special case, this construction reproduces the shortest path theory of Morozov et al. (2026) (O07 in this repository).

**Contribution 2 (Original Sec. 1, Item 2 + Sec. 3.3 + Sec. 4)**: The GFlowNet learning framework provides a feasible means to approximate graph OT solutions. Based on neural parameterization, the GFlowNet objective can learn a policy that approximates the optimal coupling; experiments reproduce exact OT solutions when exact solutions are computable, and provide approximations at scales where exact solutions are not computable (permutations \(n=20\)).

The original text places theorem-level results in three locations. The report below refers to them by these numbers:

- **Theorem 3.2** (Original Sec. 3.2): Under Assumption 3.1, the reduced primal problem Eq. (11) is equivalent to the Kantorovich problem Eq. (8) between \(L\) and \(R\), and \(\mathrm{GFlow}^\star=\mathrm{OT}^\star\) (Eq. (12)); furthermore, if \(\mathbb P^\star\) is the trajectory distribution induced by the \(\mathrm{GFlow}^\star\) solution, then \(\Pi^\star_{u,x}:=\sum_{\tau:u\rightsquigarrow x}\mathbb P^\star(\tau)\) is an optimal coupling for Eq. (8).
- **Theorem 3.3** (Original Sec. 3.2 end; full version is Theorem A.5 in Appendix A.3—the original paper's cross-reference writes the subsection as A.5, see Editorial note): The dual of the original problem Eq. (10) **without** the first-step constraint is \(\max_\pi\sum_{x}R(x)\pi_x\) s.t. \(\pi_{s_0}=0,\ \pi_{s'}-\pi_s\le 1\) (\(s'\ne s_f\)). Three conclusions: any feasible \(\pi\) satisfies \(\pi_s\le d(s)\); \(d(\cdot)=|\tau_{s_0,\cdot}|\) is feasible and dual optimal; if \(R(x)>0\ \forall x\), then any optimal \(\pi^\star\) satisfies \(\pi^\star_x=d(x)\) on terminal states. Complementary slackness gives \(\mathcal F^\star(s\to s')\big(\pi^\star_{s'}-(1+\pi^\star_s)\big)=0\), meaning the optimal flow can only lie on the tight subgraph—and the tight subgraph is the shortest path subgraph. The original text explicitly states that this "recovers the corresponding claim of Morozov et al. (2026)".
- **Proposition A.4 and Appendix A.4**: Derivation of the lifted LP's dual (Eq. (22)→Eq. (23)); the dual of the extended problem (including the first-step constraint) is \(\max_\pi\sum_x R(x)\pi_x+\sum_u L(u)(-\pi_u)\). By relabeling \(a_x=\pi_x,\ b_u=-\pi_u\), we get \(a_x+b_u\le d(u,x)\) (Eq. (24)→Eq. (25)), which has the same form as the Kantorovich dual.

### 3. Key Points of Method and Theoretical Derivation

The entire chain consists of four steps: **bilinear → linear (LP) → reduced to divergence constraint → squeezed from both sides to obtain equality**, plus one more step for the dual to provide a certificate.

#### 3.1 Step 1: Eliminating Policies, Turning Bilinear Constraints into Linear (Appendix A.1)
...
#### 3.4 Step 4: \(\mathrm{GFlow}^\star\ge\mathrm{OT}^\star\) (Proof of Theorem 3.2, Part 2 in original)
...
From Assumption 3.1, we have \(Z=1\). Define \(\Pi_{u,x}:=\sum_{\tau:u\rightsquigarrow x}\mathbb P^\star(\tau)\). The two marginals can be directly read from the constraints: \(\sum_x\Pi_{u,x}=P^\star_F(u\mid s_0)=L(u)\) (first step constraint), \(\sum_u\Pi_{u,x}=R(x)\) (termination constraint), so \(\Pi\) is a valid coupling. Then calculate the objective value (Eq. (17) in original):

\[
\mathrm{GFlow}^\star=\sum_{s\to s'\in E^\circ}\mathcal F^\star(s\to s')=\sum_\tau|\tau|\,\mathbb P^\star(\tau),
\]

where \(|\tau|\) is the number of edges in trajectory \(\tau\) that fall within \(E^\circ\). Finally, use \(|\tau_{u,x}|\ge d(u,x)\) (\(d\) is the shortest path length) for term-by-term bounding (Eq. (18) in original) to get \(\mathrm{GFlow}^\star\ge\sum_{u,x}d(u,x)\Pi_{u,x}\ge\mathrm{OT}^\star\) (Eq. (19)). Combining with Eq. (16) yields Eq. (12).

**The essence of this half**: The endpoints of trajectories sampled by the policy, when marginalized, must form a valid coupling, and any path actually traversed is no shorter than the shortest path. Thus, minimizing the total flow simultaneously forces **two independent conditions**: (a) each sampled path degenerates into the shortest path between its endpoints (equality in Eq. (18)); (b) the endpoint coupling \(\Pi\) itself is Kantorovich optimal (equality in Eq. (19)). Both are indispensable—a policy that cross-matches mass and transports it along respective shortest paths satisfies (a) but not (b), and its total flow is strictly greater than OT* (numerical counterexample in `experiments/` of this repository: cost is 60–78% higher). This also indicates that the optimal GFlowNet provides more information than just the coupling matrix: it not only tells "how much mass to transport from \(u\) to \(x\)" but also "which specific valid local actions to take".

#### 3.5 Duality and Certificates (Theorem 3.3 / Appendix A.2–A.4)

The derivation of Proposition A.4 is clean: taking the infimum over \(\mathcal F(s)\) (unconstrained sign) requires its coefficient to be zero, yielding \(c=\bar\alpha+\bar\beta\); taking the infimum over \(\mathcal F(s\to s')\ge0\) requires the coefficient to be non-negative, yielding \(\alpha_s+\beta_{s'}\ge0\) and \(\eta_x\le\alpha_x+\beta_{s_f}\). From \(c=\bar\alpha+\bar\beta\), we get \(\alpha_{s_0}=0,\beta_{s_f}=0,\beta_s=1-\alpha_s\ (s\in\mathcal I)\). Substituting these back into the edge inequality simplifies it to \(\alpha_{s'}-\alpha_s\le1\), which becomes Eq. (23) by renaming \(\alpha\) to \(\pi\).
...
3. \(R>0\) makes the objective strictly monotonic, so any optimal solution must reach the upper bound \(\pi^\star_x=d(x)\) at termination states.

**The identity of the dual variable is BFS distance**, which is worth stating directly: the optimal dual potential in Eq. (23) is precisely the hop distance from \(s_0\). Complementary slackness \(\mathcal F^\star(s\to s')(\pi^\star_{s'}-1-\pi^\star_s)=0\) thus transforms "the support of the optimal flow \(\subseteq\) shortest path subgraph" into an equality that can be checked edge by edge. This is the LP dual proof of the main theorem in O07. Appendix A.4 further maps the dual of the extended problem \(\max_\pi\sum_xR(x)\pi_x-\sum_uL(u)\pi_u\) to the Kantorovich dual Eq. (25) via \(a_x=\pi_x,b_u=-\pi_u\) and telescoping sum along paths \(a_x+b_u=\pi_x-\pi_u\le d(u,x)\).

#### 3.6 Training Objective (Sec. 3.3, Eq. (20) in original)

Instead of detailed balance + flow regularization, the authors use regularized trajectory balance with leward (the authors state that Morozov et al. 2026 has proven TB to be more effective in non-acyclic pathfinding, and TB is most suitable for their setting):
...
Each of the three components has its origin: \(L(s_1)\) in the numerator replaces the usual \(Z\) in TB—because \(Z=1\) and the first step distribution is fixed to \(L\), the original paper writes it as leward matching \(P_F(s\mid s_0)=L(s)\); \(R(s_{n_\tau})\) in the denominator handles reward matching \(P_B(s\mid s_f)=R(s)\); the regularization term \(\lambda R(s)/P_F(s_f\mid s)\) is the state flow \(\mathcal F(s)\) at the termination state (derived from the reward matching condition in Eq. (5)), which is the penalty implementation of the minimum total flow objective. Training is on-policy: a batch of trajectories is sampled using \(P_F\) at each step to compute the loss. When every internal state is a termination state (\(\mathcal X=\mathcal I\)), Eq. (20) can be computed for each prefix of the sampled trajectory, leading to higher sample efficiency (citing Morozov et al. 2026); this option was adopted in the experiments.

### 4. Experiments and Evidence

Two environments, all results averaged over 3 random seeds (Sec. 4.1 and Sec. 4.2 in original both state "averaged over three seeds"). Model: 2 hidden layers, 128 width MLP, one-hot input state, \(\mathcal F_\theta(s)\), \(P_F\), \(P_B\) share backbone, different linear heads; on-policy, batch 512, AdamW, \(\mathrm{lr}=10^{-3}\), weight decay \(10^{-4}\); **all experiments performed on CPU** (Appendix B).

#### 4.1 Hypergrid (Original Table 1)

Environment: Grid \(\{0,\dots,H-1\}^D\) plus \(s_0,s_f\); starting from \(U\) according to \(L\), transitions are single-coordinate \(\pm1\) and within bounds, every state has a terminal transition (thus \(\mathcal X=\mathcal I\)). Reward \(R\) is corner-shaped multimodal (definition in Appendix B.1, following Bengio et al. 2021 / Madan et al. 2023 / Malkin et al. 2022), while leward \(L\) is moon-shaped and ball-shaped (formulas in Appendix B.1). TV is estimated using \(2\cdot10^5\) model samples. \(\mathrm{OT}^\star\) is computed by the POT solver (Flamary et al. 2021).
...
(All numbers are copied directly from the original Table 1. \(\mathrm{TV}^\star\) is non-zero because it is the TV between a finite sample empirical distribution and the true distribution.)

Original Figure 1 also provides a visualization for \(H=10,\ L=\mathrm{Moon}\): the left panel shows the edge flow on the grid from the exact LP solution of Eq. (11) (`scipy.linprog`, Virtanen et al. 2020), and the right panel shows the direct source-target connections of the Kantorovich optimal coupling. Both have a **transport cost equal to 4.351** (Figure 1 caption). This figure is one of the most convincing pieces of evidence in the entire paper: the same numerical value, one from edge flow, the other from coupling.

**Evidence strength assessment**: For the three settings of \(H\in\{10,15,20\}\), the relative difference between the mean of \(\mathbb E|\tau|\) and \(\mathrm{OT}^\star\) ranges from \(1.2\times10^{-4}\) (Ball \(H=20\)) to \(6.4\times10^{-3}\) (Moon \(H=20\)), all less than 0.65%. \(\widehat{\mathrm{TV}}\) is close to the empirical reference value of the perfect sampler (for \(H=20\), Ball is 0.037 vs 0.040, Moon is 0.032 vs 0.040; but for Ball \(H=15\), it's 0.036 vs 0.033, which is higher than the reference—the reference value itself is a random quantity with no uncertainty reported, so it cannot be treated as a definitive threshold). Therefore, the conclusion that "the learned policy simultaneously satisfies the target marginals + achieves the optimal OT cost" is directly supported by experiments on a small scale. Note that for \(H=20\) Moon, \(\mathbb E|\tau|=9.001<\mathrm{OT}^\star=9.059\): this does not mean it beats the OT lower bound, but rather that the reading will be lower when the sampler does not precisely satisfy the marginal constraints (non-zero TV)—the original text does not discuss this point, which is my assessment.

#### 4.2 Permutations (Original Table 2)

Environment: Cayley graph of the symmetric group \(S_n\) (following T36); internal state is a permutation of length \(n\), transitions are adjacent transpositions \(s(k)\leftrightarrow s(k+1)\). \(L\) is a uniform distribution over all permutations, \(R(s)=\exp\!\big(\tfrac12\sum_{k=1}^n\mathbb I\{s(k)=k\}\big)/Z\) (T36 provides a closed-form for this normalization constant). Diagnostic metric follows T36: \(C(k)\) is the probability of "exactly \(k\) fixed points" under the reward distribution, reporting the \(L^1\) error between the model's empirical estimate and the true value (Appendix B.2, estimated using the last \(10^5\) samples of training). Reference values: for \(n=4\), \(\mathrm{OT}^\star=0.567\); for \(n=8\), \(\mathrm{OT}^\star=1.008\) (POT solver); for larger \(n\), exact \(\mathrm{OT}^\star\) is intractable.
...
(Numbers copied directly from original Table 2.)

**The trade-off of \(\lambda\) is quantitative in this table**: for \(\lambda=10^{-2}\), \(\mathbb E|\tau|=0.557\) for \(n=4\) vs. \(\mathrm{OT}^\star=0.567\), and \(1.010\) for \(n=8\) vs. \(1.008\), while \(C(k)L^1\) drops to 0.002/0.001; for \(\lambda=10^{-1}\), paths are significantly shorter (0.445, 0.645) but \(C(k)L^1\) rises to 0.012/0.011, indicating that the path shortening comes at the cost of sacrificing the termination marginal. The original text's Sec. 4.2 statement is consistent with T36: larger \(\lambda\) leads to shorter trajectories and more biased sampling, while smaller \(\lambda\) leads to accurate sampling and longer trajectories.

**Assessment of evidence strength**:

- Directly supported by experiments: The learning method can simultaneously approximate \(\mathrm{OT}^\star\) and the target marginal on small scales (hypergrid \(H\le20\), permutations \(n\le8\)); LP exact solutions and Kantorovich exact solutions have the same numerical values (Figure 1); directional trade-off of \(\lambda\).
- Inferred by authors, not directly verified by experiments: For \(n=20\), it "produces a reasonable approximation" – there is no \(\mathrm{OT}^\star\) reference value for this setting, only \(C(k)L^1=0.002\) can be observed – and this is merely a pushforward of the fixed-point count distribution. Within each fixed-point count category, severe biases can exist while this statistic remains zero, so **the correctness of the full permutation marginal is not certified** (this limitation also applies to \(n=4,8\)); path optimality has no ground truth; "scalability" is supported by only one data point, \(n=20\).
- Not provided in the original text: Wall-clock and memory comparisons with network simplex / Sinkhorn / neural OT; error at the coupling matrix level (e.g., \(\|\Pi_\theta-\Pi^\star\|_1\)); measured primal-dual gap; hyperparameter search range beyond learning rate; specific values of \(H,D\) (Table 1 only gives \(H\), the main text states the state space is \(\{0,\dots,H-1\}^D\) but **does not give \(D\)**).

### 5. Underlying Assumptions and Scope of Applicability

Stated as forward-looking scope of applicability. **Theorem 3.2 holds under the following set of conditions**:

1. **Finite directed graph**, allowing cycles, also covering DAGs that satisfy the other conditions (Assumption 3.1 and the proof of Theorem 3.2 do not require the absence of cycles; T36 after Def. 3.5 notes that for acyclic graphs, expected visit counts degenerate to visit probabilities). In general DAGs, paths between the same endpoint pair can also have different lengths (e.g., \(u\to x\) and \(u\to v\to x\) coexisting); only in hierarchical structures where path length is entirely determined by endpoint levels does the minimum flow objective degenerate to a constant.
2. **Endpoint structure**: \(s_0\) has no incoming edges, \(s_f\) has no outgoing edges; \(s_0\) only exits to \(U\), \(s_f\) only enters from \(\mathcal X\) (first two conditions of Assumption 3.1). This only specifies the adjacency of special source and sink nodes, **it does not require \(U\cap\mathcal X=\varnothing\)** – the original experiment takes \(\mathcal X=\mathcal I\), where source states are also terminal states; the right side of the divergence written as \(L(s)-R(s)\) comes from fixed injection and extraction flows. When overlapping, the same state both injects and extracts, allowing zero-step transport with \(d(u,u)=0\).
3. **All-pairs reachability**: There is a finite path between any \(u\in U\) and \(x\in\mathcal X\) (third condition of Assumption 3.1). This is a **sufficient** assumption adopted in the original text: if removed, unreachable endpoint pairs can bear zero coupling mass (e.g., two disjoint channels, each with balanced supply and demand, can still have a finite cost solution), requiring separate checks for supply-demand feasibility on reachable subgraphs.
4. **Both marginals are normalized probability distributions**: \(\sum_uL(u)=\sum_xR(x)=1\) (fourth condition of Assumption 3.1), so \(Z=1\) is known. Summing the flow matching constraints only implies that **the total mass of the two marginals is equal** (necessary condition); taking the total mass as 1 is a scaling convention, the original text explicitly states this choice is to avoid dealing with unknown normalization constants. When the common total mass is \(M\), it can be scaled to a probability problem, in which case the total state flow and internal edge flow objectives differ by \(M\) instead of 1.
5. **Shortest path cost with unit edge length**: \(d(u,x)=|\tau_{u,x}|\) counts the number of edges (Eq. (7)), i.e., the hop distance in an unweighted graph. Weighted graphs are not covered by the theorem. Since the graph is directed, \(d\) is not necessarily symmetric; it is a valid OT ground cost but not necessarily a metric in the usual sense.
6. **Absorbing / Finite expected length**: The semantics of expected visit counts for flow require the reverse chain to be an absorbing chain (T36 Lemma 3.4; O07 uses a weaker Assumption 3.1 \(\mathbb E[n_\tau]<\infty\) instead of \(P_B>0\)). The original text does not restate this, but it is a prerequisite for Eq. (3) to be well-defined.
7. **Exact flow conservation + Global optimality**: Theorem 3.2 is a statement about LP optimal solutions. Neural training uses the soft penalty of Eq. (20) (coefficient \(\lambda\)), which does not equate to exact constraint satisfaction, so the step "the trained model is OT optimal" is supported by numerical proximity at the experimental level, not guaranteed by the theorem.
8. **In Theorem 3.3, the condition "any optimal potential equals \(d(x)\) on all terminal states" requires \(R(x)>0\ \forall x\)** (third conclusion); the dual optimality of BFS distance potentials themselves only requires non-negative rewards (proof in Appendix A.3).

**Applicability in a nutshell**: Suitable for equilibrium OT on combinatorial graphs where "the state space is too large to enumerate, the cost matrix cannot be explicitly constructed, but local valid actions are clear and allow backtracking" – permutations/groups, molecular editing, program transformations, robot configuration spaces. Not suitable for small-scale explicit graphs (classic network simplex is faster, more accurate, and provides certificates), weighted/continuous costs, or unbalanced OT with unequal masses.

### 6. Position in the GFlowNet × OT Main Line

**Predecessors (Dependency Chain, All Indispensable)**:

- **T19** (Brunswic et al. 2024, AAAI): Measure-theoretic framework for non-acyclic GFlowNets, 0-flow and loss stability, and the idea of flow regularization. The hypergrid environment in this paper is directly taken from it (original text Sec. 4.1).
- **T36** (Morozov et al. 2025, ICML): The notation in Sec. 2.1 of this paper, flow definition, \(\mathbb E[n_\tau]=\frac1Z\sum_s\mathcal F(s)\) (Proposition 3.12), \(\lambda\mathcal F_\theta(s)\) regularization, permutation environment, and the \(C(k)\) diagnostic protocol are all from it. Without the expected visit count flow from T36, the objective function of this paper could not be formulated.
- **O07** (Morozov et al. 2026, this repository O07, arXiv 2603.01786): Proves that minimum total flow implies only taking shortest paths. Theorem 3.3 in this paper re-derives this conclusion using LP duality + complementary slackness (the original text explicitly states "recovers the corresponding claim") and inherits the TB objective and the trick of per-prefix computation.
- **OT Side**: Beckmann (1952)'s continuous transport model, Essid & Solomon (2018) (this repository O02)'s quadratically regularized OT on graphs—Assumption 3.1 and the min-cost-flow formulation in this paper both claim to be inspired by the latter; Villani (2008) Theorem 4.1 provides the existence of optimal couplings.

**Successors and Comparisons**:

- **C01 / ULOT** (arXiv 2506.12025, NeurIPS 2025): GNN + cross-attention amortizes the prediction of inter-graph FUGW (fused unbalanced Gromov-Wasserstein) plans, conditioned on a tradeoff hyperparameter, achieving two orders of magnitude faster speeds than classical solvers, and can also warm-start solvers. Its division of labor with this paper is clear: ULOT outputs **coupling matrices**, focuses on node alignment between two explicitly given graphs, and is natively unbalanced; this paper outputs **executable local routing policies**, focuses on an implicit, huge transition graph, and requires marginal normalization. ULOT has already occupied the path of amortization/conditioning; if this paper were to pursue "conditional GFN learning a family of \((L,R)\)", it would directly collide.
- **C02 / GSBoG** (arXiv 2602.04675): Generalized Schrödinger bridge on graphs, learning CTMC control policies, fixing both marginals, and simultaneously optimizing state-dependent running costs, using IPF + TD objectives, claiming to be the first graph GSB formulation. It is most adjacent to this paper: both require "endpoint marginals + intermediate costs + executable policies". The differences are threefold—GSBoG is **continuous-time** CTMC with KL entropy regularization to a reference process (entropic OT family), while this paper is **discrete-step** and without entropy terms (pure LP, optimal solutions at vertices of the polytope, can degenerate to deterministic routing); GSBoG's cost is a state cost functional, while this paper's cost is graph hop counts; GSBoG relies on IPF/TD, while this paper relies on TB + flow regularization. To add entropy regularization to the GFN side to interface with the Schrödinger bridge, one must directly answer "the relationship between the TB objective and path-space KL," which is not automatically true.
- **C03 / DDSBM** (arXiv 2410.01500, ICLR 2025): Discrete diffusion SB for graph transformation, also in the entropic camp.

**Contribution to the Main Line**: It provides the first geometric principle for "which internal flow to choose." Standard reward matching only fixes the terminal distribution, leaving the internal flow severely underdetermined (this underdetermination affects credit assignment and generalization to unseen states). The answer in this paper is: **among all flows that correctly terminate the distribution, choose the one with the minimum transport cost**, and this choice is equivalent to an explicit OT problem, thus allowing the use of the full suite of OT tools (dual potentials, complementary slackness, primal-dual gap) to analyze and verify GFlowNet training. Conversely, it also brings GFlowNet into the domain of computational OT: on implicit combinatorial graphs where \(C_{ux}\) cannot be explicitly constructed, GFlowNet only needs to access local neighbors.

### 7. Reusable Insights and Open Questions

#### 7.1 What's the Novelty: Not Equivalence Itself, But the Interface

"OT on graphs with shortest paths as ground cost ≡ min-cost network flow" is a classic result, and graph OT literature has long stated it this way (the original text itself acknowledges this after Eq. (8) by citing Essid & Solomon 2018, and also attributes it to Beckmann 1952). Therefore, the mathematical content of Theorem 3.2 is not new to OT researchers: Part 1 is "distributing mass along shortest paths," and Part 2 is "actual paths are no shorter than shortest paths," both steps being undergraduate-level arguments.

The novelty lies in three places, all on the GFlowNet side:

1.  **Translation**: Replacing the primal variables of min-cost flow with GFlowNet's edge flows, and constraints with detailed balance + reward/leward matching, so that this specific graph shortest path OT formulation can be approximately solved using GFlowNet's training objective (Appendix A.1 proves the algebraic equivalence of constraints, not a claim of "sampler training never before used for OT"). Proposition A.1/A.2 in Appendix A.1 serves as the technical glue for this translation—it shows that detailed balance and flow matching are the same constraint in terms of policy reconstructibility.
2.  **Output Upgrade**: Classical min-cost flow provides edge flows, and edge flows satisfying conservation and absorption conditions, when normalized by outflow, become local Markovian policies (this report's §3.1 uses precisely this reconstruction, and GSBoG also does this for classical flow baselines), so "ability to output policies" is not the difference; the difference is that GFlowNet uses neural parameterization to give \(P_F(s'\mid s)\), which can execute transport incrementally without enumerating \((u,x)\) pairs or storing an \(|U|\times|\mathcal X|\) matrix. The self-positioning in Sec. 5 of the original text is accurate: "addressing both where and how transport occurs."
3.  **Elevating Shortest Path Conclusion to OT Conclusion**: O07 is a shortest path from a single source (\(s_0\)) to multiple destinations; this paper adds \(\mathcal F(s_0\to u)=L(u)\) to make it multi-source to multi-destination, and OT is the natural super-problem of shortest paths. This step incurs almost no technical cost but changes the entire problem class.

**Easily Misinterpreted Points** (and what I consider the most important one): Theorem 3.2 states that **optimal values are equal + optimal solutions induce optimal coupling**, not that "any trained GFlowNet is doing OT." The "secretly" in the title only holds under two additional constraints: (i) the first-step edge flow is fixed to \(L\), and (ii) the total internal flow is minimized. Without (i), the first-step distribution is freely determined by \(P_F(\cdot\mid s_0)\), and the problem degenerates to O07's single-source shortest path; without (ii), any flow satisfying reward matching is valid, and mass can be infinitely accumulated in cycles (flow explosion in T19), which has nothing to do with OT. Ordinary GFlowNets do not "secretly" learn OT plans.

#### 7.2 Balance Residuals → OT Error Bounds (Our assessment: the most promising direction, lowest collision risk)

**Current Status**: Papers only provide equations at the optimal solution, completely lacking quantitative characterization for approximate solutions. During training, you have the TB residual \(\delta_\tau=\log\frac{L(s_1)\prod P_F}{R(s_{n_\tau})\prod P_B}\) (Eq. (20)); what you want are bounds for two things: source/target marginal errors, and the suboptimality of transport cost \(\mathrm{cost}(\Pi_\theta)-\mathrm{OT}^\star\).

**Why it's feasible**: This setting is more favorable than general GFlowNets. First, \(Z=1\) is known, so there are no unknown constants in the TB residual (which is precisely the most difficult term to handle in standard TB→TV bounds). Second, both marginals are hard constraints, and the residual carries information about both "starting marginal error" and "terminal marginal error". Third, the cost function is \(\mathbb E_{\mathbb P_\theta}[|\tau|]\), which is the trajectory length, a directly estimable quantity, rather than an abstract functional requiring an additional model.
...
\ =\
\underbrace{\big(\mathbb E_{\mathbb P_\theta}[|\tau|]-\textstyle\sum_{u,x}d(u,x)\Pi_\theta(u,x)\big)}_{\text{Detour term,}\ \ge 0}
\;+\;
\underbrace{\big(\textstyle\sum_{u,x}d\,\Pi_\theta-\mathrm{OT}(\widetilde L,\widetilde R)\big)}_{\text{Coupling suboptimality term,}\ \ge 0}
\;+\;
\underbrace{\big(\mathrm{OT}(\widetilde L,\widetilde R)-\mathrm{OT}(L,R)\big)}_{\text{Marginal shift term, can be positive or negative;}\ |\cdot|\le C\,D\,(\varepsilon_1+\varepsilon_2)},
\]

where \(\widetilde L,\widetilde R\) are the actual marginals at both ends of the model. **Revision Note**: The initial draft of this report omitted the middle coupling suboptimality term, leading to an immediate counterexample—two sources, two destinations, cost matrix \([[1,2],[2,1]]\), uniform marginals, and the model uses cross-pairing and transports along the shortest paths: the detour term and marginal error are both 0, model cost is 2, optimal is 1, and the original inequality becomes \(1\le0\). **Multi-source OT has two independent optimality conditions**: each trajectory is the shortest path between its endpoints (detour term is zero), **and** the endpoint coupling itself is optimal (coupling suboptimality term is zero); §3.4 also omitted the latter when equating minimum total flow to "each trajectory is shortest". The coupling suboptimality term can only be controlled by dual certificates (§7.3), not by residuals.

The first term is directly obtained by scaling Eq. (18) (non-negative, can be unbiasedly estimated by the sample mean of "actual path length − shortest path length between endpoints"); the second term requires a weak duality gap (§7.3); the third term requires the stability of OT values with respect to marginals (Lipschitz-in-marginals), which is a standard result for finite graphs + bounded costs, and \(\varepsilon_1,\varepsilon_2\) can be controlled by TB residuals (bounds of the type N03 Thm. 3.5). Therefore, an end-to-end bound for "residual → OT gap" **must** include a term given by a dual certificate and cannot be written solely from residuals. Regarding the reading of \(\mathbb E|\tau|<\mathrm{OT}^\star\) in Section 4.1: the first two terms are non-negative, only the marginal shift term can be negative, so it is one of the only possible mechanisms; however, Moon's \(H=20\) difference of 0.058 is smaller than the reported fluctuation magnitude of ±0.112, which can also be explained by finite samples or seed fluctuations, so current data cannot distinguish. The original text states: the lower reading is traded for \(\varepsilon_2\ne0\), and the two terms of the bound have opposite signs. The collision scan conclusion also supports this direction: no direct competitors were found, and ULOT/GSBoG are not on this line.

#### 7.3 Primal-Dual: Dual Potentials as Critic, Complementary Slackness as Edge-wise Certificate

Theorem 3.3 / Theorem A.5 have already provided all the necessary material, but the paper completely failed to use it in the algorithm:

- The identity of dual potentials needs to be discussed in two parts: in the single-source problem **without the first-step constraint** (Theorem 3.3 / A.5), there exists a solution where BFS distance \(d(s)\) is the most advantageous, and any most advantageous potential equals the distance at positive reward terminal points (internal potentials need not uniquely equal distance); in the extended problem **with fixed \(L\)** (Eq. (24)), the most advantageous potential is the Kantorovich graph potential, which is generally **not** the BFS distance from a specific source (counterexample: when \(U=\mathcal X=\mathcal I\), BFS distance is always 1, its dual objective is 0, while different \(L,R\) can have strictly positive OT cost). Algorithmically, one should learn a network \(\pi_\phi(s)\) that satisfies the internal edge constraint \(\pi_{s'}-\pi_s\le1\), with \(\sum_xR(x)\pi_x-\sum_uL(u)\pi_u\) as the dual objective; the constraints are edge-wise and can be penalized by sampling—this completely overlaps functionally with approaches like DeepCubeA / CayleyPy that "learn distance-to-goal", but here it has a clear LP dual identity, not a heuristic.
- **Complementary slackness provides edge-wise diagnostic quantities, not global certificates**: \(\mathcal F^\star(s\to s')(\pi^\star_{s'}-1-\pi^\star_s)=0\). Only when the primal flow satisfies **all** divergence constraints and the potentials satisfy **all** internal edge inequalities, does the primal value minus the dual value equal \(\sum_{E^\circ}\mathcal F(s\to s')(1+\pi_s-\pi_{s'})\); for flows that do not satisfy marginals, or potentials checked only on sampled edges, even if the product is small, optimality cannot be certified (sampling checks cannot prove dual feasibility on unvisited edges). During training, one can monitor \(\sum_{s\to s'}\mathcal F_\theta(s\to s')\cdot|\pi_\phi(s')-1-\pi_\phi(s)|\) as a **complementary slackness diagnostic quantity**; to report a certificate, one must simultaneously provide evidence of primal and dual feasibility and use the complete primal value minus dual value, accounting for residual terms if conservation residuals exist. (Zero TB residual only guarantees consistency, not optimality).
- Specific experiment: actor = \((P_F,P_B)\), critic = \(\pi_\phi\), loss = regularized TB + dual feasibility penalty + gap penalty. The measurable hypothesis is that "after adding the critic, \(\mathbb E|\tau|\) is lower for permutations \(n=20\) and \(C(k)L^1\) remains unchanged". Note that in the dual of the extended problem (Appendix A.4), \(a_x=\pi_x,\ b_u=-\pi_u\) are generated by the same potential function, so only one network is needed, not two.

#### 7.4 Differentiation from ULOT (arXiv 2506.12025): Do Not Compete on Amortized Prediction

ULOT's selling points are: GNN + cross-attention, conditioned on FUGW's \((\rho,\alpha)\) hyperparameters, with an \(O(n_1n_2)\) **plan prediction** complexity that directly outputs the plan. In 1000-node IBC experiments, the trained prediction is **up to** 100 times faster than classic solvers and about 10 times faster than Sinkhorn (their page 9; training costs are discussed on page 13). The plan is differentiable with respect to the input graph and can warm-start IBPP solvers. It is inherently unbalanced, inherently conditional, and inherently amortized.

Therefore, the seemingly most natural next step, "training a conditional GFlowNet to cover a family of \((L,R)\)," has a value proposition that heavily overlaps with ULOT. Moreover, GFlowNet has no structural advantage in **node alignment between two explicitly given small-to-medium graphs**—it requires rollouts, has higher variance, and needs to handle cycles.

There is only one moat for differentiation, which must be defended: **the graph is implicit, the cost matrix cannot be constructed, and transport can only be executed through valid local actions**. Permutations with \(n=20\) (\(\sim2.4\cdot10^{18}\) states, see O07 Sec. 4.1) are already in this range; molecular editing, program/proof transformation, and robot configuration spaces are also examples. The comparable experiment should be: acknowledge defeat (or a draw) on SBM graphs where ULOT can run, but demonstrate that ULOT cannot construct inputs at all on Cayley graphs where \(|U||\mathcal X|\) cannot be enumerated. Clearly defining this boundary is more convincing than merely achieving better scores.

#### 7.5 Differentiation from GSBoG (arXiv 2602.04675): Entropy Regularization Is Not Automatically Included

GSBoG solves the generalized Schrödinger bridge on graphs: fixing the marginals at both ends, it minimizes \(\mathbb E[\int f_t\,\mathrm dt]+\mathrm{KL}(p^u\|p^r)\) over the path space, controlling the jump rates of a CTMC. Training uses IPF (a path-space version of Sinkhorn) + a TD objective (because the state cost terms cancel out in IPF aggregation, IPF alone is insufficient to constrain it).

Three structural differences make direct interchangeability impossible:

1.  **Presence of entropy term**. This paper deals with unregularized LP, where optimal extreme point solutions exist (though the optimal flow and coupling may not be unique, and convex combinations on the optimal manifold are also optimal). Extreme point flows do not necessarily imply deterministic policies—when splitting flow from a single source to two positive-mass destinations, a random choice of branch is necessary; "supported on the shortest path subgraph" does not imply deterministic routing. GSBoG's KL term ensures that jump rates remain positive on edges where the reference rate is positive and the associated potential is finite, by exponentially tilting (their pages 4–5 allow controlled rates to remain zero for edges with zero reference rates, so it's not an unconditional "filling all valid paths"). To turn GFN-OT into an entropic version, one would need to explicitly add a path KL to a reference policy \(P^0\) in the objective. However, **standard TB is not automatically equivalent to path-space KL minimization**—TB is a consistency constraint, not a divergence objective; this step requires re-derivation (which is a high-collision but unresolved technical point already noted in the background analysis of this repository).
2.  **Time parameterization**. GSBoG has continuous time \(t\in[0,1]\) and time marginals; this paper only has discrete steps and no concept of "where the mass is at \(t=0.5\)". To compare intermediate behaviors, one would first need to introduce time conditioning (or step conditioning) on the GFN side, which is a modeling choice in itself.
3.  **Expressiveness of cost**. GSBoG supports state- and distribution-dependent running costs (including mean-field terms); this paper only considers the number of jumps. **The easiest extension for the GFN side is to replace \(\lambda\mathcal F_\theta(s)\) with \(\lambda c(s)\mathcal F_\theta(s)\)**, i.e., state-dependent flow penalties. In this case, the LP objective becomes \(\sum_s c(s)\mathcal F(s)\), and the ground cost becomes a weighted shortest path—the two halves of Theorem 3.2's argument hold almost verbatim when \(c\) is strictly positive (Part 1 routes mass along weighted shortest paths, Part 2 uses \(\mathrm{cost}(\tau_{u,x})\ge d_c(u,x)\) for scaling). Zero-weight edges would introduce zero-cost cycles, breaking the acyclicity of optimal flow, and must be excluded. This is a low-risk, high-certainty theorem-level increment.

#### 7.6 Unbalanced and Unknown-\(Z\): GFlowNet Abandons Its Strongest Capabilities

The hallmark of standard GFlowNet is that it only requires unnormalized \(R\). To ensure mass balance for min-cost flow, this paper requires \(\sum_uL(u)=\sum_xR(x)=1\) (Assumption 3.1) and explicitly states in Sec. 3.1 that this is to avoid dealing with unknown normalization constants. The cost is that \(L(s_1)\) replaces \(Z\) in Eq. (20), and the model no longer learns \(Z\).

The open problem is specific, but the construction given in the initial draft of this report is incorrect (adding an edge only between \(s_f\to s_0\) does not change the sum of internal node divergence constraints, \(\sum L=\sum R\), and the unequal mass problem remains infeasible when retaining fixed boundary flows, while also destroying the structure of sources having no incoming edges and sinks having no outgoing edges). The correct approach is to explicitly introduce variable transport marginals or local production/disposal variables and define penalty terms—for example, starting from a non-negative coupling, charging for the \(\ell_1\) distance of its two marginals from \(L,R\) respectively, then deriving the corresponding network form and sampling semantics. Simultaneously, "unknown normalization constant" and "allowing marginal mass to change" should be treated separately, as they are not the same problem. In this case, mass balance is absorbed by virtual edges, and \(L,R\) do not need to have equal mass. The questions to answer are: (i) how the two halves of Theorem 3.2's argument are rewritten when virtual edges exist (Part 1 needs to explicitly route "untransported mass" to virtual edges); (ii) whether TB remains stable when \(Z\) becomes unknown again in the training objective; and (iii) whether GFN's advantage over ULOT's FUGW-unbalanced approach still lies only in implicit graphs. This is a crucial, currently blank step to reconnect GFlowNet's native capabilities to OT.

#### 7.7 Does Short Flow Benefit Generalization: Unverified, and Potentially Negative

This paper uses "shortest" as the internal flow selection principle but has not measured its impact on generalization. Minimum flow solutions concentrate mass on the shortest path subgraph (complementary slackness of Theorem 3.3), and the support set can be very narrow. This has two measurable consequences, in opposite directions: positive is shorter rollouts, more direct terminal credit, and cheaper inference; negative is exploration collapse, reduced sharing of substructures, and poorer policy extrapolation to unseen states.

A feasible experiment: scan \(\lambda\) on permutation environments (using the two tiers from Table 2 and adding more points), reporting in addition to \(C(k)L^1\) and \(\mathbb E|\tau|\): coverage of visited states, entropy of terminal state support, and greedy success rate on states unseen during training (the evaluation protocol from O07 Table 1 can be directly applied). The hypothesis is that "\(\lambda\) and generalization form an inverted U-shape"—if true, "minimum flow" cannot be unconditionally considered the correct internal flow principle, but rather should be written as a regularized version.

#### 7.8 Other Open Problems (Brief)

-   **Consequences of directed asymmetry**: Theorem 3.2 still holds when \(d(u,x)\ne d(x,u)\) (the proof does not use symmetry), but the resulting "distance" is not a metric, so it cannot be directly called Wasserstein. If it is to be used for distance comparison downstream, separate justification is needed.
-   **Bridge between exact LP and neural solutions**: The paper uses `scipy.linprog` to verify small-scale equivalence (Sec. 4.1, Figure 1). For intermediate scales (e.g., \(H=30,D=3\)), a comparison of "LP solution vs. neural solution" at the edge flow level (not just scalar cost) should be performed to determine whether the neural solution finds another optimal solution with the same cost or is close to the same solution.
-   **\(D\) not reported**: Table 1 only gives \(H\), and the text states the state space is \(\{0,\dots,H-1\}^D\), but **the original text does not provide the value of \(D\)**; Figure 1's visualization is a 2D grid, suggesting that this figure corresponds to \(D=2\), but it is impossible to confirm from the original text whether the three tiers in the table use the same \(D\). This number must be determined for reproduction.
...
### Editorial note (independent judgments, distinct from the original text)

1.  **Publication Status**: The PDF footer only states "Preprint. June 5, 2026", with no venue line in the main text. According to the repository `data/papers.yaml`, it is registered as `ICML 2026 SPIGM Workshop`, so it is noted as a Workshop paper (not main conference). This distinction is made clear in both the report header and the `note` field of the bibtex entry.
2.  **Theorem Numbering**: The main theorem in the original text is numbered **Theorem 3.2** (not 3.1, which is an Assumption), and its dual is **Theorem 3.3**. Its full version is labeled **Theorem A.5** in Appendix A.3. The original text at the end of Sec. 3.2 states "Detailed proof of Theorem 3.3 can be found in Appendix A.5", while in the Appendix, this theorem is located in section **A.3** and numbered A.5. This is a minor error in the original text, and this report cites it according to the actual content. Another instance: Appendix A.4 begins with "see Appendix A.4 for derivation" as a self-reference, which is also an error.
3.  **§3.2 Emphasis on the constant 1** is my judgment (the original text only states "we will omit this constant"), because it determines the relationship between \(\mathrm{GFlow}^\star\) and \(\mathbb E[n_\tau]\), and is the most common source of error during reproduction.
4.  **§4.1 Explanation of \(\mathbb E|\tau|<\mathrm{OT}^\star\)** (\(H=20\) Moon: 9.001 vs 9.059) was not discussed in the original text. This is my inference based on the marginals not being precisely satisfied, and it has been noted in the main text.
5.  **The decomposition in §7.2 is my draft**, not a conclusion from the paper. The initial draft omitted the "coupling suboptimality term" and was refuted by external review (Codex, gpt-6-astra, 2026-09) using a counterexample of cross-pairing with two sources and two destinations. It has now been changed to a three-term decomposition; the first term comes from the scaling direction of Eq. (18) in the original text, and the third term's Lipschitz-in-marginals is standard OT stability, requiring a separate citation. The same round of review also corrected: premise 5 "cycles are necessary" (incorrect, DAGs are also covered), premise 2 "source and target sets are isolated" (incorrect, original experiment \(\mathcal X=\mathcal I\)), §7.3 misapplication of Theorem 3.3's BFS potential to problems with fixed \(L\), §7.6's unbalanced construction, §4.1's relative difference and TV summary figures, and §7.4's omitted conditions for ULOT speedup.
...
7.  **Code**: The original text and appendix do not provide a link to the code for this paper (only stating that the implementation is based on the public code of T36), so the metadata `code_url` is recorded as `null`.
8.  **Hypergrid dimension \(D\)**: The original text does not provide it. The report handles this by stating "not provided in the original text" without any imputation.

# Chapter 6: Optimal Transport Side: Prerequisites and Related Work

## 6.1 O01 · Optimal Transport for Machine Learners (Peyré's Lecture Notes) — An Introduction for GFlowNet Readers

> **In a nutshell**: Peyré's 460-page lecture notes serve as the "dictionary" for the OT side of this repository.
> For GFlowNet readers, its truly irreplaceable value lies not in the Wasserstein distance or Sinkhorn itself,
> but in **§6.5**, which expresses \(\mathcal W_1\) in the form of "edge flow on a graph + vertex conservation"—
> that is precisely the object of GFlowNet, word for word.
> The task of this introduction is to provide a "OT concept ↔ GFlowNet concept" correspondence table,
> and to **clearly indicate which correspondences are strictly equivalent and which are merely analogous**, to avoid treating "GFlowNet does OT" as an unconditional slogan.
...
| Publication | **Lecture notes / Book manuscript**, not a conference or journal paper. The author describes it as "an OT textbook organized around ML pressure" |
| Author | Gabriel Peyré (CNRS and ENS, PSL Université) |
| Code | All figures and code are in `gpeyre/ot4ml`; most computational plots are generated using the Python Optimal Transport (POT) library (explicitly acknowledged in the abstract) |
...
| Reading Priority | P0 — but **do not read cover-to-cover**. Use it according to the chapter map in Section 2; only about 40 pages of the entire book are directly relevant to the main thread of this repository |
...
> **One-sentence summary**: The first minimum-cost flow algorithm with "dual predictions." It hot-starts the classic ε-relaxation with a predicted dual solution \(\hat p\), changing the worst-case runtime from \(O(n^3\log(nC))\) to \(O(\min\{n^3\log\|\hat p-p^\star\|_\infty,\ n^3\log(nC)\})\) (Theorem 2): the more accurate the prediction, the faster it runs; if the prediction is completely wrong, it's no worse than the classic algorithm. Its significance for the main thread of this repository is: the dual potentials for the fixed double-edge problem in O08 (Appendix A.4 Eq. (24), objective \(\sum_xR(x)\pi_x-\sum_uL(u)\pi_u\); corresponding to \(p=-\pi\) plus a constant according to this paper's convention for forward edges) can serve as \(\hat p\) here. This provides the first interface with runtime guarantees for the path "GFlowNet learns potentials → feeds into classic solver for refinement." Note that O08 Thm. 3.3 itself discusses a single-source special case where the first step constraint is removed, so it cannot be directly applied.
...
| This repository's PDF | `papers/2601.20203.pdf` · Chinese translation `papers_zh/2601.20203.zh.pdf` (QA 7 issues) |
| Reading Priority | P1: The only concrete form of research topic ④ "GFN proposal + classic OT correction" that stands up |

##### 2. Core Contributions (as numbered in the original text)

**Theory.** Preprocessing \(\hat p\) (shifting to make \(\min\hat p=0\), then clipping to \([0,(n-1)C]\), Algorithm 1 lines 1–2; Lemma 1 guarantees that an optimal dual exists within this range, so clipping does not amplify error), then hot-starting ε-relaxation:
...
- **Theorem 3 / Theorem 4**: PAC sample complexity. For fixed topology and random edge costs, learning a fixed \(\hat p\) requires \(\tilde O(n/\varepsilon^2)\) samples (Theorem 3); learning a neural predictor from features to duals requires \(k=\Omega\big(\tfrac{H^2}{\varepsilon^2}(n\,d_{NN}\log(nC)+\log\tfrac1\delta)\big)\) (Theorem 4, \(d_{NN}\) is the network pseudo-dimension, Lemma 4).
...
Applicable to: integer capacities and supplies (Assumption 1) and **integer or integerizable costs** (precise optimality criterion \(\varepsilon<1/n\) and integer scaling both depend on this; original paper's Assumption 1 does not explicitly state this, but later treats costs as integers—this is a gap in the original paper's assumptions, extending to real-valued OT costs requires separate handling of precision); explicit graphs (requires enumerating nodes for ε-relaxation iterations); a family of similar instances that can be learned. The bounds are for ε-relaxation and do not transfer to network simplex.

##### 6. Position in the GFlowNet × OT Main Thread

- Interface with O08: For fixed double edges, use the extended dual from O08 Appendix A.4 (objective \(\sum_xR(x)\pi_x-\sum_uL(u)\pi_u\), internal edge constraint \(\pi_{s'}-\pi_s\le1\)); Thm. 3.3's "\(\pi^\star_x=d(x)\)" only holds for the single-source special case where the first step constraint is removed. An approximate dual feasible potential \(\hat p\) (with sign convention \(-\pi\) as per this paper) fed to ε-relaxation, Theorem 2 provides an exact solution with runtime guarantees. **One link to be verified**: There is no proven close relationship between the log of state flow learned by GFlowNet and Kantorovich potentials (state flow reflects rewards, path choices, and visit counts). Treating it as \(\hat p\) is merely a hypothesis and must be compared with constant potentials, analytical potentials, and directly trained potential predictors.
- Competing approaches: On **explicit graphs**, this paper's "learning duals + ε-relaxation" is even faster than network simplex (Table 1 average 12×) and comes with optimality certificates. Any GFN–OT experiment on explicit graphs must list it as a baseline; GFlowNet is only left with implicit graphs (where nodes cannot be enumerated, and ε-relaxation cannot run at all).
- Relationship with trend report: `TRENDS_OT_2026.md` §2.1 lists it as "borrowable + potential competitor," and this paper confirms that assessment.

##### 7. Reusable Insights and Open Problems

1. **The correct form of research topic ④**: Not a vague "GFN proposal + correction," but "GFN state flow → dual prediction → ε-relaxation," with the error-to-time mapping given by Theorem 2; experiments only need to report \(\|\hat p-p^\star\|_\infty\) and speedup.
2. **Implications for error metrics**: Runtime only depends on \(\|\hat p-p^\star\|_\infty\). If one wants to prove "balance residual → dual potential error," the target should be an infinity norm bound, not \(\ell_2\).
3. **Dual prediction on implicit graphs is useless**: ε-relaxation scans all nodes in each iteration; impossible with \(20!\) states. This paper thus conversely defines the boundary for GFN–OT: don't do it on explicit graphs, implicit graphs are the vacant niche.
...
5. Open: This paper's PAC bounds require a fixed instance distribution; how to define "a family of instances" for GFlowNet's implicit graphs remains an open question (same obstacle as research topic ②).

### 8.4.2 N02 · Stop the Sampler! (ICML 2026 SPIGM Workshop)

> **One sentence summary**: This paper embeds MCMC into a non-acyclic GFlowNet framework for continuous state spaces: at each step, a learned classifier \(d_F(s)\) decides "whether to stop", and detailed balance ties the optimal classifier to the target density (Theorem 3.6). It proves that the total flow is minimized if and only if the expected trajectory length reaches a lower bound defined by the Markov chain (Corollary 3.7). This work, from the same HSE team as O07/O08, represents the third interpretation of the identity "expected trajectory length = total flow"—the first two being shortest paths (O07) and optimal transport (O08), and this one being adaptive termination for MCMC.

| Field | Content |
|---|---|
| arXiv | [2606.16073](https://arxiv.org/abs/2606.16073) (v2, 2026-07-17; PDF in this repo is v2, 23 pages) |
...
| Code | [github.com/kkorolev1/stop-the-sampler](https://github.com/kkorolev1/stop-the-sampler) (given on page 2 of the original paper; not reproduced in this repo) |
| PDF in this repo | `papers/2606.16073.pdf` · Chinese translation `papers_zh/2606.16073.zh.pdf` (QA 2 issues) |
| Reading Priority | P1: Application of non-acyclic GFlowNet theory in continuous spaces, shares Prop. 3.5 identity with O08 |

##### 2. Core Contributions (numbered as in the original paper)
...
- **Proposition 3.5 (Eq. (14))**: \(\mathbb E_{\tau\sim P}[n_\tau]=F(\mathcal S)/F(\{s_0\})\). The original paper explicitly states that this tightens the inequality (12) from Brunswic et al. (T19) into an equality—similar to what T36 did in the discrete case, but here it's the continuous version.
- **Theorem 3.6**: Given a fixed transition kernel \(Q_F\) (density \(q_F\), full support, uniformly geometrically ergodic, stationary distribution \(\pi_Q\)), then \((f,q_F,d_F,q_B,d_B)\) satisfies detailed balance (8) **if and only if**: (1) The flow measure \(F(A)=Z\,(U(A)+n_Q\pi_Q(A))\) (Eq. (18)), where \(U(A)=\sum_{n\ge0}(p_0Q_F^n-\pi Q_F^{n+1})(A)\) (Eq. (19); the second term uses the **target distribution** \(\pi\), not the kernel's stationary distribution \(\pi_Q\)) is the "cumulative difference between injected and removed mass", and \(n_Q\ge n_Q^\star:=\sup_s\frac{\max\{\pi(s),p_0(s)\}-u(s)}{\pi_Q(s)}\); (2) The classifiers and backward kernel are given by Eq. (20): \(d_F(s)=r(s)/f(s)\), \(d_B(s)=Zp_0(s)/f(s)\), \(q_B(s\mid s')=\frac{f(s)-r(s)}{f(s')-Zp_0(s')}q_F(s'\mid s)\).
...
- **Proposition 3.8** + Multi-level scheme: The state is augmented to \((s,\ell)\), \(\ell\in\{1,\dots,L\}\), with intermediate rewards \(r(s,\ell)\) annealed by \(\beta_\ell\) (Eq. (23)), and initial and target distributions for adjacent layers given by Eq. (24); **trajectory balance is satisfied layer by layer** (not just reward matching), then the termination distribution is \(\pi\).
- Training objective: Prefix TB (prefix trajectory balance from Morozov et al. 2026, Eq. (21)) + flow regularization (directly calculated from the classifier via Eq. (16)) + weighted by stop probability (stopgrad). The paper reports that DB performs poorly in continuous environments and SubTB terms grow quadratically, hence prefix TB is chosen.

##### 5. Assumptions and Applicability Limits

Fixed kernel \(Q_F\) that is uniformly geometrically ergodic (precondition for Theorem 3.6); \(\mathbb E[n_\tau]<\infty\) (Assumption 3.1); for continuous state spaces, \(\pi,p_0\) must be absolutely continuous with respect to Lebesgue measure; the multi-level scheme requires definable annealed intermediate rewards. When the learned \(q_F\) is a modification of a ULA kernel, the theory only covers the fixed kernel case.

##### 6. Position in the GFlowNet × OT Main Line

- With O07/O08, it represents **three uses of the same quantity**: all three papers start from \(\mathbb E[n_\tau]\propto\) total flow. O07 minimizes it to get shortest paths; O08 fixes the source distribution and minimizes it to get OT; this paper treats it as "when to stop" for MCMC and provides a closed form for \(n_Q^\star\).
- Implications for the main line: The "particular solution + \(n_Q\pi_Q\)" decomposition in Theorem 3.6 is the continuous space equivalent of "acyclic flow + cyclic space"; if O08 were to be extended to continuous state spaces, the structure of the flow space is already provided here.
- Complements Berner et al. (TRENDS_GFN §2.3, discrete↔continuous equivalence): that paper deals with limit equivalence, while this paper directly constructs non-acyclic flows in continuous space.

##### 7. Reusable Insights and Open Problems

1. **Stop probability = reciprocal of state flow** (Eq. (16)): A transferable parameterization trick; O08-like models can use this to avoid a state flow network.
2. **Minimum flow has a closed-form optimal value** \(n_Q^\star\): In O08's discrete setting, the minimum flow is the solution to an LP; here, because the kernel is fixed, the minimum can be written explicitly, serving as an analytical benchmark for sanity checks.
3. Prefix TB + flow regularization + stop probability weighting: A directly reusable recipe for non-acyclic continuous training; O08 should start here if conducting continuous experiments.
4. Open: If the fixed kernel in Theorem 3.6 is replaced with an O08-style "learn a kernel that minimizes total flow", would this yield an OT formulation in continuous space? The original paper does not discuss this.
5. Open: The definition of \(n_Q^\star\) includes \(\sup_s\); how to estimate this in high dimensions? The original paper uses learning as a substitute but does not provide estimation error.

### 8.4.3 N03 · Stable GFlowNets with TV Monitoring and Probabilistic Guarantees (Preprint)

> **One Sentence Summary**: First, it is proven that "a small TV distance between the learned distribution and the target" does not preclude "unbounded training loss" (Prop. 3.3–3.4: TV is controlled by the aggregate contrast \(1-\Lambda_{\mathcal X}\), while the upper bound of the loss is controlled by the worst local contrast \((\log\min\Lambda_{\{x\}})^2\); these two can differ arbitrarily). Then, it provides a **certificate from loss to TV**: per-trajectory \(\mathcal L_{TB}(\tau)\le c^2\) ⇒ \(\mathrm{TV}\le1-e^{-2c}\) (Theorem 3.5), and replaces this unverifiable "per-trajectory" condition with a probabilistic certificate based on "sampling \(m+n\) trajectories and taking the maximum loss" (Theorem 3.6). Finally, it stabilizes training using an adaptive reference flow \(\delta(\tau)\), at the cost of a computable fidelity degradation factor \((1+\Delta/Z^\star)\) (Theorem 3.10–3.11). **This is the closest GFlowNet paper to this repository's preferred topic of "balance residual → OT error bound"**: it completes half of "residual → marginal error".
...
| Reading Priority | **P0** (for Topic ①): TB→TV bound and probabilistic certificate are directly transferable |
...
- **Theorem 3.6 (Probabilistic Certificate)**: Sample \(m\) trajectories from \(\hat\pi(\tau)=\pi_{\text{target}}(x_\tau)P_B(\tau\mid x_\tau)\) and \(n\) trajectories from \(P_F\). Let \(c=\max_i\sqrt{\mathcal L_{TB}(\tau_i)}\) (the **square root** of the maximum loss; \(c\) controls the absolute value of the log-ratio). Then, with confidence \(1-2\alpha\): \(\mathrm{TV}\le e^{2c}+1-\alpha^{1/m}-\alpha^{1/n}\le e^{2c}-1+\frac{\log(1/\alpha)}{m}+\frac{\log(1/\alpha)}{n}\) (Eq. (9)). **Independent of state space size**.
...
- **Theorem 3.11**: Probabilistic bound that holds simultaneously for all \(c\in\mathcal C\) (Eq. (16)), allowing **optimization of threshold \(c\)** during training.
- **Algorithm 1 (Stable GFlowNets)**: Adaptively injects \(\delta(\tau)\) according to Theorem 3.11, with a top-\(K\) high-reward buffer.

##### 3. Method and Theoretical Derivation Highlights

Proof idea for Theorem 3.5 (Appendix B): \(\mathcal L_{TB}(\tau)\le c^2\) is equivalent to \(e^{-c}\le ZP_F(\tau)/R(\tau)\le e^{c}\), meaning the per-trajectory ratio is bounded within \([e^{-c},e^c]\); summing over all trajectories yields \(Z/Z^\star\in[e^{-c},e^c]\), and multiplying these two ratios gives the termination probability ratio \(P_T(x)/\pi(x)\in[e^{-2c},e^{2c}]\); TV is \(\frac12\sum|P_T-\pi|\), which is bounded by \(1-e^{-2c}\) from the ratio bound. The DB/FM version accumulates per-edge ratios along a trajectory of length \(L\), making the exponent \(2Lc\).

Theorem 3.6 replaces "all trajectories" with "maximum of samples": \(m\) trajectories sampled from \(\hat\pi\) cover the target mass, and \(n\) trajectories sampled from \(P_F\) cover the model mass. The unsampled portions are controlled by tail probabilities of the form \(\alpha^{1/m}\) and \(\alpha^{1/n}\) – this is the source of "independence from state space size."

**Mechanism of Algorithm 1 (Stable GFlowNets).** Inputs: TV target \(d\), confidence \(1-2\alpha\), loss threshold \(c\), patience \(N\). Each round: sample from \(P_F\), then sample endpoints from the top-\(K\) buffer \(\mathcal X_{\text{sub}}\) according to \(R\) for backward sampling (line 3); update the buffer with states from new trajectories (line 4); only compute certificate \(B_{TV}\) when the buffer remains unchanged for \(N\) consecutive rounds (lines 5–8, subgraph version of Theorem 3.11); if the first term of the certificate is already below \(d\), **skip training and only accumulate samples** (lines 9–10), otherwise compute the minimum reference flow according to Eq. (13) and update \(P_F,P_B,\log Z\) using \(\mathcal L_{\text{aug}}\) (lines 12–13); report \(M_{TV}\) (Eq. (15)) at the end. The threshold \(c\) is updated using exponential moving average, and \(B_{TV},M_{TV}\) are found using bounded one-dimensional optimization (Appendix D). Two design decisions are noteworthy: the certificate is only computed after the buffer stabilizes (to avoid reporting false certificates during distribution drift); training can be **halted** by the certificate (the certificate is not just a post-hoc diagnostic but also a training controller).

**Our assessment:** This proof only uses two things: (i) the residual is the square of the \(\log\) ratio, and (ii) TV is a linear functional of the deviation of the ratio from 1. The OT cost gap \(\sum_e\mathcal F(e)-\mathrm{OT}^\star\) in O08 is also a linear functional of flows, and the marginal violation \(\|\hat L-L\|_1\) is similar. To replace the right-hand side of Theorem 3.5 with these quantities, what's missing is the step "per-trajectory ratio bound \(\Rightarrow\) per-edge flow ratio bound" (in O08's LP, flow = marginalization of trajectory distribution, which is linear), and that under zero-temperature LP, the optimal solution is at a polyhedron vertex, so the ratio bound does not directly give the cost gap (requiring dual potentials as a bridge).

##### 5. Assumptions and Applicability Boundaries

DAG (the DB/FM version of Theorem 3.5 uses maximum length \(L\), which fails with cycles – this is crucial for O08's non-acyclic setting: only the TB version \(1-e^{-2c}\) is length-independent and transferable); ability to sample from \(\pi_{\text{target}}\) (Theorem 3.6's \(m\) trajectories require target samples, approximated by a top-\(K\) buffer in practice); reference flow \(\delta>0\) requires target samples in the buffer.

##### 6. Position in the GFlowNet × OT Main Thread

- **Not the same paper** as Stable-GFN (Kwon et al., ICML 2026 Spotlight, pairwise comparison to eliminate \(Z\)); similar names but different problems: this paper is about certificates, that one is about red-teaming training stability.
...
2.  **Must be modified**: O08 uses non-acyclic graphs, so the \(L\)-dependent DB version fails; O08's training objective Eq. (20) is regularized TB with a source distribution term, which formally connects to Theorem 3.5's trajectory-level branch (transfer still requires prerequisites: bidirectional full trajectory law normalization, source and termination edges included in residuals, and in O08, states can continue to transition, so termination probability corresponds to the mass of the final termination edge); if implemented using T36's public code with DB + state flow regularization, Theorem 3.5's DB branch degenerates to a **trivial bound** of 1 when \(L\to\infty\) (not infinity). Bounds using expected length instead of maximum length require **re-proof** (clarifying which trajectory distribution's expectation, normalization conditions, and tail control are used); T36 Prop. 3.12 only provides an identity between expected length and total flow, which is the starting point for this path, not the proof.
3.  **Reference flow as a stabilization mechanism for "common additive reference trajectory measure"**: \(\delta(\tau)\) is added to both the model trajectory measure and the target trajectory measure. Its relationship with OT regularization is **not yet established** – the projections of the same trajectory measure onto the start and end points are generally different, so it is not equivalent to adding the same mass to both marginals; to claim correspondence to some OT regularization, the augmented primal problem, both marginals, and the dual must be written out.
4.  Open: Theorem 3.6 requires sampling from the target, but in O08's setting, the target coupling \(\Pi^\star\) is unknown – however, target **marginals** \(L,R\) are known. Can certificates be given using only marginal samples?
5.  Open: Does the loss concentration ratio (Figure 3) also appear in O08's permutation environment? If so, the role of flow regularization \(\lambda\) might primarily be to suppress this ratio.

### 8.4.4 N04 · Generative Modeling on Metric Graphs via Neural OT (Preprint)

> **One Sentence Summary**: The first work on deep generative modeling on **metric graphs** (distributions continuously supported on edges, e.g., pickup locations on a road network): it embeds the graph into a smooth ambient space (Euclidean realization, or tropical Abel–Jacobi embedding into a Jacobian torus), solves the neural semi-dual form of the entropy-regularized Kantorovich problem in the embedded space, and then projects the generated samples back to the graph via nearest projection; it proves that as neural expressivity increases, the generator weakly converges to a legitimate transport coupling on the graph (Theorem 4.1). It overlaps with O08 on the word "graph" but not on objects and outputs: here, distributions are continuous measures on edges, the training cost is the quadratic cost \(c_\Psi=\tfrac12d_\Psi^2\) in the embedded space (graph geodesic distance is only used for evaluation), and the output is a sampler rather than an edge-wise policy.
...
- Three-step method (§3): Embedding → Solving entropy OT semi-dual with neural potentials in the embedded space → Sampling from Gibbs conditional law and **nearest projection** back to \(\Gamma\) (projection–pullback generator).
- **Theorem 4.1 (Graph-supported recovery)**: If the neural function family \(\mathcal F\) is dense in \(C(\mathbb R^n,\mathbb R)\) under the ucc topology, and the neural feature map \(\varphi_\Psi\) on the target support (which is different from the graph-to-ambient-space map \(\Psi\)) is continuous and injective, then there exists a sequence of neural potentials \(g_m\) whose induced embedded Gibbs law satisfies \(\|\pi^\varepsilon_{m,\Psi}-\pi^\star_{\varepsilon,\Psi}\|_{TV}\to0\); and there exists a joint subsequence \(m_k\to\infty,t_k\downarrow0\) (heat kernel smoothing parameter) such that the projection–pullback generator weakly converges to a legitimate transport coupling on the original graph. Graphs with bridges must first undergo parallel virtual edge augmentation as in Appendix B for the injectivity of the tropical map to hold. What is recovered is the pullback of the **embedded space entropy OT** optimal coupling, not the coupling optimal for the graph geodesic cost.
- Experiments (§5): Synthetic metric graphs (theta, wheel, grid, road), comparing two "source-aware pushforward" heuristic baselines – node interpolation (vertex-level discrete graph OT then interpolation along edges) and ambient pushforward; metrics are graph \(W_1^\Gamma,W_2^\Gamma\), density \(L_1\), edge CDF \(L_1\). Real data: \(10^6\) Uber pickup locations on the Manhattan road network.

##### 5. Assumptions and Applicability Boundaries

Metric graphs are finite, edge lengths are known, and can be explicitly embedded; \(\varepsilon>0\) (entropy regularization, zero temperature is not within the framework); requires **samples** of source and target (not unnormalized densities); Theorem 4.1 is an existence + limit statement, without rates.

##### 6. Position in the GFlowNet × OT Main Line

- **Not a competitor, but a complementary branch**. O08: probability mass on discrete vertices, unit hop cost, zero-temperature LP, output edge-wise policy, implicit graph. N04: continuous measures on edges, quadratic cost training in embedded space (graph geodesic distance only for evaluation), entropy-regularized, output sampler, explicit and embeddable graphs. The only common part is "target distribution for OT on graphs".
- Useful reference for O08: N04's evaluation protocol (graph \(W_1/W_2\) + density \(L_1\) + **evaluation noise floor**) is more complete than O08's TV + \(\mathbb E|\tau|\), especially the "noise floor" row which is missing in O08's experiments.
- Noted as "adjacent competition" in `TRENDS_OT_2026.md` §2.1; after reading the full paper, this report downgrades it to "complementary".

##### 7. Reusable Insights and Open Problems

1. **Evaluation noise reference**: Compare two independent test subsamples against each other under the same evaluation budget to provide an empirical error reference due to finite test samples (and report uncertainty from repeated sampling) – O08-like experiments should add this row.
2. **Intrinsic vs. extrinsic embedding** contrast design is transferable: O08's permutation graph Cayley structure also has "intrinsic" (group distance) and "extrinsic" (some feature embedding) representations.
3. Open: Connecting N04 and O08 requires first unifying the cost – N04 trains with an embedded space quadratic cost, O08 with graph shortest path cost; simply letting the entropy coefficient tend to zero will not achieve this conversion. The \(W_1\) for shortest path cost on metric graphs has a Beckmann form (O01 §6.5), which N04 does not utilize.
4. Open: N04 requires sample access; O08's theorem setting requires normalized marginal values at both ends (Assumption 3.1), while the unnormalized reward access in general GFlowNets has not yet been derived along this line. Intermediate forms between these three input access modes have not been found within the scope of this repository's review.

### 8.4.5 N05 · Orlicz-Sobolev Transport for Unbalanced Measures on a Graph (NeurIPS 2025 Spotlight)

> **One sentence summary**: Addresses transport between two measures with **unequal total mass** on a graph metric space. It first reformulates entropic partial transport (EPT) into a standard (balanced) OT with a non-negative ground cost \(\hat c\) using Caffarelli–McCann's idea (Prop. 3.1, adding a dummy point \(\hat s\) to absorb excess mass), then endows it with Orlicz geometry to obtain Orlicz-EPT (Eq. (8)); finding it still involves a two-level optimization with hypercubic complexity, it then constructs a regularized version, **Orlicz-Sobolev Transport (OST, Def. 4.1)**, by leveraging dual EPT and graph structure. It proves that OST can be reduced to **a univariate optimization problem** (Theorem 4.2), with an explicit expression in the discrete case (Cor. 4.3), and is empirically 250–13800 times faster than Orlicz-EPT. Significance for the main line: O08 requires \(\sum L=\sum R=1\); if relaxed to unbalanced, this paper provides a candidate objective that does not rely on KL penalties, has closed-form computation, and is naturally defined on graphs.
...
| Authors | Tam Le, Truyen Nguyen (co-first authors), Hideitsu Hino, Kenji Fukumizu – The Institute of Statistical Mathematics (ISM) / University of Akron |
| Code | [github.com/lttam/OST_OrliczEPT](https://github.com/lttam/OST_OrliczEPT) (Footnote 2 on page 2 of the original paper) |
| PDF in this repo | `papers/2502.00739.pdf` · Chinese translation `papers_zh/2502.00739.zh.pdf` (QA 11 issues) |
| Reading Priority | P1: Non-KL approach for unbalanced OT on graphs |

##### 2. Core Contributions (numbered as in the original paper)

- **Prop. 3.1 (EPT ⇔ Standard OT)**: \(ET_\lambda(\mu,\nu)=(\mu(G)+\nu(G))\big(W_{\hat c}(\hat\mu,\hat\nu)-b\lambda\big)\) (Eq. (7), affine relation rather than direct equality), where \(\hat\mu=(\mu+\nu(G)\delta_{\hat s})/(\mu(G)+\nu(G))\) and \(\hat\nu\) is symmetrically defined—first adding a dummy point \(\hat s\) with cross total mass, then normalizing by the common total mass; cost \(\hat c\) is given in Eq. (5); **Remark 3.2**: Unlike existing methods, \(\hat c\ge0\) is guaranteed, which is a crucial calibration for subsequently applying the Orlicz-Wasserstein framework.
- **Orlicz-EPT (Eq. (8))**: \(OE_\Phi(\mu,\nu)=(\mu(G)+\nu(G))(W_\Phi(\hat\mu,\hat\nu)-b\lambda)\); **Prop. 3.3** monotonicity; **Prop. 3.4/3.5** monotonicity and bounds for the entropy-regularized version \(A_\varepsilon\), solved by bisection (Eq. (11), a quadratic complexity inner loop replacing the hypercubic (9)), but the two-level structure remains heavy.
- **Def. 4.1 (OST)**: \(OS_{\Phi,\alpha}(\mu,\nu)=\sup_{f\in U_{\Psi,\alpha}}\big(\int f\,d\mu-\int f\,d\nu\big)\) (Eq. (14)), where \(U_{\Psi,\alpha}\) is the set of functions in the graph Orlicz–Sobolev space \(WL^\Psi(G,\omega)\) whose derivative Orlicz norm is \(\le b\) and root value falls within the interval \(I_\alpha\)—OST is an integral probability metric (IPM).
...
- **Cor. 4.3 (Discrete Case)**: When measures are supported on vertices, the integral becomes a sum over edges (Eq. (17)), combined with Dijkstra preprocessing \(O(|E|+|V|\log|V|)\) and sparsity from "summing only over edges in \(E_{\mu,\nu}\)".
- **Prop. 5.5/5.6**: For the limiting \(N\)-function \(\Phi_0\), OST has a closed form, and Orlicz-EPT degenerates to EPT on graphs.

##### 5. Assumptions and Applicability Limits

The graph is a physical graph with unique paths \([z_0,x]\) (tree-like; for non-physical graphs, see Remark 4.4 for handling); measures are non-negative and finite; \(\Phi\) is an \(N\)-function; OST is an IPM, not EPT itself—it is a regularized substitute for EPT, not an equivalent.

##### 6. Position in the GFlowNet × OT Main Line

- O08 Assumption 3.1, item 4, requires both marginals to be normalized. Relaxing this has two paths: KL penalty (ULOT/C01, GSBoG series of methods) or this paper's "dummy point + linear mass difference term". The advantages of the latter: mass difference is explicitly absorbed by a linear term, computation is a univariate optimization; disadvantages: univariate optimization is not linear programming (general \(\Phi\) yields a nonlinear Orlicz norm), and OST computes a scalar difference between two measures, not automatically producing non-negative edge flows, samplable policies, or generation mechanisms for unbalanced masses; additionally, it requires a root \(z_0\) and a root shortest path system (Appendix B.2 provides conventions for choosing paths in graphs with cycles, perturbed weights, and multiple shortest paths).
- Similar to O02 (Essid & Solomon quadratic regularized OT on graphs) in being "regularized OT on graphs", but the regularization object differs: O02 regularizes edge flows, this paper regularizes the derivative of dual potentials.
- `TRENDS_OT_2026.md` §2.1 notes it as "complementary", which this report maintains.
...
2.  **Beckmann's net flow on trees is explicit**: If O08 performs a sanity check under the conditions of a tree-like state graph, vertex support, \(\Phi(t)=t\), and \(b=1\), then OST and graph OT are precisely equal (Prop. A.2), which can serve as a reference value; "approximately tree-like" cannot directly yield an exact reference.
3.  Open problem: The state graph in O08 is cyclic and directed. A cycle does not mean that the shortest path to the root is not unique (a unique shortest path system to the root can be chosen), but even after selection, the OST value is not equivalent to the original Beckmann optimal value on the cyclic graph; Remark 4.4 only relaxes the requirements for physical Euclidean realization and does not address directedness. The correspondence on general directed cyclic graphs remains to be established.
4.  Open problem: Comparing the dual potential \(f\) of OST (Orlicz–Sobolev class) with the dual potential \(\pi\) of O08's fixed bi-marginal problem (Appendix A.4, one-sided constraint \(\pi_{s'}-\pi_s\le1\) on directed internal edges; Thm. 3.3 only for the single-source special case without the initial step constraint)—these are versions of the same dual variable under different function classes. No comparison has been found within the scope of this repository's review.

### 8.4.6 N06 · A Benchmark for Discrete Schrödinger Bridges and EOT (ICLR 2026)

> **One sentence summary**: This is the first **benchmark with an analytical solution** for entropy-regularized OT / Schrödinger Bridges (SB) in discrete spaces. The core is Theorem 3.1: Given an initial distribution \(p_0\) and a scalar function \(v^\star\) that makes \(q^\star\) a valid probability distribution (sufficient conditions: \(v^\star\ge0\), non-zero, and \(0<c^\star(x_0)<\infty\)), let \(q^\star(x_1\mid x_0)\propto v^\star(x_1)q^{\text{ref}}(x_1\mid x_0)\). Then the EOT/SB solution between \((p_0,\ p_1:=q^\star\text{'s second marginal})\) with \(q^{\text{ref}}\) as reference is \(q^\star\). Parameterizing it with CP decomposition makes it computable in high-dimensional spaces \(|\mathcal X|=S^D\) (Prop. 3.1/3.2). Byproducts include three solvers: DLightSB, DLightSB-M, and \(\alpha\)-CSBM, with DLightSB generally being the strongest (with exceptions, see Section 4). Significance for the main line: Any "entropy-regularized GFN–OT" claiming to solve SB must report on this benchmark, which also serves as evidence for the closing of research topic ③—discrete SB has moved from the "method" stage to the "benchmark" stage.

| Field | Content |
|---|---|
| arXiv | [2509.23348](https://arxiv.org/abs/2509.23348) (v2; 24 pages in this repository's PDF) |
| Publication | **ICLR 2026 Main Conference** (Based on: PDF header "Published as a conference paper at ICLR 2026"; arXiv comment is empty, this repository's CSV has been corrected accordingly) |
...
| Code | [github.com/gregkseno/catsbench](https://github.com/gregkseno/catsbench) (given at the end of the abstract; CSBM solver at github.com/gregkseno/csbm) |
...
-   **M3.1 / Theorem 3.1 (Benchmark Pair Construction)**: Any \((p_0,v^\star)\) that makes \(q^\star\) valid induces a benchmark pair \((p_0,p_1)\), whose EOT/SB solution \(q^\star(x_1\mid x_0)=\frac{1}{c^\star(x_0)}v^\star(x_1)q^{\text{ref}}(x_1\mid x_0)\) (Eq. (7)) is known in closed form. The original text states this is a discrete version of Gushchin et al. (2023b)'s continuous space construction.
-   **M3.2 / Prop. 3.1, 3.2 (Computable Parameterization)**: In high dimensions, the summation for \(c^\star(x_0)\) and \(q^\star\) has \(S^D\) terms; by taking \(v^\star\) in CP decomposition form (one non-negative vector \(r_{kd}\in\mathbb R_+^S\) for each mixture component \(k\) and dimension \(d\)), both normalization and conditional distributions decompose into products of one-dimensional sums, allowing for exact computation (Prop. 3.1 for conditional distributions, Prop. 3.2 for SB transition distributions).
...
-   **Evaluation Metrics**: Conditional Shape Score and Trend Score (Table 1a/b), Trajectory KL and Reverse KL (Table 3/4), C2ST (Table 2, authors state its values are "uninformative"—all methods are close).

##### 5. Underlying Assumptions and Applicable Boundaries

Discrete time, factorized (dimension-wise independent) reference process; \(v^\star\) in CP form; \(\varepsilon>0\) (entropy regularization is the definition of SB itself). Zero-temperature LP (O08's setting) is not within the benchmark's scope, but can be compared as the \(\varepsilon\to0\) limit.

##### 6. Position in the GFlowNet × OT Main Line

-   Regarding research topic ③ (Entropy-regularized GFN–OT / SB on graphs): **Essential benchmark**. After GSBoG (C02) and DDSBM (C03), N06 sets the standard; any new discrete SB solver that does not report Shape/Trend Scores and trajectory KL on this benchmark cannot claim to solve SB.
-   Regarding the strength of evidence for the closing of research topic ③: **Moderately strong, with boundaries**. With an analytical benchmark on product-structured discrete spaces + multiple solvers + failure mode analysis, new entrants in this setting do not have the "define the problem first" advantage; however, the benchmark does not cover general graphs, non-factorized dynamics, and other input access patterns. Whether "it solves SB" is determined by the target, reference process, and marginal constraints, not by whether a specific benchmark is used.
-   Regarding indirect value for research topic ①: N06's \(\alpha\)-CSBM transplants O04's \(\alpha\) online update to discrete space; the original text (page 7) explicitly calls discrete \(\alpha\)-IMF a heuristic analogy, **without proof** that it is isomorphic to GFlowNet's fixed backward policy or edge flow normalization. The correspondence reported in O04 §6 remains a conjecture in discrete space.

##### 7. Reusable Insights and Open Problems

1.  **Recipe for constructing benchmarks with analytical solutions**: Fix reference dynamics + choose a potential that makes the coupling valid + define marginals. Transplanting this to O08's zero-temperature setting is **conditionally feasible but not "any potential will do"**: complementary slackness only provides the set of saturated edges that can carry optimal flow; it does not automatically generate flows that satisfy non-negativity, mass conservation, and specified terminal supports—a constant potential is a counterexample (under unit cost, no edges are saturated, only zero flow is possible, so \(R=L\); infeasible if source and terminal supports are disjoint). The correct approach is to jointly construct a feasible potential, tight edge paths, and non-negative transport flow, prove that every positive source mass can reach an allowed destination, then define \(R\) and check for non-degeneracy. This is what O08's experiments lack, but the recipe requires proof from this paper itself (see INSIGHTS §6c Day 3).
2.  Lessons from C2ST need caution: Table 2 shows ROC AUC for classifying endpoint pairs \((x_0,x_1)\) on the reverse benchmark in Appendix D.1. The authors discarded it due to similar numerical values; however, by common definition, 0.90–1.00 indicates that generation and reality are **distinguishable**, which is opposite to the authors' interpretation. The metric definition and direction need verification. One cannot conclude from this that high-dimensional binary classification tests universally fail; it only suggests that it does not differentiate solver quality on that benchmark.
3.  Open: Compare N06's benchmark as \(\varepsilon\to0\) with O08's LP solution to verify whether the plan selected by the entropy selection principle on discrete graphs equals the minimum total flow plan.
4.  Open: Theorem 3.1's endpoint reweighting construction holds for general discrete reference chains (Eq. (11)'s Doob transform does not require product structure), but this paper only provides scalable analytical normalization and sampling parameterization for dimension-wise factorized reference chains (Prop. 3.1/3.2); **efficient** construction for non-product reference chains on graphs is still missing.

### 8.4.7 Joint Review of Six Papers: Impact on Rating Four Candidate Topics

| Topic | Changes brought by the six papers |
|---|---|
| ① Residuals → OT error bounds + dual potential certificates | From "accessories elsewhere" to "accessories in hand": N03 Thm. 3.5/3.6 are ready-made certificates for residuals → marginal errors, N01 Thm. 2 bounds dual error → runtime, N06 Thm. 3.1 is a recipe for constructing analytical benchmarks. Rating remains **do**, and readiness for commencement increases |
| ② Conditional GFN amortized OT on graphs | No new evidence to change "do not do" |
| ③ Entropy-regularized GFN–OT / SB on graphs | N06, as an ICLR 2026 main conference paper, sets the evaluation standard for discrete SB, strengthening evidence that the window is closing; N02 indicates that the continuous non-acyclic GFN team itself is moving towards samplers, not SB |
| ④ GFN proposal + classical OT correction | N01 concretizes it as "dual prediction → ε-relaxation," but this only holds for explicit graphs; N04 shows there are other neural OT routes for explicit continuous graphs. Rating remains "demoted to an application of ①" |

# Chapter 9: Insights, Candidate Topic Ratings, and Decisive Experiments

This chapter includes the entire content of the cross-paper synthesis document `reports/INSIGHTS.md`: a one-page conclusion, main logical chain, OT comparison table, competitive landscape, ratings for four topics, immediately actionable decisive experiments, eight easily misinterpreted points, and open problems.

### 1. One-Page Conclusion

1.  **On general graphs, terminal rewards are insufficient to uniquely determine internal flow; rewards only fix the boundaries.** Terminal flow is determined by \(R\), while \(P_B\) on non-terminal edges is free (T02 Prop. 18 item 3; §2.6); on cyclic graphs, an additional degree of freedom is added by the cycle space \(H^1_+(G)\) (T19 Prop. 5). This is the starting point of the entire main line.
2.  **Minimum total flow is a legitimate selection principle and has precise behavioral implications.** On finite discrete non-acyclic graphs, \(\sum_{s}F(s)=Z\cdot\mathbb E[n_\tau]\) (T36 Prop. 3.12 / Eq. (10), summation over internal states, tightening T19 Thm. 2's "≤" to "="), so "minimum total flow" = "shortest expected trajectory".
...
4.  **Given a fixed source distribution, minimum total flow = Kantorovich OT under graph shortest path costs.** \(\mathrm{GFlow}^\star=\mathrm{OT}^\star\), and the endpoint distribution of trajectories sampled by the optimal policy is the optimal coupling (O08 Thm. 3.2). O08 Thm. 3.3 discusses the **single-source primal problem with the first-step distribution constraint removed**: the dual potential at terminal states equals the shortest path distance from \(s_0\), and complementary slackness restricts the optimal flow to the shortest path subgraph; for a fixed multi-source distribution \(L\), the extended dual in Appendix A.4 applies (objective includes source potential term), and terminal potentials are no longer generally equal to single-source distances.
5.  **Mathematical equivalences are classic results; the novelty lies in GFlowNet's policy interface.** Graph shortest-path OT ≡ min-cost flow ≡ discrete Beckmann (O02 Eq. (1)⇔(3); O01 Prop. 6.23, Prop. 14.9). O08's contribution is translating this into GFlowNet language, where the output is not a coupling matrix but a local routing policy executable on implicit graphs of size \(20!\) (O08 §4.2).
6.  **In the standard single-source DAG setting, "GFlowNet learns an OT plan" is a vacuous statement.** When the source marginal is \(\delta_{s_0}\), the coupling set is a singleton (O01 Remark 3.2). Non-trivial OT structure requires simultaneously relaxing the initial flow distribution and adding an exogenous criterion, both of which are indispensable.
7.  **The main line abandons GFlowNet's signature capability.** O08 Assumption 3.1 requires \(\sum L=\sum R=1\), and \(Z\) is known; "only unnormalized rewards needed" does not hold on this line (O08 §7.6).
8.  **Competitive landscape: Our two papers are Workshop papers, while three competing products are main conference papers.** ULOT (NeurIPS 2025), GSBoG (ICML 2026), DDSBM (ICLR 2025); GSBoG is in the same quadrant as O08, with almost identical selling points, and four orders of magnitude larger scale (COMPETITOR_MATRIX §0–§3); the GSBoG version checked in this repository shows no clear guarantees for convergence rates, marginal errors, or objective suboptimality (C02 report §5), which is the most specific difference from O08's LP dual certificate.
9.  **The least crowded subsequent topic is error certificates, and the components are already in place.** On the GFN side, N03 (`reports/N03_2605.01729.md`) has completed "per-trajectory TB loss \(\le c^2\) ⇒ \(\mathrm{TV}\le1-e^{-2c}\)" (Thm. 3.5) and "sampling probability certificate, independent of state space size" (Thm. 3.6); on the OT side, N01 (AAAI 2026) provides bounds for "dual prediction error \(\|\hat p-p^\star\|_\infty\) → runtime" (Thm. 2), and N06 (ICLR 2026) provides a recipe for constructing benchmarks with analytical solutions (Thm. 3.1). The only missing piece is to change N03's right-hand side from TV to OT cost gap and marginal violation, with O08 Thm. 3.3's dual potential and complementary slackness as the bridge.
10. **Two topics are already crowded:** Conditional/amortized GFN–OT (UNOT, ULOT, sliced potential amortization), and entropy-regularized GFN–OT / SB on graphs (GSBoG, DDSBM, Sampling Decisions has written GFN flow functions as one-sided Schrödinger transport).

### 2. The Logical Chain of the Main Thread: What Each Link Lacks, and What the Next Link Supplies

| Link | Paper | What this link established | Gaps it left |
|---|---|---|---|
| ① | T00 / T02 | Flow conservation + terminal flow = reward \(\Rightarrow P_T\propto R\) (T02 core correctness theorem); Markovian flow uniquely determined by terminal flow and \(P_B\) (Prop. 18, item 3); feasible set is linear (Prop. 19 / Eq. (22)) | Only on DAGs; \(P_B\)'s degrees of freedom treated as an "engineering choice" with no principle for selection. The only mention of "shortest" in the original text is a single sentence in §2.6: "can prefer shorter paths." |
...
| ③ | T19 | With cycles, flow is a measure, trajectory set is infinite; 0-flow characterizes cycles; ratio-based losses push flow into cycles (Thm. 3), difference-based losses are stable (Thm. 4); R-flow = an acyclic flow + cycle space (Prop. 5); \(\mathbb E(\tau)\le F_{\mathrm{out}}(S^*)/R(S^*)\) (Thm. 2) | "Limit flow is acyclic" does not imply "unique solution"—R-flow is still not unique on DAGs when cycle space is zero (T19 editor's note); only an inequality between total flow and length |
| ④ | T36 | Takes \(P_B\) as the primary object; flow = expected visit count × terminal flow, a one-to-one correspondence with \((P_B,F(s_f))\) (Prop. 3.7); \(\sum_{s\ \text{internal}}F(s)=Z\,\mathbb E[n_\tau]\) (Prop. 3.12 / Eq. (10), sum over internal states, \(n_\tau\) counts internal state visits); unique solution when \(P_B\) is fixed (Cor. 3.11); formulates "minimum total flow" constrained optimization Eq. (11) and approximate solution using DB + state flow regularization | Minimum falls on the boundary of the polytope, where parts of \(P_B\) are zero, exceeding T36's own positivity assumption, thus only approachable by \(\lambda\)-regularization; does not state "what" the minimum flow is |
| ⑤ | O07 | Minimum expected length \(\Leftrightarrow\) only shortest paths are taken (Thm. 3.4, iff); replaces \(P_B>0\) with Assumption 3.1 \(\mathbb E[n_\tau]<\infty\); reduces pathfinding to training a non-acyclic GFN with flow regularization | Single source; \(R\equiv1\) or single target; only answers "path" not "how quality is distributed" |
| ⑥ | O08 | Adds another constraint: first-step edge flow = source distribution \(L\). Objective bilinearized to LP (Appendix A.1), reduced to divergence constraint \(\operatorname{div}\mathcal F=L-R\) (Eq. (11)), sandwiching yields \(\mathrm{GFlow}^\star=\mathrm{OT}^\star\) (Thm. 3.2); duality + complementary slackness (Thm. 3.3) | No bound between neural training and LP optimum; \(Z\) must be known; no unbalanced; hypergrid \(H\le20\), permutation \(n\le20\), no \(\mathrm{OT}^\star\) reference for \(n=20\) |

**Our assessment:** The true "turning point" in this chain is the equality \(\sum_sF(s)=Z\,\mathbb E[n_\tau]\) in ④. It establishes a precise relationship between a linear functional on the flow space (total flow) and a measure of policy behavior (expected number of steps). Subsequently, ⑤ and ⑥ are about minimizing this linear functional under different constraints. If ④ is understood, ⑤ and ⑥ become two exercises in a linear programming course; without understanding ④, O08 might be read as "GFlowNet miraculously learned OT."

### 3. OT Side Comparison: What is Equivalent, What is Analogous
...
| Cost = graph shortest path \(d_G\) | Trajectory length / expected number of steps | **Conditionally equivalent** (path action is sum of edge lengths) | O01 Prop. 14.9; O08 Eq. (7)–(8) |
...
| Complementary slackness: support \(\subset\) contact set | Optimal flow is positive only on tight edges | **Strict** (in LP form) | O01 Prop. 5.3; O08 Thm. 3.3 |
...
| Coupling \(\mathbf P\in\mathcal U(\mathbf a,\mathbf b)\) | End-point marginals of trajectory distribution | **Analogous**, degenerates to a single point for single source | O01 Remark 3.2 |
...
| Sinkhorn = alternating KL projection | Alternatingly enforced constraint training | **Analogous** (OT half-steps have closed form, GFN only SGD) | O01 Prop. 9.4/9.5 |
...
| Benamou–Brenier / JKO | "Flow × velocity" / training dynamics | **Atmospherically analogous only**, not to be used as a theorem | O01 Thm. 14.5, Def. 15.1 |

### 4. Competitive Landscape

Three dividing lines determine who is truly competing with whom (COMPETITOR_MATRIX §2):

- **Whether the graph is the "object being transported" or the "venue where transport occurs."** ULOT (C01) and DDSBM (C03) belong to the former: transport happens between graphs. O08, O07, and GSBoG (C02) belong to the latter: transport happens within a single graph. Comparing numbers across this line is meaningless.
- **Explicit enumeration vs. implicit expansion.** GSBoG scales to \(10^6\) nodes but requires \(\mu,\nu\) to be explicit vectors, neighborhood enumeration, and fully known topology. O08's permutation environment with \(20!\) states has no explicit representation. This is the only structural advantage of GFNs that cannot be replicated.
- **Temperature.** O08/O07 operate at \(\varepsilon=0\) (pure LP, vertex solutions, with dual certificates); GSBoG/DDSBM operate at \(\varepsilon>0\) (exponential family solutions, with IPF/IMF convergence mechanisms, no error bounds). Adding a KL term to O08 would make its research positioning overlap with GSBoG.

Specific threats (absorbing conclusions from O04/O05 reports):

| Source | What is threatened | Intensity | Counterpoint |
|---|---|---|---|
| **GSBoG** (C02, ICML 2026 main track) | O08's entire positioning narrative: "not static coupling but executable policy" is almost verbatim | Highest | Implicit graphs; indefinite time horizon (GSBoG requires fixed \(T\)); O08 has LP dual certificates, while the checked version of GSBoG shows no clear error guarantees; GSBoG is not open-source |
| **UNOT** (O05, ICML 2025) | Amortization axis: cross-dataset/cross-resolution generalization, 1.3–2.8% relative error on non-training sets (Table 4); its amortized dual potentials are the continuous version of \(\pi_x\) in O08 Thm. 3.3 | High | Cost changes with graph (UNOT §6 self-admits it cannot do this); \(\lvert\mathcal S\rvert\) too large to store the cost matrix |
...
| **α-DSBM** (O04, NeurIPS 2024 Spotlight) | Not a threat, but complementary: \(\mathrm{proj}_{\mathcal R}/\mathrm{proj}_{\mathcal M}\) is isomorphic to "fixed \(P_B\)" / "infer \(P_F\) from edge flow" point-by-point; Eq. (26) + Lemma D.2 is a ready-made proof template for "SB on graphs = entropy-regularized min-flow GFN" | — | Use it, don't compete with it |
| **DDSBM** (C03, ICLR 2025) | Application niche of "entropy-regularized GFN-OT for molecule editing" | Medium | DDSBM needs to solve QAP graph matching, GFN does not need to follow paths; DDSBM does not condition |

### 5. Final Ratings for Four Candidate Topics

| Topic | Rating | Rationale (updated after 2026 trend scan) |
|---|---|---|
| **① Balance residual → OT error bound, dual potentials as certificates** | **Do it. Low collision risk, complete components (see N01/N03/N06)** | GFN side: Stable GFlowNets already have TB residual → TV bound (2605.01729), Evaluation Balance treats residual as an evaluator (2603.01047, ICLR 2026). OT side: error theory is all about potentials—dual prediction → \(\varepsilon\)-relaxation runtime bounds (2601.20203, AAAI 2026), Sinkhorn potential statistical rates (2608.29152, O08 collaborator Belomestny), QOT's PL inequality (2605.27175). **No work has been found in the literature set checked by this repository that maps the residual of an approximately feasible flow to the cost gap** (classical LP primal-dual posterior error analysis needs separate checking). O08 Thm. 3.3's complementary slackness gives edge-wise certificates \(\mathcal F(s\to s')(\pi_{s'}-1-\pi_s)=0\), directly usable |
| ② Conditional GFN amortizes a family of graph OTs | Don't do (demoted to baseline) | After UNOT, ULOT, now there's sliced potential amortization (2604.15114), min-sliced plan (2511.19741); the only moat is implicit graphs, but how "a family" is defined on implicit graphs is still unclear |
| ③ Entropy-regularized GFN-OT / Schrödinger bridges on graphs | Don't do (as \(\varepsilon>0\) generalization of ①) | GSBoG occupies the main track slot in the same quadrant; Sampling Decisions (2503.14549) has already written GFN flow functions as one-sided Schrödinger transport; discrete SB has analytical solution benchmarks (2509.23348) and convergence rates (2607.19176). New entrants lack the bonus of defining the problem |
| ④ GFN proposal + classical OT correction | Change form | Vague "correction" has been concretized by 2601.20203 as "dual prediction → \(\varepsilon\)-relaxation, prediction error mapped to runtime." Feasible form: feed GFlowNet-learned state flow as dual prediction to a classical solver, report guaranteed speedup on explicit graphs—but this is an application of ①, not an independent topic |
...
#### 5b. Failure Modes and Minimum Publishable Units for Each Topic

The rating discusses whether to pursue a topic; this section further lists major failure modes and minimum publication goals.

| Topic | Most likely failure mode | Early warning signs | Minimum Publishable Unit (MPU) |
|---|---|---|---|
| ① Residual → OT error bound + dual certificate | **Bound too loose, not informative**: Proposition D's \(D_{\max}(1-e^{-2c})\) term is much larger than the actual gap on large graphs; reviewers ask "when is this bound non-trivial?" | If the residual-gap curve on day 2 of the three-day plan has a slope much smaller than the bound's slope (bound is two orders of magnitude looser than actual) | Proofs of Propositions A + C + D + plot showing "bound covers actual gap" on hypergrid/\(S_8\) + report only certificates on \(S_{20}\). Workshop length. Main conference requires A′ or tightened D |
| ② Conditional GFN amortizes graph OT | **Cross-instance generalization beaten by UNOT/ULOT**: they are faster and more accurate on explicit graphs; "family of instances" undefined on implicit graphs | Any design that requires instantiating the cost matrix for training | No honest MPU; can only serve as a baseline row in ①'s experiments: "gap of conditional GFN vs. single-instance GFN on family \((L,R)\)" |
| ③ Entropy-regularized GFN-OT / SB on graphs | **Required to compare numbers on N06 benchmark**, where DLightSB is set to strongest; GSBoG's narrative already occupies the space | Paper states "we are the first to connect SB with GFN"—Sampling Decisions (2503.14549) has already written about it | As a section on \(\varepsilon>0\) generalization of ①: prove that the certificate for the entropy-regularized version is the Sinkhorn potential gap, and provide a set of numbers on the N06 benchmark. Not a standalone paper |
| ④ GFN proposal + classical correction | **Only holds for explicit graphs, where GFN itself is redundant**: N01's ε-relaxation requires enumerating nodes | Experimental scale stops at explicit graphs with \(n\le10^4\) | As an application section of ①: feed learned state flow as dual prediction to ε-relaxation, report \(\|\hat p-p^\star\|_\infty\) and speedup (format of N01 Thm. 2). Not a standalone paper |

**Editorial note:** Under current personnel, time, and evidence conditions, only topic ① has an independent MPU among the four; the best outcome for ②, ③, and ④ is to be a section within ①. This is a priority recommendation, not a conclusion derived from evidence: the publication of competing works does not preclude new contributions from ②, ③, and ④, while whether ① can be published depends on whether the three unresolved issues listed in the revised §6b (original repair accounting, global certificate for implicit graphs, detour term) first pass the four checks: "mathematically correct, beyond classical primal-dual certificates, computable for implicit graphs, non-trivial."

### 6. A Decisive, Immediately Executable Experiment

Objective: To verify that "balance residuals can certify OT error" and to honestly position GFN-OT relative to classical and neural OT methods. All environments and baselines have existing code or exact solutions.

**Environments** (three tiers, from enumerable to implicit):

1.  Hypergrid \(\{0,\dots,H-1\}^D\), \(H\in\{10,15,20\}\), \(D=2\), allowing \(\pm1\) transitions (with cycles). Source distribution \(L\) uses Ball/Moon from O08, target \(R\) uses a corner multimodal distribution and is normalized. The exact LP solution can be obtained from `scipy.linprog` (as in O08 Figure 1), and \(\mathrm{OT}^\star\) can be obtained via network simplex.
2.  Cayley graph of the symmetric group \(S_n\) (adjacent transpositions), \(n\in\{4,8\}\) (with exact solutions), \(n=20\) (no exact solution, only certificate reporting).
3.  A weighted variant: randomly assign weights \(\{1,2\}\) to the edges of the hypergrid to test whether the certificate still holds beyond the unit edge length assumption of O08.

**Method Group**: min-flow GFN (O08's regularized TB, Eq. (20); or T36's code for DB + state flow regularization, run both, with \(\lambda\in\{10^{-1},10^{-2},10^{-3}\}\)); the same model with a PPO trainer (2606.15793); the same model with \(f\)-TB loss (2605.15417).

**Baselines**: network simplex (exact); Sinkhorn (\(\varepsilon\in\{0.1,0.01\}\), including analytical solution verification from 2509.23348); UNOT port (using shortest path matrix as cost, potential predictor + one-step Sinkhorn, explicit tier only); GSBoG reproduction (if code is still unavailable, use discretization of \(\alpha\)-DSBM as a proxy and note it).

**Metrics** (per tier, per method, 5 seeds):

-   Marginal error: Terminal distribution TV (exact enumeration for enumerable tiers; for implicit tiers, only the probabilistic certificate from N03 Thm. 3.6 can be used, or marked as "uncertified"); T36's fixed-point number distribution \(C(k)L^1\) is separately named as coarse-grained diagnosis—it is a lower bound direction for the full TV, and cannot upper bound the full TV in reverse; source marginal violation \(\lVert \hat L-L\rVert_1\) (overall zero under hard constraints, report empirical deviation).
-   Transport cost: Report three quantities—endpoint coupling cost \(\sum_{u,x}d(u,x)\Pi^\theta_{u,x}\), actual transport segment cost \(\mathbb E\lvert\tau\rvert\), and the difference between the two (detour term); compare each with \(\mathrm{OT}^\star\). Do not report \(\lVert\Pi_\theta-\Pi^\star\rVert_1\): zero-temperature optimal coupling may not be unique, so distance to a specific reference optimal solution is not a valid suboptimality indicator.
...
**Expected Results**: For explicit small graphs, use exact solvers to verify target values and constraint satisfaction (resource cost ranking reported separately, not as a correctness criterion); the goal for implicit tiers is to construct computable and non-trivial global certificates, and compare them with constant potentials and analytically feasible potentials—this is the research objective to be verified, not a preset conclusion; certificates should cover true suboptimality and report tightness separately.

**Failure Criteria and Negative Controls**: Two negative controls must be included first—an exactly balanced but non-optimal coupling (placing mass on cross-pairings, see §6b counterexample 1), and the same coupling but with detours (§6b counterexample 2)—to check if the certificate correctly identifies them as suboptimal; if the certificate fails for negative controls, its construction is flawed. The correlation coefficient between residuals and gaps is only for empirical diagnosis, not a necessary condition for certificate validity. The weighted variant (edge weights \(\{1,2\}\)) must synchronously use weighted objectives, weighted shortest path costs, and dual constraints with edge weights as right-hand sides; failure only indicates that the proof or implementation of this extension did not pass, and does not imply that the unit edge length assumption is essential. Zero-weight cycles require separate analysis.

### 6b. Theorem Draft for Topic ①: From Balance Residuals to OT Certificates

Below, we decompose "residual → OT error bound" into four independently provable propositions. Each is marked with its dependencies on known results, its current status, and what is still missing. We use the notation from O08: finite directed graph \(G\), source set \(U\), terminal set \(\mathcal X\), source distribution \(L\), normalized target \(R\), internal edge set \(E^\circ\), and shortest path distance \(d(u,x)\). The learned model provides a forward policy \(P_F^\theta\), and from it, induced edge flow \(\mathcal F^\theta\) and endpoint coupling \(\Pi^\theta_{u,x}=\sum_{\tau:u\rightsquigarrow x}\mathbb P^\theta(\tau)\).

**Proposition A (Residual → Endpoint Coupling Marginal Error).** If \(\mathcal L_{TB}(\tau)\le c^2\) for all trajectories, then \(\|\text{second marginal of }\Pi^\theta_{\cdot,x}-R\|_{TV}\le1-e^{-2c}\).
Dependency: N03 Thm. 3.5 (trajectory-level branching) holds verbatim—the terminal marginal is the GFlowNet's \(P_T\). Status: **Established**. For the sampling version, use N03 Thm. 3.6, at the cost of a confidence term \(\log(1/\alpha)(1/m+1/n)\).
Note: N03's DB/FM branching depends on the maximum trajectory length \(L\). O08's graph has cycles, so \(L=\infty\), and **cannot be used**. O08's training objective Eq. (20) is **regularized TB with a source distribution term** (numerator uses \(L(s_1)\) instead of \(Z\), plus flow regularization \(\lambda R(s)/P_F(s_f\mid s)\)), so N03's trajectory-level branching formally aligns. The DB + state flow regularization in T36's public code is a different implementation; Proposition A′ is needed only when using it. Furthermore, the object of certification should be the squared residual of TB, without the flow regularization term.

**Proposition A′ (Non-acyclic DB Residual → Marginal Error, using Expected Length instead of Maximum Length).** If \(\mathcal L_{DB}(s\to s')\le c^2\) for every edge, and \(\mathbb E_{\mathbb P^\theta}[n_\tau]\le\bar n\), then \(\mathrm{TV}\le1-e^{-2\bar n c}\) holds in expectation (treating random length trajectories with Jensen's inequality).
Dependency: T36 identity \(\sum_sF(s)=Z\,\mathbb E[n_\tau]\) provides a computable upper bound for \(\bar n\) (total flow divided by \(Z\)). Status: **To be proven**; the difficulty is that N03's proof uses a uniform bound based on the longest trajectory. Switching to expectation requires controlling the long tail, and Assumption 3.1 (\(\mathbb E[n_\tau]<\infty\)) is precisely the prerequisite.

**Proposition B (Violation of Source Marginal).** O08 fixes the first-step edge flow to \(L\). If the implementation uses a soft constraint (penalty coefficient \(\lambda_L\)), then \(\|\hat L-L\|_1\) is a quantity directly readable from the training log and requires no proof. If it's a hard constraint (first step sampled directly from \(L\), as in O08's implementation), then \(\hat L=L\) holds identically. Status: **Trivial**, but it must be stated in the paper which method is used.

**Proposition C (Approximately Feasible Flow → Cost Gap, Dual Certificate).** Let \(\hat\pi\) be any dually feasible potential (\(\hat\pi_{s_0}=0\), \(\hat\pi_{s'}-\hat\pi_s\le1\) for all internal edges). For any exactly feasible flow \(\mathcal F\) (\(\operatorname{div}\mathcal F=L-R\)), LP weak duality gives
...
The right-hand side only contains observable quantities. Status: **Established** (this is O08 Thm. 3.3 and Prop. A.4's duality plus weak duality). Two things are missing: (i) The learned \(\mathcal F^\theta\) is only **approximately** feasible; the divergence residual \(\|\operatorname{div}\mathcal F^\theta-(L-R)\|_1\) needs to be incorporated into the gap—this term is controlled by Proposition A/A′. (ii) How to construct a **dually feasible** \(\hat\pi\) from \(\theta\): take \(\hat\pi_s=\) some affine transformation of the learned state flow logarithm, then perform a "1-Lipschitz projection" (for each edge, clip \(\hat\pi_{s'}\) to \(\le\hat\pi_s+1\), i.e., one Bellman–Ford-like relaxation). Feasibility is guaranteed after projection, at the cost of traversing edges—feasible for explicit graphs, but for implicit graphs, it can only be done on the sampled subgraph, yielding a subgraph certificate (corresponding to the subgraph version of N03 Cor. 3.7).

**Proposition D (Combining A and C) — Currently not established, due to two quantities being conflated.** The initial draft was written as a "mechanical combination of A + C", but was retracted after two counterexamples were pointed out by external reviewers (Codex, gpt-6-astra):

- **Counterexample 1 (Zero residual ≠ OT optimal; numerical verification in `experiments/README.md`).** Two sources \(u_1,u_2\) of equal mass, two targets \(x_1,x_2\). Distance is 1 for same-indexed pairs, \(K\) for cross-indexed pairs. If all mass is placed on cross-indexed pairs and transported along their shortest paths: marginals are exact, DB/TB residuals are zero, flow conservation holds exactly, but cost is \(\approx K\) while optimal is 1. **Balance residuals control "whether it is a valid GFlowNet", never "whether it is the one with minimum total flow"**; optimality can only be certified separately by the dual certificate (Proposition C's gap). Therefore, the original "residual-gap curve is monotonic" in §6 is not a necessary condition for certificate validity, but merely an empirical diagnostic.
- **Counterexample 2 (Endpoint cost ≠ Path cost; numerical verification in `experiments/README.md`).** A coupling can be realized by arbitrarily long detours or cycles and still precisely satisfy balance: \(\sum_e\mathcal F^\theta(e)=\mathbb E|\tau|\ge\sum_{u,x}d(u,x)\Pi^\theta_{u,x}\), with equality only if all trajectories take shortest paths. Certifying the "Kantorovich cost of endpoint coupling" is not equivalent to certifying the "transport cost of policy execution"; O08 Table 1 reports \(\mathbb E|\tau|\), which is the latter. If the paper claims policy execution efficiency, it must directly certify the weighted cost of the visited flow or separately control the detour term \(\mathbb E[|\tau|-d(u,x)]\).

**Revised Proposition D.** Let the source marginal be exact \(L\) (hard constraint satisfaction), and \(\delta:=\mathrm{TV}(P_T,R)\) is controlled by Proposition A. (i) There exists \(\Pi'\in\Gamma(L,R)\) such that \(\|\Pi^\theta-\Pi'\|_1\le2\delta\) (move excess termination mass along arbitrary routes to missing targets; the factor of 2 comes from the conversion between TV and \(L_1\), which was omitted in the original manuscript); (ii) Route \(\Pi'\) along shortest paths to form an exactly feasible flow \(\tilde{\mathcal F}\), then \(\text{cost}(\tilde{\mathcal F})\le\text{cost}(\Pi^\theta)+2\delta\,D_{\max}\), where \(D_{\max}=\max_{u,x}d(u,x)\) is the **maximum source-to-sink shortest path cost** (finite under O08's fully reachable assumption; it is not the graph diameter in the usual sense); (iii) Applying Proposition C to \(\tilde{\mathcal F}\) and any dually feasible \(\hat\pi\), we get
\[\text{cost}(\Pi^\theta)-\mathrm{OT}^\star\;\le\;\mathrm{gap}(\tilde{\mathcal F},\hat\pi)\;+\;2\delta\,D_{\max}.\]
This **only certifies the cost of endpoint coupling**; the execution cost also needs to add a detouring term. Replacing \(D_{\max}\) with a tighter quantity using complementary slackness is currently **unproven**—an exactly optimal flow only uses tight edges, but this does not mean that the error of an approximate flow is confined to the vicinity of tight edges, and repair might even need to reach targets outside the current support.

**Construction of Dually Feasible Potentials (Proposition C, part (ii), revised).** The original manuscript's statement "a single Bellman–Ford-like relaxation" is incorrect: a single edge scan does not guarantee all difference constraints are simultaneously satisfied (counterexample: \(a\to b\to c\), initial potentials \((0,100,100)\), checking \(b\to c\) first then fixing \(a\to b\) yields \((0,1,100)\), and \(b\to c\) immediately becomes violated). The correct approach is to solve the difference constraint system to its closure (Bellman–Ford run to convergence, \(O(|V||E|)\)), then check the maximum violation for all edges and report the numerical tolerance; this is **feasibilization**, not a projection in any norm sense. On implicit graphs, this can only be done on the sampled subgraph, but **feasible potentials on a subgraph may not be feasible on the original graph** (missing edges could be shortcuts), thus not yielding a global dual certificate—this is different from N03 Cor. 3.7's "TV certificate conditional on a subset of terminations," and the original manuscript's equating of the two is incorrect. **How to obtain a non-trivial global dual certificate on implicit graphs currently has no answer**; the trivial lower bound given by constant potentials is a baseline achievable by any method.

**What this draft indicates (revised version).** A is transferable after filling in prerequisites (requires TB form, bidirectional complete trajectory law normalization, source and termination edges both included in residuals); A′ has a brief proof under the setting of fixed source-sink flow of 1 (reviewer's suggested path: \(\mathrm{TV}(P_T,R)\le\mathrm{TV}(P,Q)\le\mathbb E_P[1-e^{-cN}]\le1-e^{-c\,\mathbb E_PN}]\), where \(N\) is the number of edges including residuals), but there are counterexamples if \(Z\) is learnable; the global distribution version of B holds, empirical frequencies reported separately; C holds for exactly feasible flows; D only certifies endpoint costs and has a slack term of \(2\delta D_{\max}\). The three truly mathematical aspects that determine whether this line of work can be published are: **primal repair and residual accounting for approximate flows** (folding \(\operatorname{div}\mathcal F^\theta-(L-R)\) into the gap), **global dual certificates on implicit graphs**, and **control of the detouring term**. Our assessment: This represents a clear differentiation from the current status of GSBoG, where "no error guarantees were found in the repository's verified version." However, the original assessment that "A + C + D all hold and mechanically combine to form a paper" is retracted; the publication-level judgment is also downgraded to "priority recommendation under current personnel, time, and evidence conditions," rather than a conclusion derived from evidence.

### 6c. Three-Day Work Plan

Before writing any code, nail down three things, each taking half a day to a day:

| Day | What to do | Output | Criterion |
|---|---|---|---|
| 1 | **Completed** (`experiments/day1_reference_lp.py`, results in `experiments/README.md`): hypergrid \(H\in\{10,15,20\}\), \(D=2\), ball/moon sources. Exact LP solution of O08 Eq. (11) and Kantorovich coupling LP cost are equal (difference \(\le4\times10^{-14}\)). HiGHS dual potentials satisfy all edge constraints, complementary slackness is zero edge-wise. **Both counterexamples numerically confirmed**: zero-residual mismatched flow cost is 60–78% higher; execution cost of optimal coupling + detours is 3.2–3.5 times OT*, residuals still zero. | Script + 6 JSONs + README | Achieved. Note: O08 did not publish \(r_{\text{in}},r_{\text{out}},\delta,\varepsilon,D,R_i\), so Table 1's 4.351 was not reproduced, only the identity was verified. |
| 2 | Train min-flow GFN with regularized TB according to O08 Eq. (20) (based on T36 public code, \(\lambda\in\{10^{-1},10^{-2},10^{-3}\}\)). Export \(\mathcal F^\theta\) every 500 steps, construct \(\hat\pi\) according to Proposition C (Bellman–Ford projection) and calculate gap; simultaneously calculate mean, max of edge-wise DB residuals and TB residuals (for sampled trajectories). | One plot: \(x\) = residual, \(y\) = gap, three \(\lambda\) curves | Curves are monotonic and gap \(\to\) 0 as residual \(\to\) 0; if gap does not decrease with residual, it means flow regularization pushes the flow to non-optimal vertices, and the narrative for topic ① needs to change. |
| 3 | Change N06's baseline construction recipe to a zero-temperature version—**cannot just randomly generate potentials** (under constant potentials, no edges are saturated, only degenerate instances with \(R=L\) are obtained): must jointly construct feasible potentials \(\pi\), tight edge paths, and non-negative transport flows, ensuring each positive source mass reaches allowed sinks along tight edges, then define \(R\) and check for non-degeneracy, to obtain a family of **GFN–OT instances with analytical optimal flows**; rerun Day 2's process on these. | `synthetic_bench.py` + 10 instances + a short proof (that the constructed flow is indeed optimal) | Report true suboptimality and certificate upper bound separately; use absolute error when gap is near zero; coverage (certificate \(\ge\) true value) should be 100%, tightness reported separately. Zero-temperature construction is a result to be proven in this paper, not directly inherited from N06's finite-temperature construction. |

Fork after three days: If Day 2 curves are monotonic, proceed with A + C + D to write a workshop version and simultaneously attack A′; if not monotonic, first diagnose the conflict between flow regularization and optimality (T36 Report §3.5 discussed this: minimum falls on polyhedron boundary, \(\lambda\) regularization can only approximate), then decide whether to change the training objective.

### 7. Eight Most Common Misinterpretations

1.  **"\(P_B\) can be freely chosen" \(\neq\) "DB is an empty constraint."** Once \(\hat P_B(s\mid s')=\hat P_F(s'\mid s)\hat F(s)/\hat F(s')\) is required, normalization \(\sum_s\hat P_B=1\) immediately implies flow matching (T02 Eq. (31)). Freedom exists on the manifold of "normalizing to the parent set for each state," and only holds in deterministic environments (Counterexample 50).
2.  **T19 "acyclic limit flow" \(\neq\) "unique solution."** T19 eliminates infinitely scalable directed cycles; the actual selection of a point comes from the linear objective in T36 Eq. (11) and \(\ell^2\) tie-breaking in the sense of O02 Cor. 1.
3.  **O08's \(\mathrm{GFlow}^\star\) is the reduced objective \(\sum_{E^\circ}\mathcal F\), not \(\sum_{s\in\mathcal I}\mathcal F(s)=\mathbb E[n_\tau]\); the two differ by 1** (the step \(s_0\to u\) is not part of the transport segment). The direct alignment of \(\mathbb E\lvert\tau\rvert\) with \(\mathrm{OT}^\star\) in Table 1 indicates that the transport segment length is reported.
4.  **On single-source DAGs, "learning an OT plan" is a vacuous statement** (O01 Remark 3.2). The first non-trivial case is \(2\times2\), with \((n-1)(m-1)\) degrees of freedom.
5.  **O04's \(\alpha\) is not a learning rate; it is an interpolation coefficient in the coupling space** (Eq. (22)), and the paper does not prove that \(\alpha<1\) is better—Appendix B states that \(\alpha=1\) is optimal when the single-step cost is constant.
6.  **O06's "flow neural network" refers to the velocity field of continuous normalizing flows, not GFlowNet.**
7.  **The local PDF of T02 is an expanded version of arXiv v5 (76 pages); proposition numbering may differ from the JMLR 2023 published version (24(210):1–55)**; T03 cites Foundations using earlier numbering (Prop. 3 / Cor. 1 / Prop. 6 / Prop. 10).
8.  **O07/O08 are from the ICML 2026 SPIGM Workshop, not the main conference; GSBoG is from the ICML 2026 main conference.** Any statement equating their prestige is incorrect; our side is at a disadvantage in "narrative positioning," a fact that must be confronted when choosing topics.

### 8. Open Problems

1.  **What does the bound for residual \(\to\) cost gap look like?** Bounds for TB residual \(\to\) TV already exist (N03 Thm. 3.5/3.6; note that its DB/FM branches depend on the maximum trajectory length \(L\), which fails in O08's non-acyclic setting, so only the TB branch is transferable); OT cost is a linear functional of the coupling, so the relationship between marginal violation and cost gap should be tighter than the TV bound. Source: O08 §7.2.
2.  **Weighted graphs.** O08 only covers unit edge lengths; with weights, zero-weight edges can create zero-cost cycles. Does the minimum flow principle still select an acyclic solution? Source: O07 Report §7, O08 §5, item 5.
3.  **Gap between neural training and LP optimality.** O08 approximates constraints with soft penalties \(\lambda\). Table 2 shows quantitative trade-offs for \(\lambda\), but no theory. Source: O08 §5, item 7.
4.  **Unbalanced and unknown \(Z\).** Can GFlowNet's ability to "only need unnormalized rewards" return after relaxing \(\sum L=\sum R\)? Unbalanced transport on Orlicz–Sobolev graphs (TRENDS_OT §2.1) is a candidate objective. Source: O08 §7.6.
5.  **What functional does the minimum total flow correspond to in continuous state spaces?** Berner et al. provide discrete↔continuous equivalence tools (TRENDS_GFN §2.3), and Ergodic Generative Flows provide a continuous non-acyclic framework, but no one has written down a continuous version of Thm. 3.2.
6.  **Are implicit regularization and minimum flow consistent?** To which internal flow does training converge without flow regularization (Secrets of GFlowNets' Learning Behavior, 2505.02035; Fisher geometry, 2608.03967)?
7.  **Relationship between path multiplicity induced by symmetry and internal flow degrees of freedom.** Symmetry-Aware GFlowNets (2506.02685) and T02's \(P_B\) degrees of freedom are two sides of the same phenomenon; there is no OT-perspective analysis yet.
8.  **Is short flow beneficial for generalization?** O08 §7.7 points out that this has not been verified and may even be negative: shortest path strategies concentrate mass on a few paths, which contradicts GFlowNet's selling point of diversity.
9.  **Relationship between minimum flow and entropy selection principle.** When the OT plan is not unique under distance costs, is the plan selected by the small regularization limit (TRENDS_OT §2.2) the same as the plan selected by the minimum total flow?
10. **Multi-objective combination.** Routing by Reaching (2602.21565) combines pre-trained GFlowNets during inference; in OT language, this corresponds to multi-marginal transport, for which no corresponding theorem exists yet.

---

*This document was compiled by the awesome_Gflow_OT project team based on 18 interpretation reports and two trend scans; evidence pointers for each judgment can be found in the parentheses (report ID and original theorem number).*

# Chapter 10: Repository Tour and Methodology

## 10.1 Directory Structure

| Path | Content |
|---|---|
| `papers/` | 24 original PDF papers (18 core + 6 2026 additions) (arXiv versions), filenames are arXiv numbers |
...
| `reports/` | 24 in-depth interpretations (`<ID>_<arXiv>.md`), competitor matrix, two trend scans, INSIGHTS, this summary report and its PDF |
| `data/meta/` | One JSON metadata card per paper (title, authors, venue, venue_type, code, one-sentence summary in Chinese and English) |
| `data/papers.yaml`, `data/candidates_*.csv`, `data/scan_*.json` | Seed list, trend candidates, raw arXiv scans |
| `src/generator.py` | Generates `README.md` / `README_zh.md` from `data/meta` and candidate CSVs (data-driven model of awesome-ml4co) |
...
Each interpretation strictly follows 8 sections: One-sentence positioning and metadata table \(\to\) Problem setting and notation \(\to\) Core contributions (by original numbering) \(\to\) Methods and derivation highlights \(\to\) Experiments and evidence \(\to\) Assumptions and applicability boundaries \(\to\) Position in the main storyline \(\to\) Reusable insights and open problems \(\to\) References; an "Editorial note" is appended at the end to record self-determined ambiguities. All numbers and theorem numbers include original sources; if not provided in the original, "Not provided in original" is written.

## 10.3 Translation Pipeline and QA

The translation engine is SuperTranslate (`pdf_zh_translator`): it does not re-layout pages; formulas, figures, and citations are frozen first, and after the main text is translated, it is filled back according to its original coordinates. Parameters are `--preserve-graphics-text --skip-overflow`, with DeepSeek backend. After each translation, `inspect` is run for page-by-page object-level comparison, and the number of issues is recorded in Appendix A. Known limitations: `--skip-overflow` causes untranslated text that doesn't fit to remain in English, leading to `untranslated_block` in mathematically dense sections of the appendix proof pages; font size scaling triggers `font_size_drift`. All 24 papers have been translated. O01 (Peyré's lecture notes, 480 pages) is the largest in volume and was translated separately using the Gemini 2.5 Flash backend on OpenRouter (`scripts/translate_o01_openrouter.sh`), taking about 4 hours. QA reported 69 issues (mainly font size drift and English remnants in mathematically dense appendix sections).

## 10.4 Publication Status Discipline

Main conference / Journal / Workshop / Preprint are marked separately; arXiv comments often lag, so for anything that can be verified against OpenReview, dblp, or official proceedings, the latter shall prevail (e.g., three competing papers were thus verified as GSBoG being an ICML 2026 main conference paper). The trend scanning section relies solely on the comment field, with each entry marked "unverified".

## 10.5 How to Contribute

Add a JSON card in `data/meta/`, optionally add an interpretation in `reports/`, then run `python3 src/generator.py` to regenerate the README; translate new papers using `scripts/translate_batch.sh <arXiv>`; rebuild this report using `python3 scripts/build_report.py zh && bash scripts/build_pdf.sh zh`.

## 10.6 External Review and Revision Log

The comprehensive sections of this report (INSIGHTS, Abstract and Chapter 1, six 2026 supplementary interpretations) were reviewed individually by the local Codex CLI (model `gpt-6-astra`, inference tier max, read-only sandbox) using the prompt "ICML Area Chair level reader" (script `scripts/codex_review.sh`, structured JSON output stored in `reviews/codex/`). Seven reviews (including interpretations of the three core papers O08/O07/T36) raised a total of 135 factual risks and 58 logical gaps. Substantive issues corrected based on the reviews include: the training objective for O08 Eq. (20) is regularized TB with a source distribution term (originally miswritten as DB + flow regularization); the identity number in T36 is Prop. 3.12 / Eq. (10) (originally miswritten as Prop. 3.6); in N03 Thm. 3.6, \(c\) is the square root of the maximum loss; the second term in N02 Eq. (19) uses the target distribution \(\pi\) instead of the stationary distribution of the kernel; O08 Thm. 3.3 only applies to single-source problems with the initial step constraint removed, fixed marginals require Appendix A.4; N01 indeed compared with network simplex and was 12 times faster on average; code links for N02/N05/N06 were all provided in the original papers; the judgment in INSIGHTS §6b Proposition D "mechanical combination works" was retracted (two counterexamples: zero residual does not imply OT optimality, and endpoint cost does not equal path cost); the Workshop status of O07 was downgraded to unverified. The second round (core three papers) further corrected: the draft boundary for "residual → OT gap" in O08 interpretation §7.2 missed the suboptimal coupling term (cross-pairing counterexample, numerically confirmed in `experiments/`), the two premises "cycles are necessary" and "source and target sets are isolated" were written incorrectly, and the BFS potential in Theorem 3.3 was misused for problems with fixed \(L\); the O07 interpretation missed \(Z\) and the \(\lambda\) multiple in reward matching, and the reading of Figure 3; the T36 interpretation incorrectly equated the sum of squares of state flows with O02's quadratic regularization of edge flows. Any item pointed out by the review that this report could not independently verify has been rephrased with qualifying language or marked "unverified" in the main text.

## 10.7 Reproducible Experiments

`experiments/day1_reference_lp.py` verifies on a hypergrid (\(H\in\{10,15,20\}\), ball/moon source, corner target) using exact LP that: GFlow* = OT* (difference \(\le4\times10^{-14}\)), dual potentials are edge-wise feasible, and complementary slackness is zero; it also numerically confirms the two counterexamples in INSIGHTS §6b—zero residual with mismatched flow costs 60–78% higher, and optimal coupling with detours results in execution costs 3.2–3.5 times OT* while residual remains zero. Results and explanations are in `experiments/README.md`.

# Appendix A Paper List and Chinese-English QA

| ID | Title | Venue | Type | File | QA Issues |
|---|---|---|---|---|---|
