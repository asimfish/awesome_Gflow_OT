...
Below, we break down "residual → OT error bound" into four independently provable propositions. Each is marked with its dependencies on known results, its current status, and what is still missing. We use the notation from O08: finite directed graph \(G\), source set \(U\), terminal set \(\mathcal X\), source distribution \(L\), normalized target \(R\), internal edge set \(E^\circ\), and shortest path distance \(d(u,x)\). The learned model provides a forward policy \(P_F^\theta\), which induces edge flows \(\mathcal F^\theta\) and endpoint couplings \(\Pi^\theta_{u,x}=\sum_{\tau:u\rightsquigarrow x}\mathbb P^\theta(\tau)\).

**Proposition A (Residual → Endpoint Coupling Marginal Error).** If \(\mathcal L_{TB}(\tau)\le c^2\) for all trajectories, then \(\|\text{the second marginal of }\Pi^\theta_{\cdot,x}-R\|_{TV}\le1-e^{-2c}\).
Dependencies: N03 Thm. 3.5 (trajectory-level branching) holds verbatim—the terminal marginal is the GFlowNet's \(P_T\). Status: **Established**. For the sampling version, use N03 Thm. 3.6, at the cost of a confidence term \(\log(1/\alpha)(1/m+1/n)\).
Note: N03's DB/FM branching depends on the maximum trajectory length \(L\). O08's graph has cycles, so \(L=\infty\), and **cannot be used**. O08's training objective Eq. (20) is **regularized TB with a source distribution term** (numerator uses \(L(s_1)\) instead of \(Z\), plus flow regularization \(\lambda R(s)/P_F(s_f\mid s)\)), so N03's trajectory-level branching formally aligns. The DB + state flow regularization in T36's public code is a different implementation; Proposition A′ is needed when using it. Furthermore, the certification object should be the squared residual of TB, without the flow regularization term.
...
The right-hand side only contains observable quantities. Status: **Established** (this is O08 Thm. 3.3 and Prop. A.4's duality plus weak duality). Two things are missing: (i) The learned \(\mathcal F^\theta\) is only **approximately** feasible; the divergence residual \(\|\operatorname{div}\mathcal F^\theta-(L-R)\|_1\) needs to be incorporated into the gap—this term is controlled by Proposition A/A′. (ii) How to construct a **dually feasible** \(\hat\pi\) from \(\theta\): take \(\hat\pi_s=\) some affine transformation of the learned state flow logarithm, then perform a "1-Lipschitz projection" (for each edge, clip \(\hat\pi_{s'}\) to \(\le\hat\pi_s+1\), i.e., one Bellman–Ford-like relaxation). Feasibility is guaranteed after projection, at the cost of traversing edges—feasible for explicit graphs, for implicit graphs only on sampled subgraphs, yielding a subgraph certificate (corresponding to the subgraph version of N03 Cor. 3.7).

**Proposition D (Combining A and C) — Currently not established, due to two quantities being conflated.** The initial draft was written as "mechanical combination of A + C", but was retracted after two counterexamples were pointed out by external reviewers (Codex, gpt-6-astra):

- **Counterexample 1 (Zero Residual ≠ OT Optimal).** Two sources \(u_1,u_2\) of equal mass, two targets \(x_1,x_2\). Distance is 1 for same-indexed pairs, \(K\) for cross-indexed pairs. If all mass is placed on cross-indexed pairs and transported along their shortest paths: marginals are exact, DB/TB residuals are zero, flow conservation holds exactly, but cost is \(\approx K\) while optimal is 1. **Balance residuals control "whether it is a valid GFlowNet", never "whether it is the one with minimum total flow"**; optimality can only be certified separately by the dual certificate (Proposition C's gap). Therefore, the original "residual-gap curve is monotonic" in §6 is not a necessary condition for certificate validity, but merely an empirical diagnostic.
- **Counterexample 2 (Endpoint Cost ≠ Path Cost).** A coupling can be realized by arbitrarily long detours or cycles and still precisely satisfy balance: \(\sum_e\mathcal F^\theta(e)=\mathbb E|\tau|\ge\sum_{u,x}d(u,x)\Pi^\theta_{u,x}\), with equality only if all trajectories take shortest paths. Certifying the "Kantorovich cost of endpoint coupling" is not equivalent to certifying the "transport cost of policy execution"; O08 Table 1 reports \(\mathbb E|\tau|\), which is the latter. If the paper claims policy execution efficiency, it must directly certify the weighted cost of the visited flow or separately control the detour term \(\mathbb E[|\tau|-d(u,x)]\).

**Revised Proposition D.** Let the source marginal be exact \(L\) (hard constraint satisfaction), and \(\delta:=\mathrm{TV}(P_T,R)\) is controlled by Proposition A. (i) There exists \(\Pi'\in\Gamma(L,R)\) such that \(\|\Pi^\theta-\Pi'\|_1\le2\delta\) (move the excess termination mass along any route to the missing targets; the factor of 2 comes from the conversion between TV and \(L_1\), which was omitted in the original manuscript); (ii) Route \(\Pi'\) along shortest paths to form an exactly feasible flow \(\tilde{\mathcal F}\), then \(\text{cost}(\tilde{\mathcal F})\le\text{cost}(\Pi^\theta)+2\delta\,D_{\max}\), where \(D_{\max}=\max_{u,x}d(u,x)\) is the **maximum source-sink shortest path cost** (finite under the all-to-all reachability assumption in O08; it is not the graph diameter in the usual sense); (iii) Applying Proposition C to \(\tilde{\mathcal F}\) and any dually feasible \(\hat\pi\), we get
\[\text{cost}(\Pi^\theta)-\mathrm{OT}^\star\;\le\;\mathrm{gap}(\tilde{\mathcal F},\hat\pi)\;+\;2\delta\,D_{\max}.\]
This **only certifies the cost of endpoint coupling**; the execution cost requires an additional detour term. Replacing \(D_{\max}\) with a tighter quantity using complementary slackness is currently **unproven**—an exactly optimal flow only uses tight edges, but this does not imply that the error of an approximate flow is confined to the vicinity of tight edges, and repair might require reaching targets outside the current support.

**Construction of Dually Feasible Potentials (Proposition C, part (ii), revised).** The original manuscript's statement "a single Bellman–Ford-style relaxation" is incorrect: a single edge scan does not guarantee all difference constraints are simultaneously satisfied (counterexample: \(a\to b\to c\), initial potentials \((0,100,100)\), checking \(b\to c\) first then fixing \(a\to b\) yields \((0,1,100)\), and \(b\to c\) immediately becomes violated). The correct approach is to solve the difference constraint system to its closure (Bellman–Ford run to convergence, \(O(|V||E|)\)), then check the maximum violation for all edges and report the numerical tolerance; this is **feasibilization**, not a projection in any norm sense. On implicit graphs, this can only be done on the sampled subgraph, but **feasible potentials on a subgraph may not be feasible on the original graph** (missing edges could be shortcuts), thus not yielding a global dual certificate—this is different from N03 Cor. 3.7's "TV certificate conditional on the termination subset," and the original manuscript's equating of the two is incorrect. **How to obtain a non-trivial global dual certificate on implicit graphs currently has no answer**; the trivial lower bound given by constant potentials is a baseline achievable by any method.

**What this draft shows (revised version).** A is transferable after supplementing prerequisites (requires TB form, bidirectional complete trajectory probability normalization, source and termination edges both included in residuals); A′ has a brief proof under the setting of fixed source-sink flow of 1 (reviewer's suggested path: \(\mathrm{TV}(P_T,R)\le\mathrm{TV}(P,Q)\le\mathbb E_P[1-e^{-cN}]\le1-e^{-c\,\mathbb E_PN}]\), where \(N\) is the number of edges including residuals), but there are counterexamples if \(Z\) is learnable; the global distribution version of B holds, empirical frequencies reported separately; C holds for exactly feasible flows; D only certifies endpoint costs and has a slack term of \(2\delta D_{\max}\). The truly mathematical content that determines whether this line of work can be published hinges on three things: **primal repair of approximate flows and residual accounting** (folding \(\operatorname{div}\mathcal F^\theta-(L-R)\) into the gap), **global dual certificates on implicit graphs**, and **control of the detour term**. Our assessment: This is a clear differentiating direction compared to the current status of GSBoG, where "no error guarantee of any kind was found in the repository's checked version." However, the original assessment that "A + C + D all hold and mechanically combine to form a paper" is retracted; the publication-level assessment is also downgraded to "priority recommendation under current personnel, time, and evidence conditions," rather than a conclusion derived from evidence.

### 6c. Three-Day Kick-off Plan

Before writing any code, nail down three things, each taking half a day to a day:

| Day | What to do | Output | Criterion |
|---|---|---|---|
| 1 | Solve the exact LP of O08 Eq. (11) on a hypergrid \(H=10\), \(D=2\) using `scipy.linprog`, and simultaneously solve Kantorovich using network simplex, verifying that their costs are equal (reproducing 4.351 from O08 Figure 1); then construct the dual \(\pi^\star\) for the exact solution and verify complementary slackness holds edge-wise | A 200-line `reference_lp.py` outputting \((\mathcal F^\star,\pi^\star,\mathrm{OT}^\star)\) | Three numbers match to \(10^{-6}\); if any does not hold, it indicates a misunderstanding of the O08 setup |
| 2 | Train a min-flow GFN according to O08 Eq. (20) regularized TB (based on T36 public code, \(\lambda\in\{10^{-1},10^{-2},10^{-3}\}\)), export \(\mathcal F^\theta\) every 500 steps, construct \(\hat\pi\) according to Proposition C (Bellman–Ford projection) and calculate the gap; simultaneously calculate the mean and max of edge-wise DB residuals and TB residuals (for sampled trajectories) | A plot: \(x\) = residual, \(y\) = gap, three \(\lambda\) curves | Curves are monotonic and gap approaches 0 as residual approaches 0; if gap does not decrease with residual, it means flow regularization pushes the flow to non-optimal vertices, and the narrative for topic ① needs to change |
| 3 | Change N06's baseline construction recipe to a zero-temperature version—**cannot just randomly generate potentials** (under constant potentials, no edges are saturated, leading to degenerate instances where \(R=L\)): must jointly construct feasible potentials \(\pi\), tight edge paths, and non-negative transport flows, ensuring each positive source mass reaches allowed sinks along tight edges, then define \(R\) and check for non-degeneracy, to obtain a family of **GFN–OT instances with analytical optimal flows**; rerun Day 2's process on these | `synthetic_bench.py` + 10 instances + a short proof (that the constructed flow is indeed optimal) | Report true suboptimality and certificate upper bound separately; use absolute error when gap is near zero; coverage (certificate \(\ge\) true value) should be 100%, tightness reported separately. Zero-temperature construction is a result that needs to be proven in this paper and cannot be directly inherited from N06's finite-temperature construction |

Fork after three days: If Day 2 curves are monotonic, proceed with A + C + D to write a workshop version and simultaneously attack A′; if not monotonic, first diagnose the conflict between flow regularization and optimality (T36 Report §3.5 discussed this: minimum lies on the boundary of the polytope, \(\lambda\) regularization can only approximate), then decide whether to change the training objective.
...
The translation engine is SuperTranslate (`pdf_zh_translator`): it does not re-layout pages; formulas, figures, and citations are frozen first, then the body text is translated and filled back into its original coordinates; parameters `--preserve-graphics-text --skip-overflow`, DeepSeek backend. After translation, each paper is run through `inspect` for page-by-page object-level comparison, and the number of issues is recorded in Appendix A. Known limitations: `--skip-overflow` causes untranslated text that doesn't fit to remain in the original English, leading to `untranslated_block` in math-intensive sections of appendix proof pages; font size scaling triggers `font_size_drift`. All 24 papers have been translated. O01 (Peyré's lecture notes, 480 pages) is the largest, translated separately using the Gemini 2.5 Flash backend on OpenRouter (`scripts/translate_o01_openrouter.sh`), taking about 4 hours, with QA reporting 69 issues (mainly font size drift and remaining English in math-intensive appendix sections).

## 10.4 Publication Status Discipline

Main conference / journal / workshop / preprint are marked separately; arXiv comments are often delayed, so if OpenReview, dblp, or official proceedings can be verified, the latter takes precedence (this is how three competing papers were verified, e.g., GSBoG as an ICML 2026 main conference paper); the trend scanning section only relies on the comment field and each item is marked "unverified".
...
The comprehensive sections of this report (INSIGHTS, Abstract and Chapter 1, six 2026 supplementary interpretations) were reviewed individually by a local Codex CLI (model `gpt-6-astra`, inference tier max, read-only sandbox) using "ICML area chair level reader" prompts (script `scripts/codex_review.sh`, structured JSON output stored in `reviews/codex/`). The four reviews collectively raised 78 factual risks and 35 logical gaps. Substantive issues corrected based on reviews include: the training objective for O08 Eq. (20) is regularized TB with a source distribution term (originally miswritten as DB + flow regularization); the identity in T36 is numbered Prop. 3.12 / Eq. (10) (originally miswritten Prop. 3.6); in N03 Thm. 3.6, \(c\) is the square root of the maximum loss; the second term in N02 Eq. (19) uses the target distribution \(\pi\) instead of the stationary distribution of the kernel; O08 Thm. 3.3 only applies to single-source problems with the first-step constraint removed, fixed dual marginals require Appendix A.4; N01 indeed compared with network simplex and the group mean was 12 times faster; code links for N02/N05/N06 were provided in the original papers; Proposition D in INSIGHTS §6b, "mechanical combination is sufficient," was retracted (zero residual does not imply OT optimality, and endpoint cost does not equal path cost are two counterexamples); the workshop status of O07 was downgraded to "to be verified." Any item pointed out by reviews that this report could not independently verify has been rephrased with qualifiers or marked "to be verified" in the main text.
