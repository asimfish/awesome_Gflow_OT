...
\item \textbf{该做什么}：\HL{balance 残差 → OT 误差界 + 对偶势证书}。配件已齐：N03 给了 TB 残差 → TV 的证书（Thm. 3.5/3.6），N01 给了对偶预测误差 → 运行时间的界（Thm. 2），N06 给了解析基准的构造配方（Thm. 3.1）；缺的只是把 N03 的右端换成 cost gap
\end{itemize}
\end{frame}
...
{\footnotesize\setlength{\tabcolsep}{3pt}\begin{tabular}{@{}>{\RaggedRight\arraybackslash}p{0.200\textwidth}>{\RaggedRight\arraybackslash}p{0.119\textwidth}>{\RaggedRight\arraybackslash}p{0.651\textwidth}@{}}\toprule
\textbf{课题} & \textbf{评级} & \textbf{依据} \\\midrule
\textbf{① Balance 残差 → OT 误差界 + 对偶势证书} & \pos{做} & GFN 侧：N03 已证 TB 残差 → TV 界（Thm. 3.5）与抽样证书（Thm. 3.6）；OT 侧：N01 对偶预测误差 → 运行时间（Thm. 2）、Sinkhorn 势统计率、QOT 的 PL 不等式；O08 Thm. 3.3 互补松弛给逐边证书；\textbf{没有一篇把近似可行流的残差映射到 cost gap} \\\addlinespace[2pt]
