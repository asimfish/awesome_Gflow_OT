...
\begin{itemize}\small
\item \textbf{一般图上奖励不足以唯一确定内部流}：奖励只约束终止边，非终止边上的 $P_B$ 自由（T02 Prop. 18）；有环时再加一个环流锥（T19 Prop. 5）
...
\item \textbf{该做什么}：\HL{以对偶势为核心的 OT 最优性证书}。N03 已给 TB 残差 → TV 证书（Thm. 3.5/3.6），N01 给对偶预测误差 → 运行时间的界（Thm. 2）。但\textbf{零 balance 残差不认证最优性}（错配流反例：残差 $10^{-15}$、代价高 60–78\%），端点耦合代价 ≠ 执行代价（绕路反例：×3.2–3.5）——两者都只有弱对偶 gap 能判出（\texttt{experiments/}）。待解决：原始修复记账、隐式图上的全局证书、绕路项
\end{itemize}
\end{frame}
...
\item 写出「最小总流」约束优化 Eq. (11)；恒等式 $\sum_s F(s)=Z\,\mathbb E[n_\tau]$ 见 Prop. 3.12；近似解法 DB + 状态流正则
...
\begin{column}{0.235\textwidth}\begin{block}{精确 vs 训练}\footnotesize 定理是关于 \textbf{LP 最优解} 的；神经训练用带源分布项的正则化 TB 加软罚 $\lambda$（Eq. (20)），训练完的模型是否 OT 最优靠数值接近程度支撑，不由定理保证\end{block}\end{column}
\end{columns}
...
{\scriptsize\setlength{\tabcolsep}{3pt}\begin{tabular}{@{}>{\RaggedRight\arraybackslash}p{0.086\textwidth}>{\RaggedRight\arraybackslash}p{0.233\textwidth}>{\RaggedRight\arraybackslash}p{0.159\textwidth}>{\RaggedRight\arraybackslash}p{0.137\textwidth}>{\RaggedRight\arraybackslash}p{0.216\textwidth}>{\RaggedRight\arraybackslash}p{0.139\textwidth}@{}}\toprule
...
误差保证 & \textbf{LP 对偶 + 互补松弛} & Thm. 3.4 & 零定理 & 核查版本未见误差保证 & IMF 收敛无速率 \\\addlinespace[2pt]
最大规模 & $S_{20}$ & 3×3×3 Rubik & $n\le10^4$ & **$10^6$ 节点** & ZINC250K \\\addlinespace[2pt]
代码 & 未见 & 开源 & 开源 & \textbf{未公开} & 开源 \\\bottomrule\end{tabular}}
\vfill\hrule\vspace{2pt}{\footnotesize\textcolor{neutral}{GSBoG 与 O08 同象限、卖点句几乎相同（可执行策略而非静态耦合），却在主会且规模高四个数量级。反制点只有三个：隐式图、不定时域、以及 O08 有 LP 对偶证书而核查版本的 GSBoG 未见任一种明确误差保证。}}
\end{frame}

\section{2025–2026 趋势}

\begin{frame}{两侧同时成熟，接线的工作尚未见到}\framesubtitle{趋势判断（每条 ≥2 篇支撑）}
\begin{columns}[T]
...
\begin{frame}{六篇增补各补了主线的哪一块}\framesubtitle{2026 增补 · 六篇 relevance=5 的新论文已配深度解读（N01–N06）}
{\scriptsize\setlength{\tabcolsep}{3pt}\begin{tabular}{@{}>{\RaggedRight\arraybackslash}p{0.086\textwidth}>{\RaggedRight\arraybackslash}p{0.252\textwidth}>{\RaggedRight\arraybackslash}p{0.106\textwidth}>{\RaggedRight\arraybackslash}p{0.526\textwidth}@{}}\toprule
\textbf{编号} & \textbf{论文} & \textbf{发表} & \textbf{对主线的作用} \\\midrule
N01 & Minimum-Cost Network Flow with Dual Predictions & AAAI 2026 & 课题④的正确形态：GFN 状态流当对偶预测喂 ε-relaxation，$O(\min\{n^3\log\|\hat p-p^\star\|_\infty,\ n^3\log(nC)\})$（Thm. 2） \\\addlinespace[2pt]
N02 & Stop the Sampler! & ICML 2026 Workshop & 「期望长度 = 总流」的第三个出口（MCMC 终止）；固定核下最小流有闭式最优值 $n_Q^\star$（Cor. 3.7） \\\addlinespace[2pt]
N03 & Stable GFlowNets with TV Monitoring & 预印本 & \pos{课题①已完成的一半}：TB 残差 $\le c^2$ ⇒ TV $\le1-e^{-2c}$（Thm. 3.5）；抽样证书与状态空间大小无关（Thm. 3.6） \\\addlinespace[2pt]
N04 & Neural OT on Metric Graphs & 预印本 & 图上 OT 的连续边分支，与 O08 互补（对象、代价、输出全不同）；「评估噪声底」协议值得照搬 \\\addlinespace[2pt]
N05 & Orlicz-Sobolev Unbalanced Graph OT & NeurIPS 2025 Spotlight & 放开 $\sum L=\sum R=1$ 的非 KL 候选：质量差由线性项吸收，一元优化可算（Thm. 4.2） \\\addlinespace[2pt]
N06 & Discrete SB / EOT Benchmark & \con{ICLR 2026 主会} & 熵正则路线的必报基准（Thm. 3.1 解析解构造）；课题③窗口关闭的强证据；配方可移植成 GFN–OT 解析基准 \\\bottomrule\end{tabular}}
\vfill\hrule\vspace{2pt}{\footnotesize\textcolor{neutral}{六篇均已下载、DeepSeek 保版式中译（QA 2–11 issues）并写入 reports/N0*.md；N06 的发表状态由 PDF 页眉核实，趋势扫描阶段仅凭 arXiv comment 记为预印本是错的。}}
\end{frame}

\begin{frame}{窗口状态与最终评级}\framesubtitle{四个候选课题}
{\footnotesize\setlength{\tabcolsep}{3pt}\begin{tabular}{@{}>{\RaggedRight\arraybackslash}p{0.187\textwidth}>{\RaggedRight\arraybackslash}p{0.111\textwidth}>{\RaggedRight\arraybackslash}p{0.671\textwidth}@{}}\toprule
\textbf{课题} & \textbf{评级} & \textbf{依据} \\\midrule
\textbf{① Balance 残差 → OT 误差界 + 对偶势证书} & \pos{做} & GFN 侧：N03 已证 TB 残差 → TV 界（Thm. 3.5）与抽样证书（Thm. 3.6）；OT 侧：N01 对偶预测误差 → 运行时间（Thm. 2）、Sinkhorn 势统计率、QOT 的 PL 不等式；O08 App. A.4 的弱对偶 + 互补松弛给证书骨架。\textbf{本仓库核查范围内未见为 GFlowNet 做的 OT 最优性证书}（经典 LP 后验误差分析需补查）；真正未解决的是原始修复记账、隐式图全局证书、绕路项 \\\addlinespace[2pt]
② 条件 GFN 摊销一族图上 OT & \con{不做}（降为对照） & UNOT、ULOT、切片势摊销、min-sliced 四篇以上；唯一护城河是隐式图，但「一族隐式图」怎么定义没人说清 \\\addlinespace[2pt]
③ 熵正则 GFN–OT / 图上 SB & \con{不做}（作①的 ε>0 推广） & GSBoG 占同象限主会位；Sampling Decisions 已把 GFN 流函数写成单侧 Schrödinger 传输；N06（ICLR 2026）给了离散 SB 的解析解基准，新进入者只能在既定口径下比数 \\\addlinespace[2pt]
④ GFN proposal + 经典 OT 修正 & 改形态 & 已被 N01 具体化为「对偶预测 → ε-relaxation，预测误差映射到运行时间」（Thm. 2）；可作①的应用：把学到的状态流当对偶预测喂经典求解器——但只在显式图上有意义 \\\bottomrule\end{tabular}}
...
\item \textbf{证书}：由状态流构造对偶可行 $\pi$，报 primal–dual gap；同时按 N03 Thm. 3.6 的格式报抽样 TV 证书（$m$ 条目标轨迹 + $n$ 条模型轨迹取最大损失）；核心图：x = balance 残差，y = cost gap
...
\begin{columns}[T]
\begin{column}{0.235\textwidth}\begin{block}{24 篇深度解读}\footnotesize 18 篇核心 + 6 篇 2026 增补（N01–N06）；严格 8 节模板，每个定理、数字带原文出处，末尾「编者注」记录自主决定的歧义。另有竞品矩阵、两份趋势扫描、INSIGHTS 综合\end{block}\end{column}
\begin{column}{0.235\textwidth}\begin{block}{24 篇保版式中译 PDF}\footnotesize SuperTranslate：公式/图表/引用冻结，正文按原坐标回填；每篇附对象级 QA（inspect.json）。O01 讲义 480 页用 Gemini 2.5 Flash 后端，其余用 DeepSeek 后端\end{block}\end{column}
\begin{column}{0.235\textwidth}\begin{block}{汇总报告 中/英}\footnotesize 由 \texttt{scripts/build\_report.py} 从解读报告拼装，pandoc + xelatex 编译：中文 92 页、英文 121 页（Gemini 分块翻译，LaTeX 与引用逐字保留）\end{block}\end{column}
