...
3. **The least-crowded follow-up is an optimality certificate.** Balance residuals certify total-variation error of the terminal distribution (N03 Thm. 3.5/3.6) but a zero residual never certifies minimal total flow (a flow can be perfectly balanced and route everything the long way). A certificate therefore needs primal-feasibility repair + a dual-feasible potential + the optimality gap (O08 App. A.4 weak duality); on implicit graphs a computable *global* certificate is still open. Both failure modes are reproduced numerically in [experiments/](experiments/README.md): a perfectly balanced but crossed flow costs 60-78% more than OT\*, and routing the optimal coupling through detours multiplies the executed cost by 3.2-3.5 while the balance residual stays at 1e-15. Within the literature we checked, nobody has done this for GFlowNets; classical LP posterior-error analysis must be surveyed before claiming novelty. See `reports/INSIGHTS.md` §6b.
...
| 2026 trend scans | [reports/TRENDS_GFN_2026.md](reports/TRENDS_GFN_2026.md), [reports/TRENDS_OT_2026.md](reports/TRENDS_OT_2026.md) |
| Day-1 reference-LP experiment (GFlow\* = OT\*, dual certificate, two counterexamples) | [experiments/README.md](experiments/README.md) |
| External review round (Codex CLI, gpt-6-astra, 7 structured reviews) | [reviews/codex/](reviews/codex/) |
